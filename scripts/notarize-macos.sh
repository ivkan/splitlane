#!/usr/bin/env bash
# Notarize + staple a signed Splitlane .app bundle.
#
# Run this on a macOS runner AFTER `scripts/sign-macos.sh` has
# stamped a Developer ID signature on `dist/Splitlane.app`. After this
# script succeeds, the bundle is ready for `.dmg` assembly (`scripts/create-dmg.sh`).
#
...
#
# Usage:
#   scripts/notarize-macos.sh                      # notarizes dist/Splitlane.app
#   scripts/notarize-macos.sh path/to/MyApp.app
set -euo pipefail

APP="${1:-dist/Splitlane.app}"
...
while true; do
    ...
    case "$STATUS" in
        ...
        Invalid|Rejected)
            ...
            # Surface the developer log on rejection so the root
