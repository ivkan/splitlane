#!/usr/bin/env bash
# Sign a Splitlane .app bundle with the Apple Developer ID certificate.
#
# Run this on a macOS runner AFTER `scripts/bundle-macos.sh`
# has produced `dist/Splitlane.app`, and BEFORE `scripts/notarize-macos.sh`.
#
...
#
# Usage:
#   scripts/sign-macos.sh                      # signs dist/Splitlane.app with release entitlements
#   scripts/sign-macos.sh path/to/MyApp.app
#   scripts/sign-macos.sh --entitlements packaging/macos/splitlane.dev.entitlements
#   scripts/sign-macos.sh --entitlements packaging/macos/splitlane.nightly.entitlements path/to/MyApp.app
#
# Arguments:
#   --entitlements <path>    Entitlements plist to embed at signing time.
#                            Defaults to packaging/macos/splitlane.entitlements
#                            (three supported variants: release, dev,
#                            nightly).
set -euo pipefail
...
ENTITLEMENTS="$REPO_ROOT/packaging/macos/splitlane.entitlements"
APP=""
...
#   1) ABCDE12345... "Developer ID Application: Your Name (TEAM1234AB)"
...
#
# bundle-macos.sh today produces a flat bundle (Contents/MacOS/splitlane +
# Contents/Resources/Splitlane.icns), so the find loops below match nothing
# and exit immediately. Kept in place so that when Splitlane grows a helper
...
# nested structure. This is the embedded sanity check plus the
# precursor to notarize-macos.sh's `spctl --assess`.
