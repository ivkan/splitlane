...
#
# Run this on a macOS runner AFTER `scripts/sign-macos.sh` and
# `scripts/notarize-macos.sh` have produced a signed, stapled bundle at
# `dist/Splitlane.app`.
#
# Output filename convention:
#   dist/splitlane-<version>-<arch>-apple-darwin.dmg
#
# The `-apple-darwin` suffix is required: `update_checker.rs::pick_asset`
# matches `.dmg` assets with `AssetFormat::target_qualifier() ==
...
VOLNAME="Splitlane"
FINAL_DMG="$REPO_ROOT/dist/splitlane-${VERSION}-${ARCH}-apple-darwin.dmg"
...
# Codesign inside the DMG must still verify. Mount the final image
