...
//!
//! Proves GPUI's Taffy-backed flexbox distributes space correctly
//! across N > 2 children using `flex_basis(relative(ratio))`.
//!
//! Validates the GPUI flexbox proxy used by `LayoutTree::render()`,
...
/// A flex_row container with 3 children using ratios [0.33, 0.33, 0.34]
...
/// A flex_row container with 5 children, each ratio = 0.2, renders
...
/// Ratios that don't sum to exactly 1.0 (0.33+0.33+0.33=0.99) must not
...
// ===========================================================================
// Render N-ary tree via GPUI flexbox
//
...
/// A 3-child container with ratios [0.33, 0.33, 0.34] and 4px dividers
...
/// A 4-level deep nested tree renders without stack overflow or visual glitch.
