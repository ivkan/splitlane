//! Atomic, idempotent extraction of the embedded AI-hook
//! binaries into the user's per-OS cache directory.
//!
//! Layout produced by `ensure_binaries_extracted`:
//!
//! ```text
//! <dirs::cache_dir()>/splitlane/bin/<version>/
//!     ├── claude[.exe]            ← copy of splitlane-shim
//!     ├── codex[.exe]             ← copy of splitlane-shim
//!     ├── …one per TerminalAgent binary (gemini, cursor-agent, …)
//!     └── splitlane-ai-hook[.exe]  ← copy of splitlane-ai-hook
//! ```
//!
...
//!
//! The `splitlane-mcp` bridge takes a **different** path.
//! The shim/ai-hook helpers live in the version-pinned cache above because
//! Splitlane re-resolves them on every launch. The bridge path, by contrast,
//! is written into external, persistent agent configs by `splitlane mcp
//! install`, so it must NOT change across Splitlane updates. It is extracted
...
//! (PTY spawn) can log-and-continue without the PATH-prepend instead of
//! aborting the user's terminal session. A hook-wiring failure must stay
//! silent outside the PTY - the caller, not this module, owns the
//! log-and-skip policy.
...
/// `build.rs` via `cargo:rustc-env=SPLITLANE_TARGET_TRIPLE=<triple>` (see
/// `src-app/build.rs`). Used to look up the correct sub-folder inside the
/// `Bins` `RustEmbed` archive.
const TARGET_TRIPLE: &str = env!("SPLITLANE_TARGET_TRIPLE");
...
/// being installed): probing Splitlane's own `$PATH` would silently disable
...
fn extract_plan() -> Vec<(&'static str, &'static str)> {
    let mut plan: Vec<(&'static str, &'static str)> = crate::agent_launcher::TerminalAgent::ALL
        .iter()
        .map(|agent| (agent.binary(), "splitlane-shim"))
        .collect();
    plan.push(("splitlane-ai-hook", "splitlane-ai-hook"));
    plan
}
...
fn embedded_bytes(name: &str) -> Result<std::borrow::Cow<'static, [u8]>> {
    ...
    Bins::get(&key)
        .map(|f| f.data)
        .ok_or_else(|| anyhow!("embed entry {key} missing - did build.rs stage it?"))
}
...
/// `<dirs::cache_dir()>/splitlane/bin/<version>/` and return the
...
/// `src-app/src/terminal/pty_session.rs::inject_ai_hook_env`.
...
fn ensure_binaries_extracted_uncached() -> Result<PathBuf> {
    ...
    {
        Err(anyhow!(
            "packaged Windows helper dir missing or incomplete; refusing to execute helper binaries from the per-user cache"
        ))
    }
    ...
    #[cfg(any(not(windows), debug_assertions))]
    {
        let cache_root = dirs::cache_dir()
            .ok_or_else(|| anyhow!("dirs::cache_dir() returned None; cannot extract"))?;
...
/// under the application directory (`Program Files\Splitlane\bin`) and the
...
/// Materialize the embedded `splitlane-mcp` bridge at the
...
/// `ensure_binaries_extracted`, but targets `data_dir()/splitlane/bin/`
/// instead of the version-pinned cache so the path written into external
/// agent configs survives Splitlane updates. When the embedded bytes differ
/// from what is on disk (a new Splitlane version shipped a newer bridge), the
...
/// at launch logs a warn and continues (the GUI still opens; `splitlane mcp
...
pub fn ensure_bridge_extracted() -> Result<PathBuf> {
    let bridge_path = crate::runtime_paths::bridge_binary_path().ok_or_else(|| {
        anyhow!("data_dir() unresolvable/unwritable; cannot extract splitlane-mcp")
    })?;
    let target_dir = bridge_path
        .parent()
        .ok_or_else(|| anyhow!("bridge path {} has no parent", bridge_path.display()))?
        .to_path_buf();
    let filename = bridge_path
        .file_name()
        .ok_or_else(|| anyhow!("bridge path {} has no filename", bridge_path.display()))?
        .to_string_lossy()
        .into_owned();

    // Embed source basename matches the bridge filename on every OS
    // (`splitlane-mcp` / `splitlane-mcp.exe`).
...
/// Materialize the embedded `splitlane-ai-hook` callback at the
/// stable, non-versioned path returned by
...
/// `splitlane hooks setup` can write a durable path into external agent configs
/// that survives Splitlane updates - unlike the version-pinned cache copy the
/// shim resolves at launch.
///
/// Unhappy path: `data_dir()` unresolvable -> `ai_hook_binary_path()` is `None`
/// -> `Err`; `splitlane hooks setup` then refuses cleanly rather than writing a
...
pub fn ensure_ai_hook_extracted() -> Result<PathBuf> {
    let hook_path = crate::runtime_paths::ai_hook_binary_path().ok_or_else(|| {
        anyhow!("data_dir() unresolvable/unwritable; cannot extract splitlane-ai-hook")
    })?;
    let target_dir = hook_path
        .parent()
        .ok_or_else(|| anyhow!("ai-hook path {} has no parent", hook_path.display()))?
        .to_path_buf();
    let filename = hook_path
        .file_name()
        .ok_or_else(|| anyhow!("ai-hook path {} has no filename", hook_path.display()))?
        .to_string_lossy()
        .into_owned();

    // Embed source basename matches the ai-hook filename on every OS
    // (`splitlane-ai-hook` / `splitlane-ai-hook.exe`).
    let bytes = embedded_bytes(&filename)?;
    let entry = Entry {
        filename,
        bytes: bytes.as_ref(),
...
pub(crate) fn extract_into(entries: &[Entry<'_>], target_dir: &Path) -> Result<()> {
    std::fs::create_dir_all(target_dir)
        .with_context(|| format!("create cache dir {} failed", target_dir.display()))?;
    ...
    for entry in entries {
        ...
        {
            return Err(anyhow!(
                "refusing to extract entry with non-basename filename {:?}",
                entry.filename
            ));
        }
        ...
        write_atomic(&final_path, entry.bytes)
            .with_context(|| format!("atomic write of {} failed", final_path.display()))?;
...
fn write_atomic(final_path: &Path, bytes: &[u8]) -> Result<()> {
    let parent = final_path
        .parent()
        .ok_or_else(|| anyhow!("{} has no parent dir", final_path.display()))?;
    ...
    tmp.write_all(bytes)
        .context("write_all to tempfile failed")?;
    tmp.as_file_mut()
        .sync_all()
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(tmp.path(), perms)
            .with_context(|| format!("chmod 0o755 on {} failed", tmp.path().display()))?;
    }
        ...
        loop {
            match tmp.persist(final_path) {
                ...
                Err(e) => {
                    ...
                    return Err(anyhow!(
                        "atomic rename {} -> {} failed after {attempt} attempt(s): {}",
                        e.file.path().display(),
                        final_path.display(),
                        e.error
                    ));
...
mod tests {
    ...
    const FAKE_SHIM: &[u8] = b"splitlane-shim synthetic bytes v0";
    const FAKE_HOOK: &[u8] = b"splitlane-ai-hook synthetic bytes v0";
    ...
    fn synthetic_entries() -> Vec<Entry<'static>> {
        ...
        vec![
            ...
            Entry {
                filename: format!("splitlane-ai-hook{suffix}"),
                bytes: FAKE_HOOK,
            },
        ]
    }
    ...
    fn extracts_all_three_filenames() {
        ...
        for expected in [
            ...
            format!("codex{suffix}"),
            format!("splitlane-ai-hook{suffix}"),
        ] {
    ...
    fn extracted_bytes_match_input_sha256() {
        ...
        for entry in &entries {
            ...
            assert_eq!(
                sha256(&on_disk),
                sha256(entry.bytes),
                "extracted {} must SHA256-match the input bytes",
                p.display()
            );
        }
    }
    ...
    fn shim_copies_are_identical() {
        ...
        assert_eq!(
            claude, codex,
            "claude and codex are both copies of splitlane-shim"
        );
    }
    ...
    fn re_extraction_is_noop() {
        ...
        for (p, first_mtime) in first_mtimes {
            ...
            assert_eq!(
                first_mtime,
                second_mtime,
                "re-extraction of unchanged bytes must be a no-op (mtime unchanged) for {}",
                p.display()
            );
        }
    }
    ...
    fn stale_bytes_are_overwritten() {
        ...
        assert_eq!(
            ...
            "stale bytes must be overwritten by the current embed"
        );
    }
    ...
    fn unix_mode_is_0o755() {
        ...
        for entry in &entries {
            ...
            assert_eq!(
                mode & 0o777,
                0o755,
                "{} must be mode 0o755 on Unix, got 0o{:o}",
                p.display(),
                mode & 0o777
            );
        }
    ...
    fn bins_embed_contains_all_staged_binaries() {
        ...
        // `splitlane-mcp` is included too.
        let suffix = exe_suffix();
        for src in ["splitlane-shim", "splitlane-ai-hook", "splitlane-mcp"] {
            let name = format!("{src}{suffix}");
            let bytes = embedded_bytes(&name)
                .unwrap_or_else(|e| panic!("Bins must contain `bin/{TARGET_TRIPLE}/{name}`: {e}"));
            assert!(!bytes.is_empty(), "embedded {name} must be non-empty");
        }
    }
    ...
    fn ensure_binaries_extracted_produces_all_agent_wrappers() {
        ...
        let mut expected: Vec<String> = crate::agent_launcher::TerminalAgent::ALL
            ...
            .collect();
        expected.push(format!("splitlane-ai-hook{suffix}"));
        for name in expected {
            let p = dir.join(&name);
            assert!(
                p.is_file(),
                "ensure_binaries_extracted must produce {}",
                p.display()
            );
        }
    }
    ...
    fn wrapped_stems_match_shim_detect_list() {
        ...
        // so the mirror in `splitlane-shim/src/detect.rs` gets updated in
        // the same change.
    ...
    fn ensure_bridge_extracted_produces_stable_path() {
        // End-to-end smoke: extract the bridge to the real
        ...
        assert!(
            path.is_file(),
            "ensure_bridge_extracted must produce {}",
            path.display()
        );
        let suffix = exe_suffix();
        assert_eq!(
            path.file_name().unwrap().to_string_lossy(),
            format!("splitlane-mcp{suffix}"),
            "bridge filename must be splitlane-mcp[.exe]"
        );
    }

    #[test]
    fn bridge_path_is_non_versioned_and_distinct_from_cache() {
        // The bridge must live at a NON-versioned path
        // (no `CARGO_PKG_VERSION` component) and be distinct from the
        // version-pinned helper cache, so a Splitlane update does not
        ...
        assert!(
            ...
            "bridge path {bridge_str} must NOT embed the version {version}"
        );
        ...
        if let Ok(cache) = ensure_binaries_extracted() {
            assert_ne!(
                bridge.parent(),
                Some(cache.as_path()),
                "bridge dir must differ from the versioned cache dir"
            );
        }
    }
    ...
    fn rejects_non_basename_filenames() {
        ...
        for bad in bad_cases {
            ...
            assert!(
                ...
                "rejection for {bad:?} must mention 'non-basename'; got {err}"
            );
        }
    }
    ...
    fn returns_err_when_parent_is_readonly() {
        // Unhappy path: "extraction failure (permission denied, disk
        ...
        assert!(
            res.is_err(),
            "extraction into a read-only parent must return Err"
        );
    }
}
