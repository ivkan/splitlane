//! Theme cache + event-driven hot-reload from `splitlane.json`.
//!
...
//!
//!   1. **Event-driven (preferred)** - [`ThemeWatcher`] installs a
...
//!
//! The watched file is `splitlane.json` itself - the theme is the
...
use super::builtin::theme_by_name;
...
/// 300 ms debounce window for `notify` events. Mirrors
/// `splitlane_config::watcher::DEBOUNCE_DURATION` so editor saves that
...
/// Set to `true` once a [`ThemeWatcher`] has installed an OS watcher
...
/// Read the theme name from the Splitlane config file.
/// Returns None if the file doesn't exist or has no theme set.
fn read_config_theme_name() -> Option<String> {
    splitlane_config::loader::load_config().theme
}

/// Resolve the theme from config, falling back to the default theme.
fn resolve_theme() -> TerminalTheme {
    ...
    apply_surface_overrides(
        theme_by_name(super::DEFAULT_THEME_NAME).expect("the default theme is bundled"),
    )
}
...
pub fn config_mtime() -> Option<SystemTime> {
    let config_path = splitlane_config::loader::config_path()?;
...
// =====================================================================
// Event-driven theme watcher
// =====================================================================

/// Watches `splitlane.json` for changes and invalidates the theme cache
/// when the file is modified. Mirrors `splitlane_config::watcher::ConfigWatcher`
...
/// caller can fall back to the polling path.
...
impl ThemeWatcher {
    ...
    /// [`splitlane_config::loader::config_path`]. Returns `None` when the
    ...
    fn install_watcher(&self) -> Result<(), notify::Error> {
        ...
        // a file inside `<config_dir>/splitlane/` - `.parent()` is always Some.
        #[allow(clippy::expect_used)]
        let watch_dir = self
            .config_path
            .parent()
...
mod tests {
    ...
    /// elapses. Mirrors the helper in `splitlane_config::watcher::tests` -
    ...
    /// A relevant FS event triggers the user callback after
    ...
    /// When init fails (here, by passing a non-existent
    ...
    fn test_theme_watcher_start_failure_keeps_polling_fallback() {
        ...
        // the three is acceptable here - we only need to assert
        // the global flag is left in a polling-fallback state.
        let bogus = PathBuf::from("/proc/self/__splitlane_us006_test/splitlane.json");
        ...
        assert!(
            ...
            "WATCHER_ACTIVE must be false after init failure so the \
             500ms polling fallback in active_theme() takes over"
        );
    }

    /// Hardening - the second `start()` call must NOT install a
    ...
    /// Documents the cooperative-shutdown contract: dropping
