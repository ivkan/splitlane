...
//! extracted from `event_handlers.rs` (code motion only).
...
impl SplitlaneApp {
    ...
    /// persisted to the session. The seeded worktrees are then filled with
    /// real diff columns.
    /// Gather the sibling-worktree seed for a repo: one [`crate::diff::DiffWorktree`]
    /// per open workspace whose `repo_root` matches. This was extracted from
    /// `open_multi_diff_for_repo` so the dedicated Diff mode (`rebuild_diff_view`) and the legacy tab path
    ...
    /// The active workspace as a single-element worktree seed (Project
    ...
    /// Every open workspace grouped by canonicalized `repo_root`
    ...
    pub(crate) fn open_multi_diff_for_repo(
        ...
        // The pane the screen says is focused - one home for that answer, so
        // this cannot come to differ from what the border and the rail draw.
        let Some(target_pane) = self.focused_pane_as_shown(window, cx) else {
            return;
        };

        let diff = cx.new(|cx| crate::diff::DiffView::new(repo_root, worktrees, cx));
        target_pane.update(cx, |pane, cx| {
            pane.show_diff(diff, cx);
            cx.notify();
        });
        cx.notify();
    }
...
/// Normalize a worktree path for dedup so the same checkout only gets
