...
use crate::{SplitlaneApp, ThemeMode, config_writer};

pub(crate) fn is_default_theme_name(name: &str) -> bool {
    name.eq_ignore_ascii_case(crate::theme::DEFAULT_THEME_NAME)
        || name.eq_ignore_ascii_case(crate::theme::DEFAULT_LIGHT_THEME_NAME)
}

impl SplitlaneApp {
    /// Resolve the theme currently persisted in config (or the built-in
    /// default). Reads the cached config, not a per-call `load_config()`.
    pub(crate) fn current_theme_name(&self) -> String {
        self.cached_config
            .theme
            .clone()
            .unwrap_or_else(|| crate::theme::DEFAULT_THEME_NAME.to_string())
    }
    ...
    pub(crate) fn render_theme_picker(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if matches.is_empty() {
            ...
            for (idx, name) in matches.iter().enumerate() {
                ...
                let label = if *name == crate::theme::DEFAULT_THEME_NAME {
                    format!("{} (Default)", name)
                } else {
                    (*name).to_string()
                };
