//! Settings lifecycle + persistence + key handlers for `SplitlaneApp`.
//!
...
//! module owns the glue on `SplitlaneApp`:
//! - [`SplitlaneApp::open_settings_window`] / [`SplitlaneApp::close_settings`] -
//!   toggle the embedded settings (set/clear `settings_section`).
//! - [`SplitlaneApp::persist_setting`] - the shared cache-mutate + repaint +
//!   off-thread write used by every settings control.
//! - [`SplitlaneApp::handle_settings_key_down`] /
//!   [`SplitlaneApp::handle_shortcut_recording`] - key routing for the font-picker
//!   typeahead, Escape handling, and shortcut capture.

use gpui::{Context, KeyDownEvent, ScrollHandle, Window};

use crate::{SettingsSection, SplitlaneApp, config_writer, keybindings};
...
impl SplitlaneApp {
    ...
    pub(crate) fn handle_settings_key_down(
        ...
        if event.keystroke.key == "escape" && self.recording_shortcut_idx.is_none() {
            if self.terminal_dropdown.is_some() {
                ...
                self.general_dropdown = None;
            } else if self.appearance_theme_menu_open {
                self.appearance_theme_menu_open = false;
            } else {
    ...
    pub(crate) fn handle_shortcut_recording(
        ...
        // would rebind the wrong action and corrupt `splitlane.json`).
