...
use crate::{
    DismissUpdate, SplitlaneApp, StartSelfUpdate, TOAST_HOLD_MS, ToastAction,
    system_package_update_command, update,
};

/// App-level backstop for a wedged `Downloading`/`Installing` state.
/// Every installer worker is spawned + detached and the only
...
/// `system_package::tests::version_validators_agree` exists to guard the two
...
impl SplitlaneApp {
        ...
        match &self.self_update.update_status {
            Some(update::checker::UpdateStatus::Available { version, .. }) => {
                let kind = match &self.self_update.install_method {
                    ...
                    // Flatpak / Snap / `SPLITLANE_UPDATE_EXPLANATION` -
    ...
    /// [`SplitlaneApp::kickoff_self_update_install`] so that auto-kickoff
    ...
    pub(crate) fn handle_start_self_update(
        ...
        // A click that ends this process goes through the exit question, so a
        // running agent's turn is not lost without a word. Every other path
        // (download, pre-install, clipboard hints) leaves the process alone and
        // starts at once.
        ...
        if ends_process {
            self.request_exit(crate::app::exit_guard::ExitIntent::Update, cx);
        } else {
    ...
    /// Intentionally NOT persisted - the next splitlane launch will
    ...
    /// Shared completion for every pre-installed update path. Flips to
    ...
    /// Enter the `Downloading` state and arm a one-shot watchdog.
    /// Replaces the bare `self_update_status = Downloading;
    ...
    /// [`SplitlaneApp::record_update_failure`] (which leaves the busy state,
    ...
    pub(crate) fn kickoff_self_update_install(&mut self, cx: &mut Context<Self>) {
        ...
        // `SPLITLANE_UPDATE_EXPLANATION`). The in-app updater is disabled
        ...
        // Ubuntu / Debian users on a signed rpm/deb repository get an
        // in-app pkexec-elevated `dnf|apt-get|zypper install`.
            ...
            &self.self_update.install_method
        {
            ...
            // toast, or argv. The installer already regex-validates inside
            // `run_update`, but the `PackageManager::Other` branch and
            // the `EnvironmentBroken` fallback both construct a
            // user-visible "Copied: sudo apt install splitlane=<ver>"
            ...
            // rpm-ostree (Silverblue / Kinoite / Bazzite).
            ...
            if matches!(manager, update::install_method::PackageManager::RpmOstree) {
                ...
                self.push_toast(
                    "Splitlane detects an immutable distribution. Update must be run via `rpm-ostree upgrade` at the system level - the update has been copied to your clipboard.".to_string(),
                    Vec::new(),
                    TOAST_HOLD_MS * 4,
                    cx,
                );
            ...
            // Same code path as before the in-app install. Also used as the
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                cx.spawn(async move |this, cx| {
                    ...
                    match result {
                        Ok(()) => {
                            let restart_path = std::path::PathBuf::from("/usr/bin/splitlane");
                            let _ = this.update(cx, |app, cx| {
                            ...
                            cx.update(|cx| {
                                log::info!(
                                    "self-update/{manager_label}: pre-installed - restart pending at /usr/bin/splitlane"
                                );
        ...
        // points the user at the releases page. Skipping the
        ...
        if self.self_update.update_attempt_count >= 3 {
            let releases_url = match &self.self_update.update_status {
                ...
                _ => "https://github.com/ivkan/splitlane/releases".to_string(),
            };
            self.push_toast(
        ...
        // before the signing keys were provisioned). Refuse to start ANY
        ...
        if !update::signature::has_embedded_key() {
            self.push_toast(
                ...
                vec![ToastAction::OpenReleasesPage(
                    "https://github.com/ivkan/splitlane/releases".to_string(),
                )],
                TOAST_HOLD_MS * 4,
                cx,
            );
        ...
        if let update::install_method::InstallMethod::AppImage { source_path, .. } = &method {
            ...
            cx.spawn(async move |this, cx| {
                let result = smol::unblock({
                    ...
                    // Pass the new asset URL so run_update can fetch
        ...
        // `~/.local/splitlane.app/` is the portable install target. macOS
        ...
        if route_to_targz {
            // Atomic directory swap under `$HOME/.local/splitlane.app/`.
            ...
            // still proceeds - the install lands at `$HOME/.local/splitlane.app/`
            ...
            if matches!(&method, update::install_method::InstallMethod::Unknown) {
                log::warn!(
                    "self-update: install method Unknown - downloading tar.gz release \
                     into $HOME/.local/splitlane.app/; the updated binary will be at a \
        ...
        // Windows MSI install. Unlike AppImage/TarGz/DMG, the MSI
        // cannot be fully pre-installed while Splitlane is still running:
        // Windows Installer would detect the live `splitlane.exe`, show its
                    ...
                    Ok(staged) => {
                        let _ = this.update(cx, |app, cx| {
                            ...
                            match update::windows::msi::spawn_relay(staged) {
                                Ok(()) => {
                                    log::info!(
                                        "self-update/msi: relay spawned - quitting so msiexec can replace splitlane.exe"
                                    );
                                    app.self_update.self_update_status =
        ...
        // macOS `.app` bundle - mount the DMG, swap the bundle
        ...
        if let update::install_method::InstallMethod::AppBundle { bundle_path } = &method {
            let url = asset_url.clone();
            // Replace the bundle at its detected location, not a
            // hardcoded /Applications path.
            let bundle = bundle_path.clone();
            // `dmg::install` runs `hdiutil attach/detach` + `cp` on
    ...
    ///   waits for an explicit click because it must quit Splitlane before
