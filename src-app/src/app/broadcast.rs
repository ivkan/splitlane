//! Broadcast Groups (CLI Cockpit).
//!
//! Named groups of panes sharing a colored left-edge stripe - the
//! explicit, visible target of a Composer broadcast. Groups are
//! session-volatile by design (persistence is a
//! future extension), live on `SplitlaneApp` (single-thread GPUI state,
...
pub(crate) struct BroadcastGroup<Id = gpui::EntityId> {
    ...
    /// always intersect with the live leaves (a closed pane
    /// disappears silently; an empty group stays valid).
    pub(crate) members: Vec<Id>,
}

/// All broadcast state owned by `SplitlaneApp`. Volatile - none of it is
...
/// so joining the active group removes it from any other.
...
/// A prompt may only be DELIVERED (pre-filled) into a pane whose
/// mapped session is not generating. A pane with no agent session is
/// always safe (the gate never requires an agent - bare shells
/// receive the prefill identically).
///
/// `Stalled` blocks too - a stalled session is a `Thinking`
/// session that went silent, i.e. the agent may STILL be mid-generation
/// (the false-positive case of stall detection). Treating it as safe would
/// reopen the exact stdin-corruption window this gate exists to close. The
...
impl SplitlaneApp {
    ...
    pub(crate) fn focused_or_first_pane(
        ...
        self.focused_pane_as_shown(window, cx)
    }
    ...
    pub(crate) fn handle_toggle_broadcast_member(
        ...
        // Pane-scoped: see `handle_open_composer`.
        ...
        match self.broadcast.active {
            ...
            // state proposes creation - never a silent no-op.
    ...
    pub(crate) fn render_broadcast_picker(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        // Live member counts so a closed pane never inflates a row.
        let mut live: HashSet<gpui::EntityId> = HashSet::new();
        for ws in &self.workspaces {
            if let Some(root) = &ws.root {
                for pane in root.collect_leaves() {
        ...
        if self.broadcast.groups.is_empty() {
            // Explicit empty state proposing creation.
            card = card.child(
                div()
                    .px(tok::space::XL)
                    .py(tok::space::XL)
...
mod tests {
    ...
    fn toggle_moves_between_groups_one_group_per_pane() {
        // A pane belongs to at most one group - joining the
    ...
    fn validate_rejects_ninth_group_with_explicit_message() {
        // At 8 existing groups the 9th create is refused.
    ...
    fn only_generating_states_block_delivery() {
        // WaitingForInput / Finished / Errored (and absent sessions,
        // handled by the caller) are safe prefill targets. Thinking blocks;
        // Stalled blocks too (a stalled agent may still be
