...
use gpui::{
    ...
    Styled, Window, deferred, div, prelude::*, px, svg,
};
use splitlane_config::schema::ButtonCommand;
...
fn default_icon() -> String {
    // `.first()` instead of `[0]` - the const is non-empty today, but a
...
impl SplitlaneApp {
    ...
    fn render_modal_form(
        ...
        for &path in AVAILABLE_ICONS {
            ...
            // same fill every selected row in the app gets; only unselected
            // tiles animate on hover. It used to be the literal `#323232` that
            // fill happens to resolve to on a dark theme, which on a light one
            // drew a near-black tile in a white dialog.
            let tile = if is_selected {
                tile.bg(crate::app::constants::sidebar_tab_active_background())
                    .into_any_element()
            } else {
