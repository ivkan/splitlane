...
//!   helpers attached to `SplitlaneApp`.
//! - `render_toast`: the deferred rendering block used by `Render for
//!   SplitlaneApp` to paint the active toast.
...
use crate::{SplitlaneApp, StartSelfUpdate, update};
...
pub(crate) struct Toast {
    ...
    /// update-failure toasts with Retry / "Open releases" buttons.
    pub(crate) actions: Vec<ToastAction>,
    /// How long the "hold" phase of the toast animation lasts, in ms.
    /// Must match the auto-dismiss timer in [`SplitlaneApp::push_toast`] -
...
impl SplitlaneApp {
    ...
    /// Hold is extended so the user has time to click the button
    /// before auto-dismiss.
