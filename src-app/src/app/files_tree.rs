...
/// One entry in a directory listing. **A hidden entry is filtered out and an
/// ignored one is kept**, drawn at `DIMMED_OPACITY` - so `is_ignored` is a fact
/// the row reads, not a reason the node is absent.
...
impl FilesTreeState {
    ...
    /// directories. The root is always expanded; each persisted path
...
///
/// **Ignored entries are tinted, not withheld.** Hiding them made the panel
/// silently disagree with the directory: a `logs/` folder excluded by `logs/*`
/// with a handful of negated files showed a third of what was there, with
/// nothing on screen to say a filter had run - and the row renderer had been
/// carrying the dimming for it all along. The cost is bounded by the watcher's
/// own shape: it is non-recursive and installed per **expanded** directory, so
/// a `node_modules` nobody expands is one dim row and no watch.
...
/// re-read: dedup, then drop any path that has an ancestor also in
...
mod tests {
    ...
    #[test]
    fn read_dir_keeps_gitignored_entries_and_filters_hidden_ones() {
        ...
        // `.env` is gone; `target/` is present and marked, which is what the
        // row reads to dim it.
        assert_eq!(
            names,
            vec![
        ...
        let ignored: Vec<bool> = nodes.iter().map(|n| n.is_ignored).collect();
