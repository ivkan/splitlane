//! Stable canonical tags for the v1 desktop events.
//!
...
//! flatten into the canonical values the analytics vocabulary commits to.
//!
//! The format-invariant helper
//! [`splitlane_telemetry::tags::is_canonical_tag_format`] lives in the
...
pub fn install_method_tag(method: &InstallMethod) -> &'static str {
    match method {
        ...
        // `SPLITLANE_UPDATE_EXPLANATION` builds report a coarse tag
...
/// failed `update_installed` events. Buckets every
...
/// coarse default so the four-bucket contract stays honest.
...
mod tests {
    ...
    fn every_published_tag_satisfies_the_canonical_format_contract() {
        ...
        // `splitlane_telemetry::tags::is_canonical_tag_format`. Breaking
