...
/// Embedded AI-hook binaries.
///
/// `splitlane-shim` (mapped at extraction time to `claude` + `codex`) and
/// `splitlane-ai-hook` are staged into `src-app/target/embed/bin/<target>/`
/// by `build.rs` before rust-embed's proc-macro expands. Entries look like
/// `bin/<target-triple>/splitlane-shim[.exe]` and
/// `bin/<target-triple>/splitlane-ai-hook[.exe]`.
///
/// Not cfg-gated on target_os: Splitlane only builds for Linux, macOS, and
...
/// (wired into `terminal::pty_session::inject_ai_hook_env`).
#[derive(RustEmbed)]
