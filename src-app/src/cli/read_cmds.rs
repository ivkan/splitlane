//! Read surface of the CLI: `ls` / `read` / `search`.
//!
...
/// `splitlane ls [--human]` - list terminal surfaces.
...
/// `splitlane read <target> [--lines N] [--offset N] [--json] [--raw]`.
pub fn read(
    ...
    // surface.read fences its output as
...
/// `splitlane search <target> <pattern> [--max N] [--human]`.
pub fn search(
    client: &impl IpcTransport,
    target: &str,
    pattern: &str,
...
/// `splitlane ps [--json]` - list running agents across the fleet.
/// Defaults to a human table (like Unix `ps`); `--json` emits the
...
/// `splitlane status <target> [--json]` - read one surface's agent state.
/// Defaults to a one-line summary; `--json` emits the full envelope.
