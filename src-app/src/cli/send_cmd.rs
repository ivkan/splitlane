...
//! passed explicitly, the only sanctioned submission path.
//! `key` refuses submitting keystrokes (`enter`, `ctrl-m`, …) server-side.
//!
//! Both methods are gated behind `SPLITLANE_IPC_SCRIPTING=1` on the RUNNING
//! instance (the gate is read from Splitlane's own process env, not the
...
/// `splitlane send <target> <text> [--broadcast] [--submit] [--paste]`.
pub fn send(
...
/// non-zero `CliError` and the `-32601` gate-off reply to an
/// actionable hint.
fn send_to(
    client: &impl IpcTransport,
    surface_id: u64,
    ...
    // Only forward `paste` when the user explicitly passed
...
/// failed. The gate-off error DOES abort: every send would fail the
...
fn send_broadcast(
    ...
    for id in ids {
        match send_to(client, id, text, submit, paste) {
            ...
            Err(e) if e.message.contains("SPLITLANE_IPC_SCRIPTING") && sent.is_empty() => {
                return Err(e);
            }
...
/// `splitlane key <target> <keystroke>`. Wraps `surface.send_keystroke`; the
/// server refuses CR/LF-resolving keystrokes (`enter`, `ctrl-m`, `ctrl-j`) so
/// submission stays exclusive to `send --submit`.
...
mod tests {
    ...
    fn send_passes_submit_flag_through() {
        ...
        // No `--paste` -> the key is omitted so the server
    ...
    fn paste_flag_is_forwarded_only_when_set() {
        // `--paste` -> the param rides through to the server.
    ...
    fn send_multi_match_without_broadcast_is_target_error() {
        ...
        // pick one silently (single-target keeps its existing semantics).
        let fake = ScriptedTransport::new(vec![]);
    ...
    fn broadcast_partial_failure_serves_the_rest_and_exits_nonzero() {
        ...
        // must still be served and the exit must be non-zero.
        let fake = ScriptedTransport::new(vec![
    ...
    fn broadcast_gate_off_aborts_with_actionable_hint() {
        ...
        assert!(err.message.contains("SPLITLANE_IPC_SCRIPTING"));
    ...
    fn report_file_adds_file_contract_to_sent_prompt() {
        ...
        assert!(text.contains("Splitlane report protocol"));
    ...
    fn key_enter_refusal_is_nonzero_exit() {
        ...
        // non-zero and surface the `send --submit` hint.
        let fake = ScriptedTransport::new(vec![Ok(
            json!({ "error": "keystroke 'enter' would submit (CR/LF); use surface.send_text with submit=true (`splitlane send --submit`) instead" }),
        )]);
    ...
    fn submit_forwards_a_full_64_kib_payload_intact() {
        // The one explicitly-mandated stress case - a 64 KiB text
