//! `splitlane up <file>` - declarative agent workspaces, with a
//! worktree per agent.
//!
//! Loads a `splitlane.workspace.toml`, resolves each pane's `agent` to its CLI
...
pub fn up(client: &impl IpcTransport, file: &str, dry_run: bool) -> Result<i32, CliError> {
    ...
    // which is also exactly what `--dry-run` reports.
    ...
    // What only this path knows: which directory a worktree moved a pane to,
    // which port stride its env was given, and the ownership record the
    // server needs for teardown. Everything else about the request - the
    // default-cwd rule, the agent-to-command resolution, the pane profile -
    // belongs to the shared builder, and is no longer decided twice.
    let config = splitlane_config::loader::load_config();
    let overrides: Vec<preset::PaneOverride> = spec
        .panes
        .iter()
        .enumerate()
        .map(|(idx, pane)| {
            ...
            preset::PaneOverride {
                cwd: plan.as_ref().map(|p| p.path.to_string_lossy().into_owned()),
                env: substitute_env(pane.env.as_ref(), offsets[idx]),
                managed_worktree: plan.as_ref().map(|p| p.managed_json()),
            }
        })
        .collect();
    let params =
        preset::workspace_up_params(&spec, &config, &overrides).map_err(CliError::runtime)?;
...
fn ensure_orchestration_gate(client: &impl IpcTransport) -> Result<(), CliError> {
    ...
    match (orchestration, scripting) {
        ...
        (Some(false), _) => Err(CliError::runtime(
            "workspace up requires pane orchestration: relaunch Splitlane with \
             SPLITLANE_IPC_ORCHESTRATION=1",
        )),
    }
}

// ---------------------------------------------------------------------------
// Worktree planning + execution
// ---------------------------------------------------------------------------
...
impl WorktreePlan {
    ...
    /// record for close-time teardown). Shared by `up` and `flow`.
...
pub(super) fn plan_worktree(idx: usize, pane: &PaneSpec) -> Result<Option<WorktreePlan>, CliError> {
    ...
    for entry in &entries {
        if entry.branch.as_deref() == Some(branch) {
            if worktree::is_splitlane_worktree_dir(&repo_root, branch, &entry.path) {
    ...
    Ok(Some(WorktreePlan {
        ...
        setup_timeout: Duration::from_secs(
            pane.setup_timeout_secs
                .unwrap_or(worktree::DEFAULT_SETUP_TIMEOUT.as_secs()),
        ),
        teardown: pane
            .worktree_teardown
            .clone()
...
fn validate_worktree_target(
    ...
    if !worktree::is_splitlane_worktree_dir(repo_root, branch, path) {
        return Err(CliError::runtime(format!(
            "pane {idx}: planned worktree path {} is outside Splitlane's worktree dirs",
            path.display()
        )));
    }
    Ok(())
...
/// `setup` failure warns and continues: the human can fix it in
...
pub(super) fn execute_worktree_plan(plan: &WorktreePlan) -> Result<(), CliError> {
    ...
    if let Some(setup) = plan.setup.as_deref()
        && let Err(e) = worktree::run_setup(&plan.path, setup, plan.setup_timeout)
    {
        eprintln!(
    ...
    Some(
        env.iter()
            .map(|(k, v)| {
                (
                    k.clone(),
                    v.replace(preset::PORT_OFFSET_TOKEN, &offset.to_string()),
                )
            })
            .collect(),
    )
...
/// spawns); a raw `command` passes through; neither leaves a bare shell.
...
mod tests {
    ...
    fn allocate_port_offsets_skips_busy_strides() {
        ...
        // to 3020. Non-referencing panes get None.
    ...
    fn substitute_env_replaces_only_the_supported_token() {
        ...
        assert_eq!(out["PLAIN"], "untouched", "passthrough exact");
    ...
    fn plan_worktree_outside_a_repo_is_an_atomic_error() {
        // A cwd outside any git repository fails the plan phase
    ...
    #[test]
    fn a_planned_worktree_satisfies_the_builder_on_both_actions() {
        // The seam the builder's new invariant stands on: `plan_worktree`
        // yields a plan whenever the field is set, so `managed_worktree` is
        // present on the CLI path - for a worktree it creates *and* for one it
        // reuses. Asserted against a real repository rather than a hand-made
        // override, because the invariant refuses everything this does not
        // produce, `--dry-run` included (dry-run returns after the builder,
        // over these very params).
        let tmp = tempfile::tempdir().expect("tempdir");
        let repo_root = tmp.path().join("repo");
        std::fs::create_dir(&repo_root).expect("repo dir");
        if !test_git(&repo_root, &["init"]) {
        ...
        std::fs::write(repo_root.join("README"), "x").expect("file");
        if !test_git(&repo_root, &["add", "."])
            || !test_git(
                &repo_root,
                &[
                    "-c",
        ...
        let mut p = pane(None, Some("pnpm dev"));
        p.cwd = Some(repo_root.to_string_lossy().into_owned());
        p.worktree = Some("feat/x".to_string());

        // 1. Nothing created yet: the plan creates, and the builder accepts.
        let plan = plan_worktree(0, &p).expect("plan").expect("some");
        assert_eq!(plan.managed_json()["action"], json!("create"));
        let preset = preset::Preset {
            panes: vec![p.clone()],
            ..Default::default()
        };
        let over = preset::PaneOverride {
            cwd: Some(plan.path.to_string_lossy().into_owned()),
            env: None,
            managed_worktree: Some(plan.managed_json()),
        };
        preset::workspace_up_params(
            &preset,
            &SplitlaneConfig::default(),
            std::slice::from_ref(&over),
        )
        .expect("the builder accepts a created worktree");

        // 2. The same branch, now checked out where this app puts it: the
        // plan reuses, and the builder accepts that too. This is the case the
        // invariant would break if `plan_worktree` answered `None` for it.
        if !test_git(
            &repo_root,
            &[
                "worktree",
        ...
        let reused = plan_worktree(0, &p).expect("plan").expect("some");
        assert_eq!(reused.managed_json()["action"], json!("reuse"));
        let over = preset::PaneOverride {
            cwd: Some(reused.path.to_string_lossy().into_owned()),
            env: None,
            managed_worktree: Some(reused.managed_json()),
        };
        preset::workspace_up_params(
            &preset,
            &SplitlaneConfig::default(),
        ...
        .expect("the builder accepts a reused worktree");
    }
    ...
    fn resolve_command_rejects_unknown_agent() {
        // An unrecognized agent fails with a pane-indexed,
