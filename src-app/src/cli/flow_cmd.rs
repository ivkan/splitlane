//! `splitlane flow run <file>` - the local-first agent-orchestration engine
//! (executor, gated feeds, fan-out/fan-in, captures, reporting).
//!
...
//! `surface.read` poll - the same machinery as `splitlane wait`. Ctrl-C stops
...
/// Scheduler tick. 500 ms = the documented `splitlane wait` poll cycle (NFR:
...
/// Barrier poll window - mirrors `splitlane wait` (bounded under the client's
...
/// prefill constants, using the public
...
/// truncated to fit, with an explicit marker.
const MAX_SEND_LEN: usize = 64 * 1024;

/// The settle/bailout decision. Fire when
...
/// `splitlane flow run <file> [--dry-run] [--json]`.
pub fn run(
    ...
    // A submitting flow is refused up-front - run AND dry-run - when
...
fn check_orchestration_gate(client: &impl IpcTransport, dry_run: bool) -> Result<(), CliError> {
    match client.call("system.capabilities", json!({})) {
        Ok(caps) => {
            ...
            match (orchestration, scripting) {
                ...
                (Some(false), _) => Err(CliError::runtime(
                    "this flow creates panes with commands/prompts: relaunch Splitlane with \
        ...
        Err(e) if dry_run => {
            eprintln!(
                "splitlane: instance unreachable ({e}); cannot verify the orchestration gate \
                 this flow requires"
            );
            Ok(())
        }
    ...
    match client.call("system.capabilities", json!({})) {
        Ok(caps) => match caps.get("scripting").and_then(Value::as_bool) {
            ...
            Some(false) => Err(CliError::runtime(
                "this flow submits prompts: relaunch Splitlane with SPLITLANE_IPC_SCRIPTING=1 \
                 (or drop the `submit = true` flags)",
            )),
        },
        Err(e) if dry_run => {
            eprintln!(
                "splitlane: instance unreachable ({e}); cannot verify the scripting gate \
                 this flow requires"
            );
            Ok(())
        }
...
impl<T: IpcTransport> Engine<'_, T> {
    fn execute(mut self) -> Result<i32, CliError> {
        // Ctrl-C stops the orchestration, never the panes: the
    ...
    fn fire_feed(&mut self, i: usize) -> Result<(), String> {
        ...
        match self.client.call(
            ...
            Err(e) if super::send_cmd::is_send_text_disabled_error(&e) => Err(format!(
                "IPC gate is off on the instance; use SPLITLANE_IPC_ORCHESTRATION=1 for \
                 pane spawning and SPLITLANE_IPC_SCRIPTING=1 for prompt submission ({e})"
            )),
    ...
    fn read_window(&self, sid: u64, lines: u64) -> Result<Read, String> {
        match self.client.call(
            "surface.read",
            // Barriers parse raw output and settling reads
    ...
    /// Final report + exit code: 0 all READY, 4 when a barrier
    ...
    fn report(self, aborted: Option<String>) -> Result<i32, CliError> {
        ...
        if let Some(reason) = &aborted {
            eprintln!("splitlane: flow aborted: {reason} (partial report above)");
...
/// unknown means the capturing step failed or was skipped. The
...
mod tests {
    ...
    fn settle_fire_bails_at_max_and_fires_when_stable_past_floor() {
        // Output that never settles (stable stays 0) must
    ...
    /// mutation - including under --dry-run.
    #[test]
    fn submitting_flow_refused_when_gate_off() {
        ...
        assert!(
            err.message.contains("SPLITLANE_IPC_SCRIPTING"),
            "got: {}",
            err.message
        );
    ...
    fn flow_run_refused_when_orchestration_gate_off() {
        ...
        assert!(
            err.message.contains("SPLITLANE_IPC_ORCHESTRATION"),
            "got: {}",
            err.message
        );
        let calls = fake.calls.borrow();
    ...
    /// Dry-run prints the plan and never mutates the instance.
    #[test]
    ...
    /// capture feeds the second step, feed is sent, flow exits 0. The fake's
    /// read script: first read misses, second matches - exercising at least one
    /// real poll wait.
    #[test]
