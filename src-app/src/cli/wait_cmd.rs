//! `splitlane wait` - block until a regex appears in a pane.
//!
...
/// Default quiescence window for `wait --idle` when `--for` is
...
enum PaneState {
    ...
    /// `wait` can surface them. May be empty if the match spans
...
/// `splitlane wait --match <sel> --pattern <regex> [--timeout N] [--any|--all]`.
pub fn wait(
    ...
    loop {
        ...
        // Every watched pane closed: no outcome is reachable (defined
        ...
        if Instant::now() >= deadline {
            eprintln!(
                "splitlane: timeout after {}s waiting for /{}/",
                timeout.as_secs(),
                pattern
            );
...
fn read_snapshot(client: &impl IpcTransport, id: u64) -> Result<Option<ReadSnapshot>, CliError> {
    match client.call(
        "surface.read",
        // `wait` regex-matches raw scrollback; the untrusted
        ...
        // A down instance is fatal - propagate the "is Splitlane running?" error.
...
fn read_matches_since(
    ...
    // individual matching lines for the caller.
...
// ---------------------------------------------------------------------------
// `wait --idle` - block on output quiescence via the
...
enum IdleOutcome {
    ...
    Idle,
    /// The subscription died before idle: exit 1, never a hang.
    Dead,
    /// The overall `--timeout` elapsed without idle: exit 4.
    TimedOut,
}

/// Pure quiescence rule, factored so the exit-code matrix and the
/// dead-detection are unit-tested without a socket. `since_change` is the
...
/// `splitlane wait --idle <sel> [--for <ms>] [--timeout <s>] [--pattern <re>]`.
///
...
/// to fire. Exit codes: 0 idle/match, 1 dead stream, 3 no instance /
...
pub fn wait_idle(
    ...
    // subscription on its next write (RAII), so nothing leaks.
    ...
    let stream_result =
        splitlane_ipc_client::subscribe_stream_timed(&socket, params, slice, |ev| {
    ...
    match stream_result {
        Ok(()) => match outcome {
            ...
            IdleOutcome::TimedOut => {
                eprintln!(
                    "splitlane: timeout after {}s waiting for surface {id} to go idle",
                    timeout.as_secs()
                );
                Ok(EXIT_TIMEOUT)
            }
            ...
            IdleOutcome::Dead => Err(CliError::runtime(
                "the Splitlane event stream closed before the pane went idle (did Splitlane exit?)",
            )),
        ...
        // already actionable (start Splitlane / set SPLITLANE_SOCKET_PATH), so
...
mod tests {
            ...
            match method {
                ...
                "surface.read" => {
                    ...
                    match next {
                        ...
                        None => Err("splitlane error -32602: surface_id 1 not found".to_string()),
                    }
                }
    ...
    fn wait_succeeds_and_surfaces_matched_line() {
        ...
        // is surfaced: read_matches collects it from the window.
    ...
    fn idle_decision_exit_code_matrix() {
        // A spinner never goes quiet; past the overall
        ...
        // A vanished server -> Dead (exit 1) regardless of timing.
        assert_eq!(
