//! Target selector for the `splitlane` CLI.
//!
...
//! are the portable selectors when argv is unavailable.
...
/// All `surface_id`s matching the selector (for `wait --any/--all`).
...
/// candidates (id + name) rather than silently picking one.
...
mod tests {
    ...
    fn cmdline_matches_basename_only_cmd() {
        ...
        // `cmdline:` selector must still match it on every platform.
