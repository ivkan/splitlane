...
/// sequences are Splitlane bindings whose bytes are the behavior, so a backend
...
///
/// A user can always override per-platform via `splitlane.json#option_as_meta`;
...
mod tests {
    ...
    #[test]
    fn page_keys_match_alt_screen_constants() {
        // Invariant: the alt-screen page-forward bytes hardcoded in
    ...
    fn alt_arrow_reports_modifier_regardless_of_option_as_meta() {
        // `option_as_meta` gates only the ESC-prefix paths, never the
    ...
    fn alt_accented_letter_sends_esc_prefix() {
        // A single multi-byte key (AZERTY "é" = 2 UTF-8 bytes) must take
