...
//! the scanned cwd before Splitlane renders it.
...
fn run_list_command(config: &CommandSessionConfig, cwd: &str) -> Option<Vec<u8>> {
    ...
    let output =
        match splitlane_process::run_with_timeout(cmd, COMMAND_DEADLINE, COMMAND_STDOUT_CAP) {
            Ok(out) => out,
...
fn parse_session_line(
    ...
    Some(SessionMeta {
        agent,
        session_id,
        // These agents are scraped from a `list` subcommand, so there is no
        // file to stat: the listed timestamp is all the recency we get.
        last_activity_secs: crate::agent_sessions::last_activity_secs_from_iso(&timestamp),
        timestamp,
        cwd: cwd.to_string(),
