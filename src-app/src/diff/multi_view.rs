//! Multi-project scope - a **repo tab
...
impl MultiRepoDiffView {
    /// Build from the repo groups. The first repo is selected (and
    ...
    /// Warm-resume passthrough: suspend all cached child `DiffView`s so
    ...
    /// Warm-resume passthrough: re-arm + revalidate the mounted child.
