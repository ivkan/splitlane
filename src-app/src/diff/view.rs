...
//! stale results when a refresh is superseded (last write wins).
//!
//! N columns render side by side in one tab with a shared base-branch
//! selector, per-column hide/show, and live refresh on working-tree / HEAD /
//! index / base-ref changes via an entity-owned `notify` watcher. Rendering is
//! virtualized by the custom `DiffElement` in both unified and split modes.
...
/// re-diff per window, short enough to feel live.
...
enum ColumnState {
    ...
    Loaded {
        ...
        file_count: usize,
        /// Per-file summary for the git panel (shared `Rc` so the
        ...
        /// The raw per-file diffs retained so "copy hunk/file" and the agent
        /// review payload serialize an exact unified diff at action time (no stable
...
enum Built {
    ...
    Loaded {
        ...
        file_count: usize,
        files: Vec<FileEntry>,
        /// Raw per-file diffs retained for copy/review, moved out
        ...
        theme_generation: u64,
        /// Captured in the same off-thread pass as the diff, so a later
        ...
        /// Agent sessions matched to this column, computed in the
...
struct Column {
    ...
    /// Hidden columns are skipped in render and in refresh (no wasted
    ...
    /// Cumulative top offsets of each hunk's first changed row, cached
    ...
    /// Warm resume: the git fingerprint (HEAD + base + status hash) this
    ...
    /// Per-column last-write-wins guard. Bumped each time THIS column
    ...
    /// under the diff body. Empty until the user runs
    ...
    review_height: f32,
    /// Local agent sessions matched to this worktree (cwd + branch),
    /// most-relevant first. Computed off-thread in the same task as the diff
    /// (folded into the column-load) and re-fetched only on re-diff, so
    /// per-frame render reads it O(1). Empty = no matching session (the
    /// attribution slot collapses to zero width).
    attribution: Vec<SessionMeta>,
...
impl Column {
    ...
    /// Cached hunk-start offsets for `mode`. Lockstep with the display
    ...
    /// Total content height for `mode`, which is the last row offset - the
    /// offsets vector holds one entry per row plus a terminating one.
    fn content_height(&self, mode: ViewMode) -> f32 {
        self.offsets(mode).last().copied().unwrap_or(0.0)
    }

    fn offsets(&self, mode: ViewMode) -> &Rc<Vec<f32>> {
        match mode {
            ViewMode::Unified => &self.disp_unified_offsets,
            ViewMode::Split => &self.disp_split_offsets,
        }
    }

    /// The content-space `y` of the first file header strictly below `y`, or
    /// `None` when `y` is inside the last file.
    ///
    /// The bar over the current hunk has to stop where the file does: hunk
    /// starts alone would run it through the next file's header card and into
    /// that file's first context lines, claiming a hunk boundary where there is
    /// a file boundary.
    fn next_file_header_top(&self, mode: ViewMode, y: f32) -> Option<f32> {
        let offsets = self.offsets(mode);
        let spans = match mode {
            ViewMode::Unified => &self.disp_unified_spans,
            ViewMode::Split => &self.disp_split_spans,
        };
        spans
            .iter()
            .filter_map(|span| offsets.get(span.header_row).copied())
            .find(|top| *top > y)
    }
}
...
pub struct DiffView {
    repo_root: PathBuf,
    /// Resolved base ref every column diffs against (the base selector).
    base_ref: String,
    ...
    /// Watcher epoch. Bumped by [`Self::suspend`]; [`Self::start_watchers`]
    ...
    /// Entity-owned filesystem watchers. Dropped on tab close, which
    /// unregisters the OS handles and ends the debounce loop.
    _watchers: Vec<RecommendedWatcher>,
    /// Warm resume: `true` while the diff surface is hidden (CLI/Agents
    ...
    suspended: bool,
    /// `true` once `bootstrap` has resolved the base + branches. Guards
    ...
    /// Transient "copied" confirmation pill, auto-cleared by a spawned
    /// timer. Self-hosted so the diff view needs no SplitlaneApp toast handle.
    ...
    /// The 210px file list, pushed the same way and for the same reason: it is
    /// built from app-level filter state (branch, revision, the search box),
    /// and a `DiffView` reads no app state.
    ///
    /// It used to be a sibling of this entity inside the full-area diff
    /// render. With the diff living in a pane there is no such wrapper - the
    /// pane draws the tab and this entity fills it - so the column moved
    /// inside, where the design has always put it: "its own file-list column",
    /// a column of the surface rather than a rail of the window.
    pub files_slot: Option<AnyElement>,
}

/// Events a [`DiffView`] raises to its host (`SplitlaneApp`). Today: the user
/// asked to drop a branch column from the Worktree scope via its header `×`.
pub enum DiffViewEvent {
    CloseColumn { path: PathBuf },
}

/// An open right-click menu on the diff body, anchored at the
...
impl DiffView {
        ...
        cx.spawn(async move |this, cx| {
            ...
            let _ = cx.update(|cx| {
                this.update(cx, |view: &mut Self, cx| {
                    ...
                    // If the surface was hidden (parked to CLI) before
    ...
    /// Tab-strip title, e.g. `Diff: splitlane`.
    ...
    /// Hide a column (drop its data, skip future refreshes).
    ...
    /// Re-show every hidden column and reload them.
...
impl DiffView {
    ...
    /// Replaces the old wrapping chip-row: it floats above the diff
