...
//! theme's dedicated [`SyntaxPalette`] - a ≈30-slot semantic palette in
//! Splitlane's Catppuccin brand, NOT the 8-hue ANSI terminal set. Resolution is
//! longest-prefix-wins (Zed's `SyntaxTheme` model): a dotted capture like
//! `function.method.call` falls back to the `function` slot, with exact-name arms (`variable.builtin`,
...
///
/// `Copy`, because the file surface snapshots one on the main thread and hands
/// it to a background read - the same move `diff/view/loader.rs` makes, and for
/// the same reason: `active_theme()` polls an mtime and must not be called off
/// the render thread.
#[derive(Clone, Copy)]
...
mod tests {
    ...
    fn dotted_capture_falls_back_to_longest_prefix() {
        // `function.method.call` → the `function` slot.
        let syn = DiffSyntax::from_theme(&one_dark());
    ...
    fn operators_punctuation_variables_now_colored() {
        // Previously these returned `None`; now each is a slot.
        let syn = DiffSyntax::from_theme(&one_dark());
    ...
    fn exact_builtin_arms_win_over_their_prefix() {
        // `variable.builtin` / `constant.builtin` / `comment.doc`
    ...
    fn legacy_markdown_captures_map_to_palette_slots() {
        // tree-sitter-md's legacy capture names each resolve.
        let syn = DiffSyntax::from_theme(&one_dark());
        for name in [
            "text.title",
            "text.literal",
    ...
    fn unknown_capture_inherits_default_without_panic() {
        // Unhappy path: a capture matched by no arm returns
