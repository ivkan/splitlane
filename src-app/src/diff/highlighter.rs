...
fn grammar_for_ext(ext: &str) -> Option<&'static Grammar> {
    ...
    match ext {
        ...
        // C, C++, Java, Ruby. NOTE: tree-sitter-c and
...
/// The Markdown *inline* grammar - a second pass over the same text
...
pub fn highlight_lines(
    ...
    // Markdown gets a second inline pass merged into the same runs.
...
mod tests {
    ...
    fn unknown_extension_returns_empty_runs_without_panic() {
        // Unknown ext → one empty run-list per line (monochrome).
        let syn = DiffSyntax::from_theme(&one_dark());
    ...
    fn new_p1_grammars_produce_colored_runs() {
        // Go / YAML / CSS / HTML each color their core families.
        let syn = DiffSyntax::from_theme(&one_dark());
        let cases: &[(&str, &str)] = &[
            (
                "go",
    ...
    fn new_p2_grammars_produce_colored_runs() {
        // C / C++ / Java / Ruby each color keyword/type/fn/string.
        let syn = DiffSyntax::from_theme(&one_dark());
        let cases: &[(&str, &str)] = &[
            (
                "c",
    ...
    fn markdown_block_and_inline_passes_color_richly() {
        // Heading + fenced code + inline link + list marker
        ...
        // (no double-coloring / artifact from overlap).
        for line in &lines {
    ...
    fn malformed_and_empty_inputs_never_panic() {
        // Empty + garbage input of every supported new
    ...
    fn malformed_query_compiles_to_none_not_panic() {
        // Simulated query-compile failure: the interning step
