...
//! builds. This element is modeled on Splitlane's `TerminalElement` (which is
...
const STAT_GAP: f32 = 16.0; // min gap between the path region and the right-aligned diffstat
...
// The design's "two 34px right-aligned line-number gutters". A floor, not a
// fixed width: a file with five-digit line numbers still has to print them.
const GUTTER_W: f32 = 34.0; // single line-number column FLOOR width (widened per max digits)
...
/// The design's `inset 2px 0 0` on the hunk the reader is on.
const CURRENT_HUNK_BAR_W: f32 = 2.0;
...
pub struct DiffElement {
    ...
    gutter_w: Pixels,
    /// Content-space `(top, bottom)` of the hunk the reader is on, or `None`
    /// when the column has no hunks. Painted as a 2px accent stripe down the
    /// body's left edge - the design's "the current hunk carries a 2px accent
    /// bar". A range rather than a hunk index because the element indexes rows,
    /// not hunks, and the offsets it already culls against are in exactly these
    /// units.
    current_hunk: Option<(f32, f32)>,
}
...
impl DiffElement {
    ...
    /// Mark the hunk the reader is on, in content-space pixels.
    pub fn with_current_hunk(mut self, range: Option<(f32, f32)>) -> Self {
        self.current_hunk = range;
        self
    }
    ...
    /// Shape a single line in a specific font weight (the semibold basename
    /// segment of a file header). One run, one color.
    fn shape_weighted(
        &self,
        window: &mut Window,
        text: SharedString,
    ...
    fn layout_unified(
        ...
        match row.kind {
            RowKind::FileHeader => match &row.header {
                // Structured header (icon + path + diffstat).
            ...
            RowKind::Context | RowKind::Added | RowKind::Removed => {
                // (row-bg wash, opaque hunk-bar color, word-diff bg). Context
                // gets a faint document wash instead of the bare window
                // background.
                ...
                // Gutter rail - a slightly stronger tint over the
    ...
    fn layout_split(
        ...
        match row {
            // Same structured header as the unified view.
    ...
    fn layout_half(
        ...
        let (bg, gutter_bg, bar_color, word_bg) = match cell.kind {
            ...
            // Faint document wash on unchanged code.
        ...
        // Gutter rail over this half's line-number column.
        quads.push(Quad {
...
impl Element for DiffElement {
    ...
    fn prepaint(
        ...
        // The design's "the current hunk carries a 2px accent bar". Pushed
        // after the row loop so it lands over the per-line add/remove bars
        // rather than under them: it says where the reader is, which outranks
        // what each line is. Clipped to the viewport, so a hunk taller than the
        // pane still draws one continuous stripe.
        if let Some((hunk_top, hunk_bottom)) = self.current_hunk {
            let top = hunk_top.max(vtop);
            let bottom = hunk_bottom.min(vbot);
            if bottom > top {
                quads.push(Quad {
                    bounds: Bounds::new(
                        point(bounds.origin.x, bounds.origin.y + px(top)),
                        size(px(CURRENT_HUNK_BAR_W), px(bottom - top)),
                    ),
                    color: self.palette.current_hunk_bar,
                });
            }
        }
