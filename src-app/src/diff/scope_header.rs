//! The app-level scope-selector header
...
//!
//! This renders `SplitlaneApp` (app-level) state, so the `impl` lives on
//! `SplitlaneApp` even though the file sits in the `diff` module next to the
//! other scope types.
...
impl SplitlaneApp {
    pub(crate) fn render_scope_header(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        let popover: Option<AnyElement> = if open {
            ...
            // hoists it above everything (the pattern every other Splitlane menu
        ...
        let project_trigger = div()
            ...
            .text_size(crate::ui_primitives::BODY)
            // Secondary context label - muted, demoted under the
        ...
        let (branches_trigger, branches_popover): (Option<AnyElement>, Option<AnyElement>) =
            match repo_root.clone().filter(|_| show_branches) {
                Some(root) => {
                    // A state badge ("4/6 branches") readable
                    ...
                    let trigger = div()
                        ...
                        .bg(trigger_bg)
                        .text_size(crate::ui_primitives::BODY)
                        // Secondary context label - muted, demoted.
                        .text_color(ui.muted)
                        .animated_hover_bg(trigger_bg, ui.subtle)
                        .cursor_pointer()
                        .on_click(cx.listener(move |this, _: &ClickEvent, _w, cx| {
