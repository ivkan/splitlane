...
impl DiffView {
    ...
    /// `generation` guard discards results superseded by a newer load (warm
    /// resume keeps the task count bounded to the visible columns).
    ...
    /// [`Self::start_loading`] passes every visible column; warm-resume
    ...
    /// superseded by a newer load (last write wins).
        ...
        for &i in indices {
            ...
            cx.spawn(async move |this, cx| {
                ...
                let built = smol::unblock(move || {
                    // Snapshot the fingerprint BEFORE reading the tree, so a
                    ...
                    // Lightweight per-file summary for the git panel,
                    ...
                    // Match local agent sessions to this worktree
                    ...
                    Built::Loaded {
                        ...
                        files,
                        // Move the raw FileDiffs out for copy/review;
