//! Agent attribution and cost rendering for the Review view: the per-column
//! attribution badge and the estimated-cost figures. The matching + token/usage
//! parsing happens off-thread in the column-load task (see [`super::loader`]);
//! this module is pure render over the `Column::attribution` already cached on the column, so it stays O(1) per
...
/// [`crate::ui_primitives::SplitlaneTooltip`] (single-line) because the badge and
/// the cost figures want a per-session breakdown.
...
impl DiffView {
    ...
    /// "Total ~$X.XX across N worktrees" for the toolbar - summed
    ...
    /// The attribution badge for a column header - agent
    ...
    pub(super) fn render_attribution_badge(
        ...
        // Aggregate token tiers across the matched sessions (the cost breakdown).
