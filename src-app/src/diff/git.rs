//! Git plumbing for the multi-worktree diff viewer.
//!
//! All heavy git operations run via `std::process::Command` subprocesses
//! (matching Zed's actual diff/worktree path and Splitlane's existing
...
//! missing ref. Callers run these off the GPUI main thread.
//!
...
///
/// On the live Worktree-scope discovery path: the porcelain parser
...
/// Per-file diff payload consumed by the renderer. Carries the raw
...
/// [`list_repo_worktrees`] calls this for the Worktree-scope "include
...
/// must be re-diffed (warm resume). It captures the worktree `HEAD`, the
...
/// Candidate base refs for the selector: local branches *and*
...
/// Shared core of [`compute_worktree_diff`]: diff the working tree against the
/// already-resolved commit-ish `base`. `worktree_dir` must already be the
/// worktree toplevel (the caller resolves it first).
