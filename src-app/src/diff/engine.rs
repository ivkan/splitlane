...
//! `Range<Anchor>` over a `SumTree`); Splitlane has no editor, so hunks carry
...
//!
//! Word-level intra-line diff (Zed's `word_diff_ranges`) lives in `worddiff.rs`.
...
/// `new_row_range`. The alignment layout math (`align.rs`) consumes these ranges to align
...
/// callers run it off the GPUI main thread. Mirrors Zed's use of
