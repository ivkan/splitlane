//! Unified diff display-row model + per-row rendering.
//!
...
//! rows) is built further down from `align.rs`; this top part is the unified view.
...
/// populated for small modified hunks; other lines highlight at the
/// line level only.
...
/// in `Column::recompute_display` so the toolbar's hunk counter and
...
/// The file-header row, split into typed segments at build time
...
pub struct RowPalette {
    ...
    /// Stronger backgrounds for word-diff-highlighted spans.
    pub add_word_bg: Hsla,
    pub del_word_bg: Hsla,
    /// Faint document wash on context (unchanged) code so the
    /// diff body reads as a surface, not the bare window background.
    pub context_bg: Hsla,
    /// Persistent line-number-rail tint, painted over every
    /// content row's gutter region so the gutter reads as a structural column.
    pub gutter_bg: Hsla,
    pub add_gutter_bg: Hsla,
    pub del_gutter_bg: Hsla,
    /// The 2px stripe down the left edge of the hunk the reader is on
    /// (the design's "the current hunk carries a 2px accent bar").
    pub current_hunk_bar: Hsla,
}
...
pub fn palette(ui: crate::theme::UiColors) -> RowPalette {
    ...
    RowPalette {
        text: ui.text,
        muted: ui.muted,
        // File card at the `surface` tier (one step off the
        // `base` body), so each file reads as an elevated card over the body.
        header_bg: ui.surface,
        // The sticky bar is the file header pinned to the
        ...
        // Intra-line word emphasis. Light keeps the theme's 0.40
...
/// share one row (and therefore one scroll offset - synchronized scroll is free).
#[derive(Clone)]
pub enum SplitRow {
    /// File-section header. Carries the same typed
...
/// Build the side-by-side row list for a column's files. Aligns each
    ...
    for (file_idx, file) in files.iter().enumerate() {
        ...
        while i < total {
            if is_ctx(&aligned[i]) {
                ...
                // Lead/hidden/trail partition `run = j - i`, so every
...
mod tests {
    ...
    fn split_header_path_separates_dir_from_basename() {
        // The directory prefix keeps its trailing slash and the
    ...
    fn file_header_row_carries_typed_segments() {
        // build_display_rows must populate the structured header
    ...
    fn unified_hunk_tops_marks_each_hunk_start_at_its_row_offset() {
        // The cached hunk tops MUST equal the precomputed row offset of
    ...
    fn split_hunk_tops_marks_each_hunk_start_at_its_row_offset() {
        // The split analog of the unified drift guard.
