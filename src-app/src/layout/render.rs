            ...
            LayoutTree::Container {
                ...
                for (i, child) in children.iter().enumerate() {
                    if i > 0 {
                        ...
                        // The gap between two slots is the desk showing
                        // through, and the mockup paints it `transparent`.
                        // It used to be a solid bar of `border` across the
                        // whole gap - a seam drawn between two panes that had
                        // no edges of their own. Now that a slot is a card on
                        // the gutter, that bar was the loudest line in the
                        // work area and said nothing the cards do not.
                        //
                        // Under the pointer it lights: the design gives the
                        // handle `#3f6a5c` on hover, and a grab zone that says
                        // nothing until you find it is a grab zone people
                        // discover by accident. Half the gap, so it reads as a
                        // rule to grab and not as the seam we just removed.
                        let handle_group: gpui::SharedString =
                            format!("pane-divider-{divider_idx}").into();
                        let handle_thickness = px(DIVIDER_PX / 2.0);
                        let hover_color = crate::theme::ui_colors().accent_border;
                        let divider = match dir {
                            SplitDirection::Horizontal => div()
                                .group(handle_group.clone())
                                .h(px(DIVIDER_HIT_PX))
                                .w_full()
...
mod tests {
                ...
                .map(|_| {
                    let columns = (0..4)
                        .map(|_| {
                            ...
                            LayoutTree::Leaf(cx.new(|cx| Pane::new(terminal, cx)))
                        })
                        .collect();
    ...
    fn available_main_axis_excludes_fixed_dividers() {
        let expected = 500.0 - DIVIDER_PX * 2.0;
        assert!((available_main_axis_px(500.0, 3) - expected).abs() < f32::EPSILON);
    }
