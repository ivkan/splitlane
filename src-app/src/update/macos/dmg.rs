//! macOS DMG self-update pipeline.
//!
//! Flow:
//!   1. Download the `.dmg` to the native user cache dir
//!      (`~/Library/Caches/splitlane` on macOS) as `update-<pid>.dmg`
//!      via ureq with the 30-second per-call timeout.
//!   2. Verify the asset's detached **minisign** signature (`.minisig`
//!      sibling) against a key baked into this binary **before
...
//!      the extracted `.app` before promotion.
...
//!   5. `cp -R <mount>/Splitlane.app /Applications/Splitlane.app.new`.
//!   6. Atomic swap: rename `/Applications/Splitlane.app` →
//!      `/Applications/Splitlane.app.old`, then `.new` → `Splitlane.app`,
//!      then `rm -rf .old`. If the second rename fails, the first is
//!      rolled back so `/Applications/Splitlane.app` never disappears.
...
//!      but NOT a bare Mach-O - so it must receive `Splitlane.app`, not the
//!      inner `Contents/MacOS/splitlane`. Mirrors Zed returning `Ok(None)`,
...
//! manually" message. `ENOSPC` during copy routes to
...
/// hardcoded `/Applications/Splitlane.app` - so a user who installed
...
pub fn install(asset_url: &str, bundle_path: &Path) -> Result<PathBuf> {
    ...
    if !is_expected_bundle_location(bundle_path, &home) {
        ...
        let message = if is_translocated_path(bundle_path) {
            format!(
                "Splitlane is running translocated from a quarantine sandbox ({}), so in-app updates are disabled. Move Splitlane.app into /Applications (drag it there in Finder) and reopen it.",
                bundle_path.display()
            )
        } else {
            format!(
                "Splitlane is installed at an unexpected location ({}); reinstall from the DMG into /Applications or ~/Applications to enable in-app updates.",
                bundle_path.display()
            )
        };
...
/// replace in place. Pure path logic, unit-tested on Linux CI.
...
/// Gatekeeper / code-signature verification of `bundle`, fail-closed.
/// `codesign --verify --strict --deep` proves the signature is intact and
/// covers every nested item; the Team-ID designated-requirement check
...
fn verify_macos_bundle(bundle: &Path) -> Result<()> {
    ...
    // Pin the signing identity to our Team ID via a designated
...
fn install_in(
    ...
    let mount_point = PathBuf::from(format!(
        "/private/tmp/splitlane-update-{}.mount",
        std::process::id()
    ));
    ...
    // OS-native gatekeeper check on the bundle inside the (read-only,
...
/// Download the DMG, verify its detached **minisign** signature,
...
fn copy_and_swap(mounted_volume: &Path, install_dir: &Path) -> Result<()> {
    ...
    //
    // Discriminate the first rename's error. `NotFound` means a fresh
    ...
    if let Err(e) = std::fs::rename(&new_dir, install_dir) {
        ...
        // left without `/Applications/Splitlane.app`. Verify the
            ...
            && let Err(rb) = std::fs::rename(&old_dir, install_dir)
        {
            ...
            .context(format!(
                "promote {} → {} failed ({e}); rollback from {} also failed ({rb}) - no live install remains, reinstall Splitlane manually",
                new_dir.display(),
                install_dir.display(),
                old_dir.display()
            )));
...
/// Copy `Splitlane.app` into the `.new` staging path.
///
...
/// SINGLE invariant instead of two hardcoded `"Splitlane.app"` literals that
...
/// denied routes to `InstallDeclined`; everything else
...
fn classify_filesystem_error(raw: &str, context: &str) -> anyhow::Error {
        ...
        || lower.contains("read-only file system")
    {
        return anyhow::Error::new(UpdateError::InstallDeclined {
            message: "Unable to replace Splitlane.app in its install location - reinstall manually from the DMG."
                .to_string(),
        })
...
fn run_native_command(
    ...
    label: &str,
    deadline: Duration,
) -> Result<splitlane_process::BoundedOutput> {
    splitlane_process::run_with_timeout(cmd, deadline, NATIVE_STDOUT_CAP).map_err(|err| match err {
        splitlane_process::ProcError::Timeout => {
            anyhow::Error::new(UpdateError::Timeout).context(format!("{label} timed out"))
        }
        splitlane_process::ProcError::Spawn(e) => {
            anyhow::Error::new(e).context(format!("spawn {label}"))
        }
        splitlane_process::ProcError::Wait(e) => {
...
mod tests {
    ...
    fn staging_dirs_derives_sibling_paths() {
        let (old, new) = staging_dirs(Path::new("/Applications/Splitlane.app")).unwrap();
        assert_eq!(old, PathBuf::from("/Applications/Splitlane.app.old"));
        assert_eq!(new, PathBuf::from("/Applications/Splitlane.app.new"));
    }
    ...
    fn bundle_file_name_derives_from_install_dir() {
        assert_eq!(
            bundle_file_name(Path::new("/Applications/Splitlane.app")).unwrap(),
            std::ffi::OsStr::new("Splitlane.app")
        );
    ...
    fn expected_bundle_location_accepts_applications_dirs() {
        ...
        assert!(is_expected_bundle_location(
            Path::new("/Applications/Splitlane.app"),
            home
        ));
        assert!(is_expected_bundle_location(
    ...
    fn expected_bundle_location_rejects_arbitrary_paths() {
        let home = Path::new("/Users/alice");
        // A drag-install to an unexpected location is refused.
        assert!(!is_expected_bundle_location(
            Path::new("/opt/third-party/Splitlane.app"),
            home
        ));
        assert!(!is_expected_bundle_location(
            Path::new("/Users/alice/Downloads/Splitlane.app"),
            home
        ));
        // Another user's Applications dir is not ours.
        assert!(!is_expected_bundle_location(
            Path::new("/Users/bob/Applications/Splitlane.app"),
            home
        ));
    }
    ...
    fn classify_sip_operation_not_permitted_as_install_declined() {
        ...
        let err = classify_filesystem_error(
            "rename /Applications/Splitlane.app: Operation not permitted (os error 1)",
            "swap step",
        );
        assert!(matches!(
            UpdateError::classify(&err),
    ...
    impl Hdiutil for StubHdiutil {
        fn attach(&self, _dmg: &Path, target: &Path) -> Result<PathBuf> {
            ...
            // Mirror the structure hdiutil would produce: `<mount>/Splitlane.app`.
            let dst = target.join("Splitlane.app");
    ...
    /// Build a minimal Splitlane.app skeleton under `root/Splitlane.app/`
    ...
    fn fake_bundle_at(root: &Path) -> PathBuf {
        let bundle = root.join("Splitlane.app");
        let macos = bundle.join("Contents").join("MacOS");
        std::fs::create_dir_all(&macos).unwrap();
        std::fs::write(macos.join("splitlane"), b"#!/bin/sh\necho splitlane").unwrap();
        bundle
    }
    ...
    fn copy_and_swap_performs_atomic_rename() {
        ...
        let install_dir = tmp.path().join("Applications").join("Splitlane.app");
        ...
        assert!(install_dir.join("Contents/MacOS/splitlane").exists());
        assert!(!install_dir.join("old-marker").exists());
        // `.old` was cleaned up.
        let old_dir = install_dir.parent().unwrap().join("Splitlane.app.old");
    ...
    fn copy_and_swap_aborts_when_source_has_no_bundle() {
        ...
        assert!(err.to_string().contains("Splitlane.app"), "got: {err}");
    }

    /// A fresh install (no pre-existing bundle at `install_dir`)
    ...
    fn copy_and_swap_fresh_install_no_existing_bundle() {
        ...
        let install_dir = install_parent.join("Splitlane.app");
        ...
        assert!(install_dir.join("Contents/MacOS/splitlane").exists());
        assert!(
            !install_parent.join("Splitlane.app.old").exists(),
    ...
    fn recover_restores_live_bundle_when_install_dir_missing() {
        ...
        std::fs::create_dir_all(&install_parent).unwrap();
        let install_dir = install_parent.join("Splitlane.app");
        let old_dir = install_parent.join("Splitlane.app.old");
        std::fs::create_dir_all(old_dir.join("Contents/MacOS")).unwrap();
        std::fs::write(old_dir.join("Contents/MacOS/splitlane"), b"prev").unwrap();
    ...
    fn copy_and_swap_cleans_stale_old_when_live_bundle_exists() {
        ...
        std::fs::create_dir_all(install_parent.join("Splitlane.app.old")).unwrap();

        copy_and_swap(&mount, &install_dir).unwrap();

        assert!(install_dir.join("Contents/MacOS/splitlane").exists());
        assert!(!install_parent.join("Splitlane.app.old").exists());
    }

    /// hdiutil attach failure must surface to the caller (no
    ...
    /// The StubHdiutil-backed install_in exercises the copy_and_swap
