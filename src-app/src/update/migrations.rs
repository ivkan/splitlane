...
//!
//! Install-method hygiene: when a user upgrades from the
//! tar.gz flavor (installed under `$HOME/.local/splitlane.app/`) to the signed
//! rpm/deb flavor (installed under `/usr/bin/splitlane`), the tar.gz
//! post-install script leaves stale icon files under
//! `~/.local/share/icons/hicolor/*/apps/splitlane.png`. Those user-local files
...
//! overrides and preserved). A marker file under `~/.cache/splitlane/` makes
...
/// Freedesktop hicolor icon sizes shipped by Splitlane's rpm/deb payloads
...
/// Marker filename written under `~/.cache/splitlane/` to short-circuit the
...
/// startup: a failing migration is better than a crashed startup.
pub fn run_startup_migrations(method: &InstallMethod) {
    ...
    let cache_dir = home.join(".cache").join("splitlane");

    if let Err(err) =
        migrate_user_hicolor_icons(&user_icon_dir, &system_icon_dir, &cache_dir, HICOLOR_SIZES)
    {
        log::warn!(
            "splitlane: hicolor icon migration failed ({err}); leaving user-local icons untouched"
        );
    }
}
...
///   `user_icon_dir/{N}x{N}/apps/splitlane.png` against
///   `system_icon_dir/{N}x{N}/apps/splitlane.png` by SHA-256. If they differ,
...
fn migrate_user_hicolor_icons(
    ...
    if marker_path.exists() {
        // Fast path: marker short-circuit, without any further
        // filesystem access.
        return Ok(());
    }

    // Distinguish "missing" (Ok(false) → nothing to migrate, mark
    ...
    // Remember the first deletion failure. A stale icon we couldn't
    ...
    for &size in sizes {
        let rel = format!("{size}x{size}/apps/splitlane.png");
        ...
        let user_meta = match std::fs::symlink_metadata(&user_file) {
            ...
            Err(err) => {
                log::warn!(
                    "splitlane: cannot stat user icon {} ({err}); skipping this size",
                    user_file.display()
                );
                continue;
            }
        ...
        if user_meta.file_type().is_symlink() {
            log::warn!(
                "splitlane: user icon {} is a symlink; skipping (refusing to follow for hash + remove)",
                user_file.display()
            );
            continue;
        }
        ...
        let user_hash = match sha256_of(&user_file) {
            ...
            Err(err) => {
                log::warn!(
                    "splitlane: cannot hash {} ({err}); leaving user-local icon in place",
                    user_file.display()
                );
                continue;
            }
        ...
        let system_hash = match sha256_of(&system_file) {
            ...
            Err(err) => {
                ...
                log::warn!(
                    "splitlane: system icon {} unreadable ({err}); preserving {}",
                    system_file.display(),
                    user_file.display()
                );
                continue;
        ...
        match std::fs::remove_file(&user_file) {
            Ok(()) => {
                log::info!(
                    "splitlane: removed stale user-local icon {} (sha256 differs from system copy)",
                    user_file.display()
                );
                deleted_any = true;
            }
            Err(err) => {
                log::warn!(
                    "splitlane: cannot remove stale user-local icon {} ({err})",
                    user_file.display()
                );
    ...
    // Write the marker only on a genuinely complete run (Completed /
...
fn maybe_remove_orphaned_cache(user_icon_dir: &Path) {
    ...
    if index_exists {
        log::info!(
            "splitlane: user-local hicolor theme has an index.theme at {}; preserving icon-theme.cache",
            index_file.display()
        );
        return;
    }
    ...
    match std::fs::remove_file(&cache_file) {
        Ok(()) => log::info!(
            "splitlane: removed orphaned user-local icon-theme.cache at {}",
            cache_file.display()
        ),
        Err(err) => log::warn!(
            "splitlane: cannot remove orphaned icon-theme.cache at {} ({err})",
            cache_file.display()
        ),
    }
}
...
fn write_marker(cache_dir: &Path, marker_path: &Path) {
    if let Err(err) = std::fs::create_dir_all(cache_dir) {
        log::warn!(
            "splitlane: cannot create cache dir {} ({err}); migration marker will retry next boot",
            cache_dir.display()
        );
        return;
    }
    if let Err(err) = std::fs::write(marker_path, b"v0.2.3 hicolor cleanup\n") {
        log::warn!(
            "splitlane: cannot write migration marker {} ({err}); migration will retry next boot",
            marker_path.display()
        );
    }
}
...
// ────────────────────────────────────────────────────────────────────────────
// Coexistence detection + one-time advisory toast.
// ────────────────────────────────────────────────────────────────────────────
...
/// Canonical absolute path of the rpm/deb-installed binary. Shared by the icon
/// migration and coexistence detection so a rename is a single-site change.
const SYSTEM_BIN_PATH: &str = "/usr/bin/splitlane";
...
pub struct CoexistenceReport {
    ...
    /// For SystemPackage this is `/usr/bin/splitlane`; for TarGz it's the
    /// binary under `$HOME/.local/splitlane.app/bin/splitlane`.
...
/// Detect when the host carries both flavours of Splitlane at
...
/// also contains a Splitlane binary:
///
/// - `SystemPackage { Dnf | Apt | Zypper }` running + `$HOME/.local/splitlane.app/bin/splitlane` present
/// - `TarGz` running + `/usr/bin/splitlane` present
///
    ...
    match current {
        ...
        InstallMethod::TarGz { app_dir } => {
            if system_bin_exists {
                Some(CoexistenceReport {
                    running_path: app_dir.join("bin").join("splitlane"),
...
/// Resolve the coexistence marker path: `~/.cache/splitlane/<MARKER_FILENAME>`.
...
/// recur next session, which is accepted as low annoyance.
pub fn write_coexistence_marker(marker_path: &Path) {
    if let Some(parent) = marker_path.parent()
        && let Err(err) = std::fs::create_dir_all(parent)
    {
        log::warn!(
            "splitlane: cannot create cache dir {} ({err}); coexistence toast may recur next session",
            parent.display()
        );
        return;
    }
    if let Err(err) = std::fs::write(marker_path, b"v0.2.3 coexistence warned\n") {
        log::warn!(
            "splitlane: cannot write coexistence marker {} ({err}); toast may recur next session",
            marker_path.display()
        );
    }
}
...
/// testable without standing up a full `SplitlaneApp` + GPUI context. Returns
/// `Some(report)` iff:
///
...
/// warned" case, which must be a silent no-op on the toast
...
mod tests {
    ...
    impl Layout {
        fn new() -> Self {
            ...
            let cache_dir = tmp.path().join("cache/splitlane");
            Self {
                _tmp: tmp,
                user_icon_dir,
                system_icon_dir,
        ...
        fn write_png(&self, root: &Path, size: u32, bytes: &[u8]) {
            let p = root.join(format!("{size}x{size}/apps/splitlane.png"));
        ...
        fn user_png(&self, size: u32) -> PathBuf {
            self.user_icon_dir
                .join(format!("{size}x{size}/apps/splitlane.png"))
        }
    }
    ...
    fn migrate_refuses_to_follow_user_icon_symlinks() {
        ...
        let link_path = layout.user_icon_dir.join("48x48/apps/splitlane.png");
    ...
    // ── coexistence detection + marker short-circuit ─────────────

    #[test]
    fn detect_coexistence_reports_tar_gz_when_system_package_is_running_and_home_app_dir_exists() {
        // Running binary: /usr/bin/splitlane (Dnf system package).
        // Tar.gz residue: $HOME/.local/splitlane.app/bin/splitlane still on disk.
        ...
        assert_eq!(
            report,
            Some(CoexistenceReport {
                running_path: PathBuf::from("/usr/bin/splitlane"),
                other_path: PathBuf::from("/home/alice/.local/splitlane.app/bin/splitlane"),
                other_method_label: "tar.gz",
            })
        );
    }
    ...
    fn detect_coexistence_reports_system_package_when_tar_gz_is_running_and_usr_bin_exists() {
        // Running binary: $HOME/.local/splitlane.app/bin/splitlane (TarGz).
        // System-package residue: /usr/bin/splitlane also on disk.
        let app_dir = PathBuf::from("/home/bob/.local/splitlane.app");
        let report = detect_coexistent_install_with_probes(
            &InstallMethod::TarGz {
                app_dir: app_dir.clone(),
            },
            Some(PathBuf::from("/home/bob")),
            true, // /usr/bin/splitlane present
            |_p| false,
        );
        assert_eq!(
            report,
            Some(CoexistenceReport {
                running_path: app_dir.join("bin").join("splitlane"),
                other_path: PathBuf::from("/usr/bin/splitlane"),
                other_method_label: "system package",
            })
        );
    }
    ...
    #[test]
    fn coexistence_marker_path_uses_versioned_filename_under_cache_splitlane() {
        // Pin the marker-path layout so future migrations can
        // rely on the `~/.cache/splitlane/migration-v<version>-*` convention.
        let home = Path::new("/home/carol");
        assert_eq!(
            coexistence_marker_path(home),
            PathBuf::from("/home/carol/.cache/splitlane/migration-v0.2.3-coexistence-warned"),
        );
    }

    #[test]
    fn write_coexistence_marker_creates_parent_directory() {
        // `~/.cache/splitlane/` may not exist on a very fresh install - the
