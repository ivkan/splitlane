...
//! root of trust for the self-updater.
//!
//! ## Why this exists
//!
//! Before this module, every installer (`linux/targz.rs`, `macos/dmg.rs`,
...
//! `SPLITLANE_MINISIGN_PUBKEY`); the matching secret key lives
...
//! ## Dual-key rotation
//!
//! Two slots are embedded - `SPLITLANE_MINISIGN_PUBKEY` (current) and
//! `SPLITLANE_MINISIGN_PUBKEY_NEXT` (next). Verification accepts a signature
...
//! it, then retire the old key after a few releases.
//!
//! ## Cross-platform
//!
...
//! second, independent layer added per-platform.
...
/// the rest of the update flow uses.
const SIG_HTTP_TIMEOUT: Duration = Duration::from_secs(30);

/// Current minisign public key, embedded at build time by CI. This
...
/// var - dev builds, and any release cut before CI signing was wired.
const EMBEDDED_PUBKEY_CURRENT: Option<&str> = option_env!("SPLITLANE_MINISIGN_PUBKEY");

/// Next minisign public key for rotation. A signature made by this
...
const EMBEDDED_PUBKEY_NEXT: Option<&str> = option_env!("SPLITLANE_MINISIGN_PUBKEY_NEXT");
...
fn embedded_public_keys() -> Vec<PublicKey> {
    [
        ("SPLITLANE_MINISIGN_PUBKEY", EMBEDDED_PUBKEY_CURRENT),
        ("SPLITLANE_MINISIGN_PUBKEY_NEXT", EMBEDDED_PUBKEY_NEXT),
    ]
    .into_iter()
...
/// frozen "corrupt or tampered" toast - exactly the right message
...
/// the mandated unhappy path: we never install an unsigned
/// artifact.
...
mod tests {
    ...
    fn verifies_a_correctly_signed_artifact() {
        let data = b"splitlane-0.3.9-x86_64.tar.gz payload";
    ...
    fn rejects_a_tampered_artifact() {
        ...
        // exact MITM scenario signature verification closes. Must fail before any caller
        // would extract/exec.
