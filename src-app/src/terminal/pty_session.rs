...
use super::marks::{
    CommandMark, Osc133Scanner, RawMark, ScanEvent, SharedLastCommand, SharedMarkRing,
...
/// Default scrollback history length, in lines. Splitlane keeps this standard
/// for predictable terminal memory use. `TermConfig::default()` is `0`, which
/// disables scrollback entirely. Overridable via
/// `terminal.scrollback_lines` in `splitlane.json` - see
/// [`splitlane_config::TerminalConfig::resolved_scrollback_lines`].
...
/// it is a *nested child* of the Claude Code session that launched Splitlane.
///
...
/// whatever spawned Splitlane. Inheriting is the only way these reach the child
/// - Splitlane itself namespaces everything it sets under `SPLITLANE_*`.
///
/// The list itself lives in `splitlane-acp`, which also owns the process-wide
/// startup scrub. That scrub is what actually keeps these away from a child:
/// the map below is an OVERLAY on an inherited environment (the backend spawns
/// with no `env_clear`), so dropping a key here cannot unset an inherited one.
/// What this layer still buys is refusing a marker written by hand into
/// `terminal.env` or restored from a `session.json` surface env.
const INHERITED_AGENT_SESSION_ENV: &[&str] = splitlane_acp::INHERITED_AGENT_SESSION_ENV;
const MAX_PENDING_CLIPBOARD_OPS: usize = 8;

/// Read the user's configured scrollback length, clamped to the
/// [`splitlane_config::TerminalConfig`] allowed range. Falls back to
/// [`DEFAULT_SCROLLBACK_LINES`] when no `terminal` block exists.
fn resolved_scrollback_lines(profile: TerminalSurfaceProfile) -> usize {
    splitlane_config::loader::load_config()
        .terminal
...
/// Map the pure config cursor shape to the backend's nearest native
/// shape. Splitlane paints `Vintage` and `DoubleUnderline` itself on top of
/// these fallback shapes.
fn map_cursor_shape(
    c: splitlane_config::schema::CursorShapeConfig,
...
/// Resolve the configured default cursor shape into an alacritty
...
// ---------------------------------------------------------------------------
// PTY notifier - replaces alacritty's Notifier
// ---------------------------------------------------------------------------
...
/// one named enum instead of an anonymous `Option`, and so the background spawn can *promote*
/// a `DisplayOnly` terminal to `Pty` once it resolves.
#[derive(Clone)]
...
/// stay private to this backend module; GPUI receives only Splitlane-owned
/// commands and snapshots. A Ghostty implementation can sit behind the
...
impl TerminalSessionBackend {
    ...
    fn scroll(&self, scroll: AlacScroll) -> bool {
        ...
        if let Some(ghostty) = &self.ghostty {
            ...
            let scroll = match scroll {
                AlacScroll::Top => splitlane_terminal_ghostty::Scroll::Top,
                AlacScroll::Bottom => splitlane_terminal_ghostty::Scroll::Bottom,
                AlacScroll::Delta(delta) => splitlane_terminal_ghostty::Scroll::Delta(delta),
                AlacScroll::PageUp => splitlane_terminal_ghostty::Scroll::Delta(
                    i32::try_from(metrics.screen_lines).unwrap_or(i32::MAX),
                ),
                AlacScroll::PageDown => splitlane_terminal_ghostty::Scroll::Delta(
                    -i32::try_from(metrics.screen_lines).unwrap_or(i32::MAX),
                ),
            };
            ...
            if matches!(scroll, splitlane_terminal_ghostty::Scroll::Delta(0)) {
                return false;
            }
    ...
    pub(crate) fn search(
        ...
        if let Some(ghostty) = &self.ghostty {
            return ghostty.search(query, regex, case_sensitive);
        }
        crate::search::search_term(&self.term, query, regex, case_sensitive)
    }
...
enum PendingTerminalInput {
    ...
    Key {
        input: splitlane_terminal_ghostty::KeyInput,
    ...
    Mouse {
        input: splitlane_terminal_ghostty::MouseInput,
        repeat: usize,
    ...
    Focus(splitlane_terminal_ghostty::FocusEvent),
    #[cfg(any(
        all(target_os = "linux", feature = "libghostty-linux"),
        all(
            target_os = "windows",
...
impl PendingTerminalInput {
    fn queued_bytes(&self) -> usize {
        match self {
            ...
            Self::Key { input, legacy } => {
                std::mem::size_of::<splitlane_terminal_ghostty::KeyInput>()
            ...
            Self::Mouse { repeat, legacy, .. } => {
                std::mem::size_of::<splitlane_terminal_ghostty::MouseInput>()
            ...
            Self::Focus(_) => std::mem::size_of::<splitlane_terminal_ghostty::FocusEvent>(),
            #[cfg(any(
                all(target_os = "linux", feature = "libghostty-linux"),
                all(
                    target_os = "windows",
    ...
    fn queue_limit(&self) -> usize {
        match self {
            ...
            Self::Key { input, .. }
                if input.action == splitlane_terminal_ghostty::KeyAction::Release =>
            {
                MAX_PENDING_INPUT_BYTES
            }
            Self::Mouse { input, .. }
                if input.action == splitlane_terminal_ghostty::MouseAction::Release =>
            {
                MAX_PENDING_INPUT_BYTES
            }
    ...
    fn into_legacy_bytes(self) -> Option<Cow<'static, [u8]>> {
        match self {
            ...
            Self::Focus(event) => Some(Cow::Borrowed(match event {
                splitlane_terminal_ghostty::FocusEvent::Gained => b"\x1b[I",
                splitlane_terminal_ghostty::FocusEvent::Lost => b"\x1b[O",
            })),
            #[cfg(any(
                all(target_os = "linux", feature = "libghostty-linux"),
                all(
...
pub struct TerminalState {
    ...
    /// The newest command line the shell reported through `133;E`, for the
    /// slot header's "↻ rerun". Empty until the shell integration reports one.
    pub(crate) last_command: SharedLastCommand,
    pub(crate) marks: SharedMarkRing,
    pub exited: Option<i32>,
    /// Set true once any user input (keystroke, paste, mouse report,
    ...
    pub child_pid: u32,
    /// Raw fd of the PTY master, captured at spawn before the master
    ...
    pub title: String,
    /// Current working directory of the shell process. OSC 7
    ...
    /// User-assigned custom name. When `Some`, it overrides the
    ...
    /// Agent CLI detected in this terminal's PTY subtree by
    ...
    /// `false` while `detected_agent` is a session-restored "last
    ...
    /// LISTEN ports attributed to this terminal's PTY
    ...
    /// Ports whose service URL was announced in THIS terminal
    ...
    /// Ports announced by service URLs detected in this terminal's
    ...
    /// Per-pane font-size override in points. `None` follows
    ...
    clipboard_gate: Arc<ClipboardGate>,
    /// Shell syntax used when Splitlane inserts OS file paths into the PTY.
    ...
    /// Throttle counter for the proc-based CWD refresh in
    ...
    /// Input written through `write_to_pty` while the terminal is
...
pub(super) struct SpawnedPty {
    ...
    last_command: SharedLastCommand,
    #[cfg(all(unix, not(test)))]
    pty_guard: Option<crate::agents::parent_guard::PtyGuardHandle>,
    #[cfg(target_os = "macos")]
    pty_master_fd: Option<i32>,
...
struct Osc7Pty<T: tty::EventedPty> {
    ...
    /// The command line the shell last reported (`133;E`). Written here on the
    /// reader thread and read on the render thread, so it is a lock rather
    /// than a channel: only the newest value matters and a queue of them would
    /// just be discarded.
    last_command: SharedLastCommand,
}
...
impl<T: tty::EventedPty> Read for Osc7Pty<T> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        loop {
            ...
            let last_command = &self.last_command;
            self.marks_scanner
                .feed(&buf[..read], &mut |event| match event {
                    ScanEvent::Mark(mark) => {
                        let _ = marks_tx.try_send(mark);
                    }
                    ScanEvent::CommandLine(payload) => {
                        if let Some(command) = sanitize_command_line(payload) {
                            *last_command
                                .lock()
...
/// disposition even when the spawn runs on a background thread.
#[cfg(unix)]
...
impl TerminalState {
    ...
    pub(super) fn attach_ghostty(
        ...
        self.last_command = ghostty.last_command();
        ...
        while let Some(input) = pending.front().cloned() {
            match input.try_send(ghostty) {
                ...
                GhosttyInputSendResult::Closed => {
                    ...
                    pending.clear();
                    log::warn!(
                        target: "splitlane::terminal::ghostty",
    ...
    pub fn backend_diagnostics(&self) -> TerminalBackendDiagnostics {
        ...
        let ghostty = {
            let identity = splitlane_terminal_ghostty::build_identity();
            Some(GhosttyBuildDiagnostics {
                version: splitlane_terminal_ghostty::GHOSTTY_APP_VERSION,
        ...
        TerminalBackendDiagnostics {
            ...
            target_triple: env!("SPLITLANE_TARGET_TRIPLE"),
            ghostty,
        }
    }
    ...
    /// (`TerminalView::with_cwd_and_env`) runs the same four steps but
    ...
    /// Grid size a surface starts at before the first layout pass measures the
    /// real one. Shared by [`resolve_spawn_params_with_profile`] and the
    /// display-only placeholder built at construction, so a deferred backend
    /// start cannot drift from the size its params later resolve to.
    pub(super) const DEFAULT_GRID_SIZE: (usize, usize) = (120, 40);

    /// Resolve the shell, the merged + assembled child env, the cwd, and the
    /// grid size - the non-forking half of a spawn. Factored out of `new` so
    /// the off-thread path runs the *blocking* half ([`open_pty_and_eventloop`])
    /// on the background executor.
    ///
    /// Not render-thread-safe despite the name: resolving the shell stats
    /// candidates (and searches PATH on Windows) and `setup_shell_integration`
    /// writes an rc snippet to disk. The deferred backend start therefore runs
    /// this on the background executor too, at mount rather than at session restore.
    #[allow(dead_code)]
    ...
    pub(super) fn resolve_spawn_params_with_profile(
        ...
        // Fallback chain handled by `resolve_default_shell`:
        ...
        // Layer the per-surface `user_env` on top of the global
        ...
        // Clean opt-out - with `shell_integration: false` no
        ...
        let (cols, rows) = initial_size.unwrap_or(Self::DEFAULT_GRID_SIZE);
        SpawnParams {
            shell,
            shell_quoting,
            extra_args,
    ...
    /// channel and [`promote`](Self::promote) it. The returned
    ...
    pub(super) fn open_pty_and_eventloop(
        ...
        let last_command: SharedLastCommand = Arc::new(std::sync::Mutex::new(None));
        ...
        let pty_master_fd = {
            ...
            // `dup()` the master fd so we own a copy whose lifetime we
        ...
        let pty = Osc7Pty::new(pty, cwd_tx, marks_tx, Arc::clone(&last_command));
    ...
    pub(super) fn promote(&mut self, spawned: SpawnedPty) {
        ...
        self.last_command = spawned.last_command;
        self.child_pid = spawned.child_pid;
        #[cfg(all(unix, not(test)))]
        {
        ...
        self.cursor_blinking = true;
        self.dirty = true;
        // Flush input queued while display-only: the launch command
    ...
    /// channel and [`promote`](Self::promote) it. `new_display_only`
    ...
    /// Refresh the shell CWD from the process table.
    ///
    ...
    fn drain_marks(&mut self) {
        ...
        let column = metrics.cursor.column.0;
    ...
    fn process_event(&mut self, event: AlacEvent) {
        match event {
            AlacEvent::Wakeup => {
                self.dirty = true;
                // Advance the readiness signal `workspace.up` polls.
            ...
            AlacEvent::ChildExit(status) => {
                ...
                // Exit status now comes natively from alacritty's
            ...
            AlacEvent::Exit => {
                self.reset_active_modes();
                // First-write-wins: `Exit` is the EOF fallback with no
    ...
    fn process_ghostty_event(&mut self, event: GhosttyUiEvent) {
        match event {
            ...
            GhosttyUiEvent::InputRejected(error) => {
                log::warn!(target: "splitlane::terminal::ghostty", "{error}");
            }
            GhosttyUiEvent::RuntimeFailed(error) => {
                log::error!(target: "splitlane::terminal::ghostty", "{error}");
                if self.exited.is_none() {
                    self.exited = Some(-1);
                }
                self.dirty = true;
    ...
    pub fn cwd_now(&self) -> Option<std::path::PathBuf> {
        // Once the child has exited, `child_pid` is stale and the OS
    ...
    pub fn cwd_now(&self) -> Option<std::path::PathBuf> {
        ...
        // After exit, `child_pid` may have been reused - `proc_pidinfo`
    ...
    pub(super) fn write_ghostty_focus(
        &self,
        event: splitlane_terminal_ghostty::FocusEvent,
    ) -> BackendInputResult {
    ...
    /// display-only (the pre-promotion window). The display-only notifier
    ...
    /// Whether a child exit should close the pane. A user-initiated
    ...
    /// result. Backs the `surface.search` IPC method. Alacritty holds
    ...
    pub fn search_scrollback(
        ...
        let result = crate::search::search_term(&self.term, pattern, false, false);
    ...
    /// Remember a port announced by a service URL in this
    ...
    /// a prior truncated scrollback - ANSI-safe defense-in-depth.
...
/// Compute the Splitlane IPC socket path, delegating to `runtime_paths` so
/// the fallback chain stays in sync with `ipc::socket_path`.
fn splitlane_socket_path() -> Option<String> {
    crate::runtime_paths::socket_path().map(|p| p.display().to_string())
}

/// Extract the embedded AI-hook binaries into the user's cache
/// dir, then expose that dir via `SPLITLANE_BIN_DIR` and prepend it to
...
/// opens normally without the AI-hook loader for this session. The
/// terminal must never fail to open because of AI-hook wiring.
///
...
/// (already unit-tested there); this glue only layers the env
...
fn inject_ai_hook_env(env: &mut std::collections::HashMap<String, String>) {
    let bin_dir = match crate::ai_hooks::extract::ensure_binaries_extracted() {
        ...
        Err(e) => {
            ...
            log::warn!(
                "splitlane: AI-hook binary extraction failed ({e:#}); sidebar loader will not activate for this terminal session"
            );
            return;
        }
    };

    // `SPLITLANE_BIN_DIR` is the source-of-truth the shim uses for its
    // self-exclusion PATH walk. Set it even in the unlikely
    // event the PATH-prepend below fails, so the shim can still
    // identify its own dir if a later code path routes into it.
    env.insert("SPLITLANE_BIN_DIR".into(), bin_dir.display().to_string());

    prepend_bin_dir_to_path(env, &bin_dir);
}

fn reassert_splitlane_bin_dir_first(env: &mut std::collections::HashMap<String, String>) {
    let Some(bin_dir) = env.get("SPLITLANE_BIN_DIR").cloned() else {
        return;
    };
    if bin_dir.is_empty() {
        return;
...
fn prepend_bin_dir_to_path(
    ...
    match std::env::join_paths(components) {
        ...
        Err(e) => {
            log::warn!(
                "splitlane: could not prepend AI-hook bin dir {} to PATH: {e}",
                bin_dir.display()
            );
        }
    }
...
fn merge_wslenv<'a>(
    ...
    let additions = keys
        .into_iter()
        .filter_map(|key| {
            let requires_path_translation =
                matches!(key, "SPLITLANE_BIN_DIR" | "SPLITLANE_HOOK_LOG");
            if existing_entries
                .iter()
...
mod title_filter_tests {
    ...
    fn drops_shell_self_path_title_but_keeps_human_labels() {
        ...
        assert!(!is_executable_path_title(r"C:\dev\splitlane"));
        assert!(!is_executable_path_title(""));
...
/// Assemble the child PTY environment: Splitlane identity vars, explicit TERM /
...
/// Splitlane owns and `SPLITLANE_BIN_DIR` is re-prepended after any user PATH).
...
/// `PtyBackend::spawn` seam is gone. Mirrors Zed's
...
fn assemble_pty_env(
    ...
    // Splitlane identity vars (AI-hook + MCP bridge integration).
    ...
    if workspace_id != 0 {
        env.insert("SPLITLANE_WORKSPACE_ID".into(), workspace_id.to_string());
    }
    env.insert("SPLITLANE_SURFACE_ID".into(), surface_id.to_string());
    ...
    if let Some(log_path) = std::env::var_os("SPLITLANE_HOOK_LOG")
        .filter(|v| !v.is_empty())
        .map(|v| v.to_string_lossy().into_owned())
    {
        env.insert("SPLITLANE_HOOK_LOG".into(), log_path);
    }
    ...
    env.insert("TERM_PROGRAM".into(), "splitlane".into());
    env.insert(
    ...
    // Splitlane itself inherited (typically >= 2 when launched from a terminal),
    ...
    // Silent-fail (the terminal still opens). Sets `SPLITLANE_BIN_DIR`.
    inject_ai_hook_env(&mut env);

    // Merge user-supplied env on top, EXCEPT the protected keys Splitlane owns:
    // TERM/COLORTERM/TERM_PROGRAM drive capability detection; SHLVL is reset so
    // shells start fresh; the SPLITLANE_* identity vars are how the MCP bridge
    // and the AI-hook shim find Splitlane - letting a user clobber them would
    ...
    if let Some(user_vars) = user_env {
        ...
        for (k, v) in user_vars {
            ...
            // documented use case), but SPLITLANE_BIN_DIR is re-prepended
    ...
    reassert_splitlane_bin_dir_first(&mut env);
...
/// graceful-shutdown step is unit-testable.
#[cfg(unix)]
...
/// Format a numeric signal (from alacritty's native
...
impl Drop for TerminalState {
    fn drop(&mut self) {
        ...
        // Close the dup'd PTY master fd we own (macOS). Done exactly
        ...
        #[cfg(windows)]
        {
            ...
            // Mirrors the Unix path above: skip the kill ladder entirely
            ...
            if pid != 0 && self.exited.is_none() {
                // Asymmetry: Windows console processes have no
...
fn request_windows_pid_termination(
    ...
    if handle.is_null() {
        ...
        log::debug!(
            "splitlane: Windows pane cleanup could not open pid={pid} (os_error={os_error:?})"
        );
        return None;
    }
    ...
    match unsafe { WaitForSingleObject(process.handle, 0) } {
        ...
        WAIT_FAILED => {
            ...
            result.failures = result.failures.saturating_add(1);
            log::warn!(
                "splitlane: Windows pane cleanup precheck failed for pid={pid} (os_error={os_error:?})"
            );
        }
        status => {
            result.failures = result.failures.saturating_add(1);
            log::warn!(
                "splitlane: Windows pane cleanup precheck returned status={status:#x} for pid={pid}"
            );
        }
    }
    ...
    if unsafe { TerminateProcess(process.handle, 1) } == 0 {
        ...
        if exited {
            ...
            result.failures = result.failures.saturating_add(1);
            log::debug!(
                "splitlane: Windows pane cleanup could not terminate pid={pid} (os_error={terminate_error:?})"
            );
        }
        return None;
    }
    ...
    for process in handles {
        ...
        match unsafe { WaitForSingleObject(process.handle, 0) } {
            ...
            WAIT_FAILED => {
                ...
                result.failures = result.failures.saturating_add(1);
                log::warn!(
                    "splitlane: Windows pane cleanup wait failed for pid={} (os_error={os_error:?})",
                    process.pid
                );
            }
            status => {
                result.failures = result.failures.saturating_add(1);
                log::warn!(
                    "splitlane: Windows pane cleanup wait returned status={status:#x} for pid={}",
                    process.pid
                );
            }
        }
...
pub(super) fn terminate_windows_process_tree(
    ...
    for pass in 0..KILL_PASSES {
        ...
        let (entries, snapshot_failed) = match windows_process_entries() {
            ...
            Err(error) => {
                result.failures = result.failures.saturating_add(1);
                log::warn!(
                    "splitlane: Windows pane cleanup snapshot failed for root_pid={root_pid} (os_error={:?})",
                    error.raw_os_error()
                );
                (Vec::new(), true)
            }
    ...
    if result.failures != 0 || result.timed_out != 0 {
        log::warn!(
            "splitlane: Windows pane cleanup incomplete (root_pid={root_pid}, targeted={}, terminate_requested={}, already_exited={}, failures={}, timed_out={}, deadline_exhausted={})",
            result.targeted,
...
mod tests {
    ...
    fn display_only_sender_is_inert() {
        // The DisplayOnly variant of TerminalType has no EventLoop, so
    ...
    fn new_display_only_terminal_has_no_pty() {
        // The spawn-failure fallback builds a DisplayOnly terminal; its
    ...
    fn new_pending_terminal_starts_display_only_then_promotes_conceptually() {
        // A pending terminal is display-only (no PTY) until promoted.
    ...
    fn backend_diagnostics_expose_target_triple() {
        ...
        assert_eq!(diagnostics.target_triple, env!("SPLITLANE_TARGET_TRIPLE"));
    ...
    fn backend_diagnostics_expose_pinned_ghostty_build_identity() {
        ...
        assert_eq!(
            ghostty.version,
            splitlane_terminal_ghostty::GHOSTTY_APP_VERSION
        );
    ...
    fn write_to_pty_buffers_input_while_display_only() {
        // Regression: the Agents-view "New thread" picker writes the
    ...
    fn test_key_input(
        action: splitlane_terminal_ghostty::KeyAction,
    ) -> splitlane_terminal_ghostty::KeyInput {
        splitlane_terminal_ghostty::KeyInput {
            key: splitlane_terminal_ghostty::Key::Function(5),
            action,
            modifiers: splitlane_terminal_ghostty::Modifiers::CONTROL,
    ...
    fn test_mouse_input(
        action: splitlane_terminal_ghostty::MouseAction,
    ) -> splitlane_terminal_ghostty::MouseInput {
        splitlane_terminal_ghostty::MouseInput {
            action,
            button: Some(splitlane_terminal_ghostty::MouseButton::Left),
            ...
            padding_left: 0,
            padding_right: 0,
            any_button_pressed: action != splitlane_terminal_ghostty::MouseAction::Release,
        }
    }
    ...
    fn control_releases_fit_after_general_input_saturates() {
        ...
        let press = PendingTerminalInput::Key {
            input: test_key_input(splitlane_terminal_ghostty::KeyAction::Press),
            legacy: None,
        };
        let key_release = PendingTerminalInput::Key {
            input: test_key_input(splitlane_terminal_ghostty::KeyAction::Release),
            legacy: None,
        };
        let mouse_release = PendingTerminalInput::Mouse {
            input: test_mouse_input(splitlane_terminal_ghostty::MouseAction::Release),
            repeat: 1,
            legacy: Some(b"mouse-up".to_vec()),
        };
        let focus = PendingTerminalInput::Focus(splitlane_terminal_ghostty::FocusEvent::Lost);
    ...
    fn ghostty_startup_fallback_preserves_structured_input_in_order() {
        ...
        assert_eq!(
            state.write_ghostty_key(
                test_key_input(splitlane_terminal_ghostty::KeyAction::Press),
        ...
        assert_eq!(
            state.write_ghostty_mouse(
                test_mouse_input(splitlane_terminal_ghostty::MouseAction::Press),
                2,
                Some(b"M".to_vec()),
            ),
            BackendInputResult::Accepted
        ...
        assert_eq!(
            state.write_ghostty_focus(splitlane_terminal_ghostty::FocusEvent::Gained),
            BackendInputResult::Accepted
        );
        assert_eq!(
    ...
    fn promote_flushes_buffered_input_into_grid() {
        // End-to-end: input written while display-only must reach the
        ...
        state.write_to_pty(b"echo SPLITLANE_FLUSH_OK\n".to_vec());
        assert!(!state.notifier.0.is_pty());

        let spawned = TerminalState::open_pty_and_eventloop(params, pending, None)
            .expect("open a PTY-backed terminal via tty::new + EventLoop");
        state.promote(spawned);
        assert!(state.notifier.0.is_pty());
        ...
        for _ in 0..60 {
            ...
            state.sync();
            if grid_to_string(&state.term).contains("SPLITLANE_FLUSH_OK") {
                found = true;
                break;
            }
        }
        assert!(
            found,
            "input buffered before promotion never reached the shell"
        );
    }

    #[test]
    fn resolve_spawn_params_honors_initial_size() {
        // The cheap, render-thread-safe half of a spawn picks up the
    ...
    fn capture_foreground_signal_mask_succeeds_on_unix() {
        // The foreground mask snapshot must succeed on the main thread
    ...
    fn cursor_shape_maps_hollow_to_hollow_block() {
        // Config shapes map to renderer (vte) shapes; the config's
    ...
    fn prepend_puts_bin_dir_first_and_preserves_existing_entries() {
        ...
        let bin_dir = PathBuf::from("/home/u/.cache/splitlane/bin/0.2.6");
        ...
        assert_eq!(
            components.first(),
            Some(&bin_dir),
            "bin_dir must be first on PATH; got {components:?}"
        );
        assert!(
            components.iter().any(|p| p == Path::new("/usr/bin")),
            "original PATH entries must be preserved; got {components:?}"
        );
        assert!(
    ...
    fn prepend_inserts_bin_dir_even_when_env_path_absent() {
        ...
        let mut env: HashMap<String, String> = HashMap::new();
        let bin_dir = PathBuf::from("/tmp/splitlane-bins");
        prepend_bin_dir_to_path(&mut env, &bin_dir);
        ...
        assert_eq!(
            components.first(),
            Some(&bin_dir),
            "bin_dir must be first on PATH in the no-prior-PATH case"
        );
    }
    ...
    fn prepend_uses_platform_separator() {
        ...
        assert_eq!(
            ...
            "split_paths(join_paths(...)) must round-trip on all platforms"
        );
    }
    ...
    fn prepend_treats_empty_path_as_absent() {
        ...
        assert!(
            ...
            "empty PATH must not yield a phantom CWD entry; got {components:?}"
        );
        assert_eq!(
            components.first(),
            Some(&bin_dir),
            "bin_dir must still be first when empty PATH is treated as absent"
        );
    }

    // -----------------------------------------------------------------
    // Exit-status correctness (real code, first-write-wins).
    // -----------------------------------------------------------------

    #[test]
    fn child_exit_records_real_code_not_sentinel() {
        // A real child exit code must round-trip into `exited`,
    ...
    fn exit_fallback_does_not_clobber_real_child_exit_code() {
        // First-write-wins. A bare `Exit` (EOF, no status) must
        ...
        #[cfg(any(unix, windows))]
        {
            ...
            assert_eq!(
                state.exited,
                Some(1),
                "Exit must not clobber the real ChildExit code"
            );
        }
    }

    // -----------------------------------------------------------------
    // Keep pane open on launch failure (keyboard_input_sent).
    // -----------------------------------------------------------------

    #[test]
    fn close_on_exit_discriminator_covers_both_branches() {
        // Clean exit (code 0) closes even with no input.
        let mut clean = TerminalState::new_display_only(24, 80);
        clean.exited = Some(0);
        assert!(
            clean.should_close_on_exit(),
            "a clean exit (code 0) must close the pane"
        );
        ...
        assert!(
            ...
            "a non-zero exit with no input must keep the pane open"
        );
        ...
        assert!(
            failed.should_close_on_exit(),
            "after user input, a non-zero exit must close the pane"
        );
    }

    #[test]
    fn write_to_pty_marks_user_input_but_fresh_state_does_not() {
        // A fresh terminal has not received user input; write_to_pty
    ...
    // -----------------------------------------------------------------
    // Graceful teardown sends SIGTERM before SIGKILL.
    // -----------------------------------------------------------------
    ...
    fn terminate_process_group_is_noop_for_dead_or_invalid_group() {
        // Unhappy path: an empty/invalid group must be a harmless
    ...
    // -----------------------------------------------------------------
    // Env assembly contract. The mockable `PtyBackend::spawn` seam is gone
    // (the IO core now opens the PTY via alacritty's `tty::new`), so the env
    // that the child inherits is asserted directly against the pure
    // `assemble_pty_env`.
    // -----------------------------------------------------------------
    ...
    fn wslenv_merge_preserves_existing_entries_and_deduplicates() {
        let merged = merge_wslenv(
            ...
            ["ZED", "ALREADY", "EXISTING", "ZED", "SPLITLANE_BIN_DIR"],
        );

        assert_eq!(
            merged.as_deref(),
            Some("EXISTING/p:ALREADY/u:CUSTOM/uw:SPLITLANE_BIN_DIR/up:ZED/u")
        );
    }
    ...
    #[test]
    fn wslenv_merge_adds_up_when_splitlane_paths_lack_path_flag() {
        let merged = merge_wslenv(
            Some("SPLITLANE_HOOK_LOG/u:SPLITLANE_BIN_DIR/u"),
            ["SPLITLANE_HOOK_LOG", "SPLITLANE_BIN_DIR"],
        );

        assert_eq!(
            merged.as_deref(),
            Some(
                "SPLITLANE_HOOK_LOG/u:SPLITLANE_BIN_DIR/u:SPLITLANE_BIN_DIR/up:SPLITLANE_HOOK_LOG/up"
            )
        );
    }
    ...
    #[test]
    fn pty_spawn_injects_splitlane_bin_dir_and_prepends_path() {
        ...
        let bin_dir = env
            .get("SPLITLANE_BIN_DIR")
            .expect("SPLITLANE_BIN_DIR must be set in the child env")
            .clone();
        assert!(!bin_dir.is_empty(), "SPLITLANE_BIN_DIR must not be empty");
        ...
        assert_eq!(
            ...
            "SPLITLANE_BIN_DIR must be first on PATH"
        );
    }
    ...
    fn detached_terminal_does_not_advertise_fake_workspace_id() {
        ...
        assert!(
            !env.contains_key("SPLITLANE_WORKSPACE_ID"),
    ...
    fn user_env_is_merged_into_pty_env() {
        ...
        assert_eq!(
            ...
            Some("sk-test-123"),
            "user env var must be present in the child env"
        );
        assert_eq!(
            env.get("MY_CUSTOM_VAR").map(String::as_str),
            Some("hello"),
            "a second user env var must also be present"
        );
    }

    #[test]
    fn user_path_cannot_shadow_splitlane_bin_dir() {
        let mut user = HashMap::new();
        user.insert("PATH".to_string(), "/custom/bin".to_string());
        let env = assemble_pty_env(HashMap::new(), 1, 1, Some(user));
        let Some(bin_dir) = env.get("SPLITLANE_BIN_DIR") else {
            eprintln!("skip: SPLITLANE_BIN_DIR unavailable in this environment");
            return;
        };
        ...
        assert_eq!(
            ...
            "SPLITLANE_BIN_DIR must stay first even when user env sets PATH"
        );
        assert!(
    ...
    // TERM/COLORTERM are protected and cannot be overridden by user env.
    #[test]
    fn protected_keys_cannot_be_overridden_by_user_env() {
        ...
        assert_eq!(
            ...
            Some("xterm-256color"),
            "TERM must stay Splitlane-owned even if the user sets it"
        );
        assert_eq!(
            env.get("COLORTERM").map(String::as_str),
            Some("truecolor"),
            "COLORTERM must stay Splitlane-owned even if the user sets it"
        );
        assert_eq!(
            env.get("TERM_PROGRAM").map(String::as_str),
            Some("splitlane"),
            "TERM_PROGRAM must stay Splitlane-owned even if the user sets it"
        );
        assert_eq!(
            env.get("TERM_PROGRAM_VERSION").map(String::as_str),
            Some(env!("CARGO_PKG_VERSION")),
            "TERM_PROGRAM_VERSION must stay Splitlane-owned even if the user sets it"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("yes"),
            "a non-protected user var alongside protected ones still wins"
        );
    }
    ...
    fn inherited_agent_session_markers_are_dropped_from_child_env() {
        // Launching Splitlane from inside an agent session (or from an IDE
    ...
    fn osc7_payload_decodes_windows_file_uri() {
        ...
        assert_eq!(
            cwd_from_osc7_payload("7;file://DESKTOP-123/C:/dev/splitlane"),
            Some(r"C:\dev\splitlane".to_string())
        );
    }
    ...
    fn osc7_payload_rejects_legacy_malformed_windows_uri() {
        assert_eq!(
            cwd_from_osc7_payload(r"7;file://DESKTOP-123C:\dev\splitlane"),
            None
        );
    }
    ...
    /// Live smoke: spawn a REAL PTY-backed shell via
    ...
    fn eventloop_pty_echoes_input_into_grid() {
        let mut state = TerminalState::new(None, 1, 1, Some((80, 24)), None, None)
            .expect("spawn a PTY-backed terminal via tty::new + EventLoop");
        ...
        state.notifier.notify(b"echo SPLITLANE_SMOKE_OK\n".to_vec());
        ...
        for _ in 0..60 {
            std::thread::sleep(std::time::Duration::from_millis(50));
            state.sync();
            if grid_to_string(&state.term).contains("SPLITLANE_SMOKE_OK") {
                found = true;
                break;
            }
        }
    ...
    fn eventloop_drains_final_output_after_exit() {
        ...
        state
            .notifier
            .notify(b"echo SPLITLANE_FINAL_OK\nexit\n".to_vec());
        ...
        for _ in 0..240 {
            ...
            if visible.contains("SPLITLANE_FINAL_OK") || history.contains("SPLITLANE_FINAL_OK") {
                found = true;
                break;
            }
        }
    ...
    fn output_generation_advances_on_pty_output() {
        ...
        std::thread::sleep(std::time::Duration::from_millis(250));
        state.notifier.notify(b"echo SPLITLANE_GEN_OK\n".to_vec());
