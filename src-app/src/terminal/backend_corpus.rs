...
impl Harness {
    ...
    fn replay(mut self, case: &CorpusCase, chunks: &[usize]) -> NormalizedSnapshot {
        ...
        let search = case.search_after_feed.map(|query| {
            normalize_alacritty_search(crate::search::search_term(&self.term, query, false, false))
        });
...
struct GhosttyHarness {
    terminal: splitlane_terminal_ghostty::DisplayTerminal,
}
...
impl GhosttyHarness {
    fn new() -> Self {
        let size = splitlane_terminal_ghostty::WindowSize::new(80, 24, 0, 0)
            .expect("corpus dimensions are valid");
        Self {
            terminal: splitlane_terminal_ghostty::DisplayTerminal::new(size, 10_000)
    ...
    fn replay(mut self, case: &CorpusCase, chunks: &[usize]) -> NormalizedSnapshot {
        ...
        let resize_before = if let Some((columns, rows)) = case.resize_after_feed {
            ...
            let size = splitlane_terminal_ghostty::WindowSize::new(columns, rows, 0, 0)
        ...
        if let Some(range) = case.selection_after_feed {
            self.terminal
                .set_selection(splitlane_terminal_ghostty::SelectionRange {
                    start: splitlane_terminal_ghostty::Point::new(
                        range.start.line.0,
                        range.start.column.0,
                    ),
                    end: splitlane_terminal_ghostty::Point::new(
                        range.end.line.0,
                        range.end.column.0,
                    ),
        ...
        let search = case.search_after_feed.map(|query| {
            normalize_ghostty_search(
                self.terminal
                    // `false, false` - not a regex, not case-sensitive. This
                    // corpus exists to prove the two VT backends answer
                    // identically, so it has to ask the ghostty side the same
                    // question the alacritty side is asked.
...
fn normalize_ghostty_event(event: splitlane_terminal_ghostty::BackendEvent) -> Option<String> {
    match event {
        splitlane_terminal_ghostty::BackendEvent::WritePty(bytes) => {
            Some(normalize_pty_write(&String::from_utf8_lossy(&bytes)))
        }
        splitlane_terminal_ghostty::BackendEvent::ClipboardStore(text) => {
            Some(format!("ClipboardStore({text:?})"))
        }
        splitlane_terminal_ghostty::BackendEvent::Bell => Some("Bell".to_owned()),
        splitlane_terminal_ghostty::BackendEvent::Title(title) => Some(format!("Title({title:?})")),
        splitlane_terminal_ghostty::BackendEvent::WorkingDirectory(cwd) => {
            Some(format!("WorkingDirectory({cwd:?})"))
        }
        splitlane_terminal_ghostty::BackendEvent::CallbackPanicked
        | splitlane_terminal_ghostty::BackendEvent::InputDropped { .. } => None,
    }
}
...
fn normalize_ghostty_search(result: splitlane_terminal_ghostty::SearchResult) -> SearchObservation {
    SearchObservation {
        matches: result
            .matches
            .into_iter()
...
#[test]
#[ignore = "captures the machine-specific performance baseline"]
    ...
    for (case_index, case) in cases.iter().enumerate() {
        for (pane_index, pane) in panes.iter_mut().enumerate() {
            ...
            if let Some((columns, rows)) = case.resize_after_feed {
                pane.terminal
                    .resize(
                        splitlane_terminal_ghostty::WindowSize::new(columns, rows, 0, 0)
...
#[test]
#[ignore = "promotion gate: release-only relative parser and snapshot benchmark"]
#[allow(
