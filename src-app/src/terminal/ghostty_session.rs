...
use splitlane_terminal_ghostty as ghostty;
#[cfg(all(
    target_os = "windows",
    target_arch = "x86_64",
    target_env = "msvc",
...
impl GhosttySession {
    ...
    pub(super) fn start(
        ...
        if let Err(error) = std::thread::Builder::new()
            .name("splitlane-ghostty-runtime".into())
            .spawn(move || {
    ...
    pub(super) fn last_command(&self) -> SharedLastCommand {
        self.inner.last_command.clone()
    }
    ...
    pub(super) fn update_selection(&self, point: Point, _side: SelectionSide) -> Option<String> {
        ...
        // committed, which Splitlane already does in `finish_selection`.
        None
    }
...
fn run_runtime(
    ...
    let reader_worker = match std::thread::Builder::new()
        .name("splitlane-ghostty-pty-reader".into())
    ...
    loop {
        ...
        match mailbox.recv_timeout(Duration::from_millis(10)) {
            ...
            Ok(RuntimeMessage::Scroll(scroll)) => {
                ...
                if let Err(error) = refresh_shared_state(&inner, &mut terminal) {
                    log::warn!(target: "splitlane::terminal::ghostty", "Ghostty scroll failed: {error}");
                }
            }
...
fn search_scrollback_lines(
    ...
    // `false, false` = not a regex, not case-sensitive, which is not a taste
    // but an invariant: the same `search_pane` verb is answered by the OTHER VT
    // backend as `crate::search::search_term(&self.term, pattern, false, false)`
    // (`pty_session.rs`), and one CLI verb must not answer differently
    // depending on which backend happened to be selected at startup. The find
    // bar is a different caller and threads the user's `Aa` toggle through.
    let search = terminal
        .search(query, false, false)
        .map_err(|error| error.to_string())?;
...
fn process_output_batch(
    ...
    let result = (|| {
        while let Some(bytes) = next.take() {
            ...
            let emitted_mark =
                scan_chunk_for_marks(marks_scanner, bytes, &mut raw_marks, &inner.last_command);
...
fn scan_chunk_for_marks(
    ...
    scanner.feed(bytes, &mut |event| match event {
        ScanEvent::Mark(raw) => raw_marks.push(raw),
...
fn record_command_marks(inner: &SessionInner, raw_marks: &[RawMark]) {
    ...
    let column = state.content.cursor.point.column.0;
...
mod tests {
    ...
    fn output_batching_barrier_trips_only_when_a_chunk_completes_a_mark() {
        ...
        let last_command: SharedLastCommand = Arc::new(Mutex::new(None));
    ...
    fn run_windows_shell_case(
        ...
        let params = SpawnParams {
            ...
            env: std::collections::HashMap::from([
                ...
                ("TERM_PROGRAM".into(), "splitlane".into()),
                ("SPLITLANE_MATRIX".into(), "matrix-é中".into()),
                (
                    "WSLENV".into(),
                    "SPLITLANE_MATRIX/u:TERM/u:COLORTERM/u:TERM_PROGRAM/u".into(),
                ),
            ]),
            cwd: cwd.to_path_buf(),
            cols: 100,
    ...
    fn windows_shell_matrix_preserves_unicode_environment_and_cwd() {
        ...
        cases.push((
            ...
            vec![
                ...
                "/Q".into(),
                "/C".into(),
                "chcp 65001>nul & echo SPLITLANE_SHELL:cmd:%SPLITLANE_MATRIX% & echo %CD% & echo \x1b[38;2;1;2;3mCOLOR\x1b[0m"
                    .into(),
            ],
        ));
        cases.push((
            ...
            vec![
                ...
                "-NonInteractive".into(),
                "-Command".into(),
                "[Console]::OutputEncoding=[Text.UTF8Encoding]::new(); Write-Output \"SPLITLANE_SHELL:powershell:$env:SPLITLANE_MATRIX\"; Write-Output (Get-Location).Path; Write-Output \"$([char]27)[38;2;1;2;3mCOLOR$([char]27)[0m\""
                    .into(),
            ],
        ));
        cases.push((
            ...
            vec![
                ...
                "-NonInteractive".into(),
                "-Command".into(),
                "Write-Output \"SPLITLANE_SHELL:pwsh:$env:SPLITLANE_MATRIX\"; Write-Output (Get-Location).Path; Write-Output \"$([char]27)[38;2;1;2;3mCOLOR$([char]27)[0m\""
                    .into(),
            ],
        ));
        ...
        cases.push((
            ...
            vec![
                ...
                "--norc".into(),
                "-lc".into(),
                "printf 'SPLITLANE_SHELL:git-bash:%s\\n%s\\n\\033[38;2;1;2;3mCOLOR\\033[0m\\n' \"$SPLITLANE_MATRIX\" \"$PWD\""
                    .into(),
            ],
        ));
        ...
        for (name, shell, quoting, args) in cases {
            ...
            assert!(
                rendered.contains(&format!("SPLITLANE_SHELL:{name}:matrix-é中")),
            ...
            assert!(
                rendered.contains("splitlane shell é"),
    ...
    fn live_runtime_runs_platform_shell_and_reports_one_exit() {
        ...
        #[cfg(target_os = "windows")]
        {
            assert!(
                session
                    .write(b"echo SPLITLANE_CONPTY^_READY\r\n".to_vec())
                    .is_sent()
            );
            ...
            while Instant::now() < ready_deadline {
                if session
                    .recent_output_lines()
                    .iter()
                    .any(|line| line.contains("SPLITLANE_CONPTY_READY"))
                {
                    child_ready = true;
                    break;
                }
        ...
        assert!(
            rendered.contains("SPLITLANE_GHOSTTY_RUNTIME_OK:ghostty"),
        ...
        #[cfg(target_os = "windows")]
        assert!(
            rendered.contains("SPLITLANE_SIZE:30x100"),
    ...
    fn final_drain_closes_writer_and_master_before_reader_eof() {
        ...
        let closer = PtyCloser::new("splitlane-ghostty-test-pty-closer")
