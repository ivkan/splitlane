...
#[cfg(target_os = "windows")]
const JOB_HELPER_ENV: &str = "SPLITLANE_GHOSTTY_JOB_ABORT_HELPER";
#[cfg(target_os = "windows")]
const JOB_MARKER_ENV: &str = "SPLITLANE_GHOSTTY_JOB_ABORT_MARKER";
...
fn cycle_spec() -> SpawnSpec {
    SpawnSpec {
        ...
        args: vec![
            "-c".into(),
            "IFS= read -r line; printf 'SPLITLANE_STRESS:%s\\n' \"$line\"".into(),
        ],
    }
}
...
fn cycle_spec() -> SpawnSpec {
    SpawnSpec {
        ...
        args: vec![
            ...
            "/V:ON".into(),
            "/C".into(),
            "set /p SPLITLANE_LINE= & echo SPLITLANE_STRESS:!SPLITLANE_LINE!".into(),
        ],
    }
}
...
fn burst_spec() -> SpawnSpec {
    SpawnSpec {
        ...
        args: vec![
            ...
            "/Q".into(),
            "/K".into(),
            "for /L %i in (1,1,512) do @echo SPLITLANE_BURST".into(),
        ],
    }
}
...
#[cfg(target_os = "windows")]
#[test]
#[ignore = "performance gate: 5 warmups and 100 sequential ConPTY host creations"]
#[allow(
...
#[cfg(target_os = "windows")]
#[test]
#[ignore = "promotion gate: 32 concurrent ConPTY panes"]
...
#[cfg(target_os = "windows")]
#[test]
#[ignore = "promotion gate: Windows lifecycle scenario matrix"]
fn windows_ghostty_lifecycle_scenario_matrix_is_bounded() {
    ...
    ctrl_c.write(b"echo SPLITLANE_CTRL_C_OK\rexit\r".to_vec());
    assert!(
        ctrl_c.wait_for_marker("SPLITLANE_CTRL_C_OK", CYCLE_TIMEOUT),
...
#[cfg(target_os = "windows")]
#[test]
#[ignore = "promotion gate: abrupt app close cleans ConPTY Job Object"]
