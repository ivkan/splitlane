...
impl TerminalView {
    ...
    pub(super) fn reset_terminal(&mut self, cx: &mut Context<Self>) {
        ...
        // directly so the keyboard_input_sent flag is not tripped.
    ...
    /// step is a silent no-op (no toast). Writing the override is
    ...
    /// Hand the current query to the app, which widens it to
    /// every session in the palette's "Output" tab. Empty query is a silent
    /// no-op.
    ...
    /// Arm THIS view's local search with a fleet query (the
    ...
    /// scrollbar match rail renders from the same state.
    ...
    /// The find bar's `Aa` toggle. Governs both matching modes: in plain mode
    /// it stops the fold, in regex mode it drops the `i` flag - two toggles
    /// that disagreed about case would make a pattern with a letter in it
    /// unanswerable.
    ...
    fn schedule_search(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(
            async move |this: gpui::WeakEntity<Self>, cx: &mut gpui::AsyncApp| {
                ...
                let result =
                    smol::unblock(move || backend.search(&worker_query, regex, case_sensitive))
                        .await;
                let _ = cx.update(|cx| {
