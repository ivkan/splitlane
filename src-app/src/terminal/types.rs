...
//! this neutral leaf, allowing further decomposition.
//!
//! ## Backend-neutral types (Zed #57483)
//!
...
/// `alacritty_terminal` directly (backend confinement).
...
/// A grid line index. Splitlane-owned mirror of `alacritty_terminal::index::Line`
...
/// A grid column index. Splitlane-owned mirror of
/// `alacritty_terminal::index::Column` (a `pub usize` newtype).
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Column(pub usize);

/// A grid position, Splitlane-owned mirror of `alacritty_terminal::index::Point`.
///
...
/// Splitlane adds Windows Terminal-style `Vintage` and `DoubleUnderline` for
...
/// Cell attribute flags, Splitlane-owned mirror of the `term::cell::Flags`
...
/// Terminal private-mode flags, Splitlane-owned mirror of the `term::TermMode`
...
/// A computed selection span, Splitlane-owned mirror of
...
/// no GPU/display (the golden-frame net). Replaces the alacritty-typed
/// `CellSnapshot`.
#[derive(Clone, Debug)]
pub struct Cell {
...
/// instead of locking `Term` and importing alacritty types. Mirror of
...
/// is a zero `LayoutState` delta change. The caller holds the lock;
...
// ---------------------------------------------------------------------------
// Rendering glue types (Splitlane-owned, neutral)
// ---------------------------------------------------------------------------
...
/// Fields are populated here and consumed by hover/click.
/// `Clone`: the press point's link is stashed on mouse-down so the
...
mod tests {
    ...
    /// Confinement guard: `alacritty_terminal` must only be
    ...
    fn alacritty_confined_to_backend_allowlist() {
        ...
        assert!(
            violations.is_empty(),
            "alacritty_terminal leaked outside the backend allowlist. Route \
