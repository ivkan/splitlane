...
// Re-export APCA primitives so theme code (and theme tests) can
...
// Re-export the scrollbar geometry so the view's mouse handlers
...
/// Returns `true` if a cell at `point` (viewport coordinates) lies
...
struct BatchedTextRun {
    /// `SharedString` (not `String`) so the per-frame paint pass
...
///
/// Fallback: extension beyond the original `U+2580..U+2590` range, after
...
/// fully fill the cell, producing visible vertical gaps.
...
/// [`crate::terminal::types::Cell`] - no alacritty types reach here.
...
pub struct LayoutState {
    ...
    exited: Option<i32>,
    /// Signal name if the child was killed by a signal; the exit
...
pub struct TerminalElement {
    ...
    /// Shared sink for the painted scrollbar geometry. `paint()` writes
    ...
    /// Search-match positions as lines-from-grid-bottom,
...
impl TerminalElement {
    ...
    fn build_layout(
        ...
        // The grid sits inside a margin on **both** sides. It used to be
        // subtracted once and added to the origin, so the left margin was the
        // gutter and the right margin was the gutter plus whatever fraction of
        // a cell did not fit - anywhere from the gutter to nearly a full cell
        // wider, changing as the pane is resized. The text was never centred
        // in its pane and the error was largest exactly where a pane is
        // narrow.
        let gutter = px(crate::app::constants::PANE_TERMINAL_INSET);
        let available_width = (bounds.size.width - gutter * 2.0).max(px(0.0));
        let available_height = (bounds.size.height - gutter * 2.0).max(px(0.0));
...
/// Window-free rendering layout pass (the golden-frame net).
///
...
pub(crate) fn layout_from_snapshot(inputs: LayoutInputs<'_>) -> LayoutState {
    ...
    for cell in cells.iter() {
        ...
        //    Indices 0..=15 still go through contrast correction:
        ...
        // Cells inside the selection rect get the precomputed
    ...
    if let Some(sel) = &selection_range {
        ...
        if sel.is_block {
            // Block (rectangular) selection - emit one rect per
...
impl Element for TerminalElement {
    ...
    fn paint(
        ...
        let gutter = px(crate::app::constants::PANE_TERMINAL_INSET);
        // Offset the grid origin by the same inset reserved in layout, and put
        // the sub-cell remainder half on each side so the columns sit centred
        // between the two margins rather than pushed against the left one.
        let columns = layout.desired_cols as f32;
        let rows = layout.desired_rows as f32;
        let slack_x = (bounds.size.width - gutter * 2.0 - cell_width * columns).max(px(0.0));
        let slack_y =
            (bounds.size.height - gutter * 2.0 - layout.dimensions.line_height * rows).max(px(0.0));
        let mut origin = Point {
            x: bounds.origin.x + gutter + slack_x / 2.0,
            y: bounds.origin.y + gutter + slack_y / 2.0,
        };
        // Snap the origin to physical-pixel boundaries so the grid
        ...
        *self
        ...
        // SPLITLANE_PIXEL_PROBE: log the per-frame origin once, before any
        ...
        // The shared integer pixel boundary arrays are derived purely
        ...
        window.with_content_mask(Some(ContentMask { bounds }), |window| {
            ...
            // 3a. SPLITLANE_PIXEL_PROBE_OVERLAY: draw thin red cell borders
            // above the text. Independent of `SPLITLANE_PIXEL_PROBE`; opt-in
            ...
            // 5b. Search match rail - decimated ticks on the
            // same strip. Click-to-jump rides the existing proportional
            // track click (the scrollbar hit-test below); the rail disappears with
            ...
            // Publish the painted scrollbar geometry so the view's
...
// ---------------------------------------------------------------------------
// Block-character fallback - block_char_coverages tests
//
...
// tests lock in coverage for every codepoint added in the fallback
...
mod block_char_coverage_tests {
    ...
    /// Every original-table codepoint must still resolve to a single rect with
    /// the same geometry as before the slice refactor - guards against an
    /// accidental table edit during the fallback extension.
    #[test]
    ...
    /// The fallback extension targets exactly the codepoints found in the
    ...
    #[test]
    fn claude_code_codepoints_all_covered() {
        for c in [
            '▔', // U+2594 upper 1/8
            '▖', '▗', '▘', '▝', // single quadrants
            '▙', '▚', '▛', '▜', '▞', '▟', // multi quadrants
...
impl LayoutState {
    ...
    /// for the golden-frame net. Does NOT rely on any GPUI `Debug`
    ...
    /// with `SPLITLANE_BLESS_GOLDEN=1 cargo test -p splitlane-app golden_frame`.
...
mod golden_frame_tests {
    //! Golden-frame net: deterministic `LayoutState` snapshots over a
    ...
    //! never touching a GPUI context - is the Window-free proof. Each
    //! fixture asserts against a committed golden under `golden/`;
    //! regenerate with `SPLITLANE_BLESS_GOLDEN=1`.
    use super::*;
    ...
    fn assert_golden_text(name: &str, actual: String) {
        ...
        if std::env::var_os("SPLITLANE_BLESS_GOLDEN").is_some() {
        ...
        let expected = std::fs::read_to_string(&path).unwrap_or_else(|e| {
            panic!(
                "golden '{name}' missing ({e}); regenerate with \
                 SPLITLANE_BLESS_GOLDEN=1 cargo test -p splitlane-app golden_frame"
            )
        });
    ...
    /// Structural invariant: block-element cells
