...
//! `splitlane.json`. Exposes `resolve_frame_metrics` to turn the current font
...
pub(crate) const DEFAULT_FONT_SIZE: f32 = 10.0;
...
/// Used as Splitlane's primary UI family and as the `.SplitlaneSans`
...
/// Splitlane-side virtual font aliases. Mirror Zed's `.ZedMono` /
/// `.ZedSans` pattern from `crates/gpui/src/text_system.rs:1167-1173`,
/// but expanded at the Splitlane boundary (in `resolve_font_family`)
/// before the family name reaches GPUI - GPUI's pinned rev does not
/// know about Splitlane-specific aliases.
///
/// Users can write an alias (`".SplitlaneMono"`, `"JetBrainsMono NFM"`, or
/// `".SplitlaneSans"`) or a concrete embedded family in `splitlane.json`.
...
pub(crate) const SPLITLANE_MONO_ALIAS: &str = ".SplitlaneMono";
pub(crate) const SPLITLANE_SANS_ALIAS: &str = ".SplitlaneSans";

/// Resolve a Splitlane-virtual alias to its concrete embedded family.
...
/// Splitlane boundary before the family name reaches GPUI.
fn expand_splitlane_alias(name: &str) -> &str {
    match name {
        SPLITLANE_MONO_ALIAS | JETBRAINS_MONO_NFM_ALIAS => EMBEDDED_MONO_FAMILY,
        SPLITLANE_SANS_ALIAS => EMBEDDED_SANS_FAMILY,
        other => other,
    }
}

// Per-`Font` `fallbacks: Some(...)` was REMOVED on purpose. Splitlane
...
// `terminal.font_fallbacks` in their settings. Splitlane mirrors that
// pattern: `base_font` emits `Some(FontFallbacks)` ONLY when the user sets
// the top-level `font_fallbacks` array in `splitlane.json` (e.g. a Nerd
...
struct CachedFontConfig {
    ...
    /// Render ligatures when `true`. Hot-reload comes for free
    /// via the surrounding 500 ms cache: editing `splitlane.json` is
...
/// The default monospace family Splitlane uses out of the box.
///
...
/// `splitlane.json#font_family` - `resolve_font_family` validates the
...
pub(super) fn cached_font_config() -> (String, f32, f32, f32, FontWeight, bool, Option<Vec<String>>)
{
    ...
    // Ligatures are off by default to preserve the historical
    // monospaced cell-stride behavior. Opt-in via `terminal.ligatures: true`
    // in `splitlane.json`. Both `terminal == None` and
...
pub(super) fn base_font() -> Font {
    ...
    // When the user opts into ligatures, hand GPUI the font's
    // native feature set untouched. Default behavior (and explicit
    // `ligatures: false`) keeps the historical `disable_ligatures()`
    // call so a Splitlane upgrade never silently flips ligatures on.
...
/// Bounds shared by the global config validation, the
...
/// Validate a `font_size` read back from session.json
/// (UNTRUSTED-adjacent: local-only, but every value read back from the
/// session file is validated anyway). NaN/±inf are DROPPED (`None` - they would poison the cell
...
/// `size_override` is the per-pane zoom: `Some(pt)` wins over the
...
/// The global (non-overridden) font size in points - the zoom
...
pub fn resolve_frame_metrics(
    ...
    // (gpui/src/text_system.rs:148-160). The Splitlane log line
    // `font: resolved family='...'` reflects only what Splitlane
    ...
    // Snap raw font measurements to integer pixels via `.round()`
    ...
    // Acceptable for pixel-perfect rendering (glyph origins and cell
    // backgrounds both rely on integer strides).
    let cell_width = cell_width_raw.round();
    let line_height = line_height_raw.round();

    // SPLITLANE_PIXEL_PROBE: record both raw and snapped cell dimensions so a
...
mod tests {
    ...
    fn round_snap_no_op_for_integer_advance() {
        ...
        // observed while investigating block-character rendering:
        // cell_width=9.0), the snap must be a no-op so snapping introduces no
        // regression on
    ...
    fn round_snap_yields_integer_for_fractional_advance() {
        // 8.4 px advance is the canonical fractional case
    ...
    // session.json ingress for the per-pane zoom - NaN/inf dropped, finite
    // values clamped to [8.0, 32.0].
    #[test]
    ...
    fn default_cell_width_matches_windows_terminal_multiplier() {
        ...
        assert!(
            (raw.as_f32() - 8.0).abs() < 0.00001,
            "10pt x 0.6 should be 8px, got {}",
            raw.as_f32()
        );
        assert_eq!(raw.round(), px(8.0));
    }
    ...
    fn round_snap_yields_integer_for_fractional_line_height() {
        // 13 pt x 96/72 x 1.2 multiplier = 20.8 logical px. A literal size
        // rather than the default: the default (10 pt) lands on exactly 16px,
        // which has nothing left to snap.
        let raw_lh = font_points_to_pixels(13.0) * DEFAULT_LINE_HEIGHT;
        let snapped = raw_lh.round();
        assert_eq!(snapped, px(21.0));
        assert!(snapped.as_f32().fract().abs() < 1e-6);
    }

    // ─── Splitlane virtual-alias resolution ────────────────────────────
    // Lock in the contract that `.SplitlaneMono` and `.SplitlaneSans`
    // resolve to the embedded family names BEFORE leaving Splitlane.
    ...
    #[test]
    fn expand_splitlane_alias_resolves_mono_alias() {
        assert_eq!(
            expand_splitlane_alias(".SplitlaneMono"),
            EMBEDDED_MONO_FAMILY
        );
        assert_eq!(
            expand_splitlane_alias(".SplitlaneMono"),
        ...
        assert_eq!(
            expand_splitlane_alias("JetBrainsMono NFM"),
            EMBEDDED_MONO_FAMILY
        );
    }

    #[test]
    fn expand_splitlane_alias_resolves_sans_alias() {
        assert_eq!(
            expand_splitlane_alias(".SplitlaneSans"),
            EMBEDDED_SANS_FAMILY
        );
        assert_eq!(expand_splitlane_alias(".SplitlaneSans"), "Geist");
    }

    #[test]
    fn expand_splitlane_alias_passes_concrete_names_through() {
        ...
        assert_eq!(expand_splitlane_alias("Menlo"), "Menlo");
        assert_eq!(expand_splitlane_alias("Cascadia Mono"), "Cascadia Mono");
        assert_eq!(expand_splitlane_alias("Lilex"), "Lilex");
        assert_eq!(expand_splitlane_alias(""), "");
        // Case-sensitive: `.splitlanemono` is not `.SplitlaneMono`.
        assert_eq!(expand_splitlane_alias(".splitlanemono"), ".splitlanemono");
    }
    ...
    #[test]
    fn resolve_font_family_expands_splitlane_aliases() {
        ...
        assert_eq!(
            resolve_font_family(Some(".SplitlaneMono")),
            "JetBrainsMono Nerd Font Mono"
        );
        assert_eq!(
        ...
        assert_eq!(resolve_font_family(Some(".SplitlaneSans")), "Geist");
    }
    ...
    fn resolve_font_family_short_circuits_embedded_concrete_names() {
        ...
        // splitlane.json get the embedded font even on platforms whose
