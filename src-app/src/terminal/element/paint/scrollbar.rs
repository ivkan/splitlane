...
/// Match-rail tick height AND bucket size. One tick per
...
/// Project match positions (lines from the grid bottom)
...
/// Paint the search match rail - one [`TICK_PX`]-high quad
...
/// Resolved pixel geometry of the scrollbar strip + thumb for a single
...
pub fn paint_scrollbar(
    ...
    // thumb and the view's interactive hit-test never diverge.
...
mod tests {
    ...
    // Match-rail projection + decimation.
    ...
    fn ticks_are_decimated_to_one_per_bucket() {
        ...
        // track_height / TICK_PX = 200 ticks (paint is bounded by the
        // track, never the match count).
    ...
    // Top of track = fully scrolled back; bottom = at the live edge.
    #[test]
