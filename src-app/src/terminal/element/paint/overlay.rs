...
pub fn paint_exit_overlay(
    ...
    // Distinguish a signal kill (crash) from a clean non-zero exit.
...
/// `SPLITLANE_PIXEL_PROBE_OVERLAY=1` (independent of the log-only probe).
///
/// Uses the same `floor(x)`-shared-boundary math as the background pass so
/// the borders align with the underlying rects - any visible misalignment is a real
