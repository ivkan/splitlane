...
//! - `detect_urls_on_line_mapped`  - Zed-style URL regex.
//! - `detect_file_paths_on_line_mapped` - `.md` / `.markdown` paths with
//!   existence check + heuristics.
//!
...
pub fn detect_urls_on_line_mapped(
    ...
    re.find_iter(line_text)
        .filter_map(|m| {
            ...
            // Trim trailing punctuation / unbalanced close-parens the
...
/// free-form prose. Returns a sub-slice of the input (zero alloc).
///
...
/// Splitlane extends Zed with `! ?` (sentence ends) and `]` (Markdown link tails).
...
///
/// A **subset** of the regex above, and the difference is the point. This used
/// to say "everything the regex captures, since the OS handler knows whether a
/// scheme is registered" - true, and the OS's way of saying *no* is a modal
/// list of every application on the machine, naming neither the link nor the
/// reason. We cannot intercept that panel or explain it, so the decision has to
/// be made before the link is armed.
///
/// The judgement is asymmetric, which is what settles it: a scheme we decline
/// to open is still text on screen the reader can select and copy, while a
/// scheme we open and the desktop cannot costs them a modal they can only
/// cancel. So the allowlist holds the schemes a desktop registers *to show
/// somebody something*, and excludes the ones that are a **transport for a
/// tool**: `git://` is the git wire protocol, consumed by `git clone` and
/// registered by nothing, and `ipfs:` / `ipns:` need a gateway app that a stock
/// machine does not have. Measured on macOS 26 via `lsregister -dump`: those
/// three had **no** handler, every other scheme here had at least two.
///
/// `file://` is intentionally excluded too. Local files must go through the
/// canonicalized file/code scanners instead of OS scheme dispatch.
...
/// Python traceback frame `File "path", line N`. The path is quoted and
...
fn split_path_and_location(matched: &str) -> (&str, Option<u32>, Option<u32>) {
    // Paren-location form `file(42,7)` / `file:(12,3)` (tsc, C#,
...
pub fn detect_code_paths_on_line_mapped(
    ...
    // Python traceback frames (`File "x.py", line N`) first, so they
...
mod tests {
    ...
    // ── URL trailing-punctuation / unbalanced-paren trimming ────────────────
    ...
    #[test]
    fn a_transport_scheme_is_detected_but_not_armed() {
        // The scanner still finds these - they read as links and the text is
        // selectable - but clicking one would hand the desktop a scheme it does
        // not register, and the OS answers that with a modal list of every
        // application, naming neither the link nor the reason.
        for uri in [
            "git://github.com/ivkan/splitlane.git",
            "ipfs://bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi",
            "ipns://en.wikipedia-on-ipfs.org",
        ] {
            let map = ascii_map(uri);
            let zones = detect_urls_on_line_mapped(uri, line0(), &map);
            assert_eq!(zones.len(), 1, "{uri} should still be detected");
            assert!(!zones[0].is_openable, "{uri} must not be armed");
            assert!(!is_url_scheme_openable(uri));
        }

        // The rest of the allowlist is unchanged.
        for uri in [
            "https://example.com",
            "http://example.com",
            ...
            assert!(is_url_scheme_openable(uri), "{uri} should stay openable");
        }
    }
    ...
    fn perf_scan_200_lines_under_budget() {
        ...
        // workload, so we relax to 15 ms in release without
    ...
    fn split_location_non_numeric_paren_is_not_a_location() {
        // Adversarial: `foo.rs(copy)` must NOT yield a false line/col;
    ...
    fn code_path_scanner_matches_paren_location() {
        // End-to-end: tsc-style `app.ts(42,7)`.
        let tmp = tempfile::tempdir().expect("tempdir");
        ...
        assert!(
            ...
            "tsc paren-location must resolve line+col; got {zones:?} zones",
            zones = zones.len()
        );
    }
    ...
    fn code_path_scanner_matches_python_traceback() {
        // `File "main.py", line 10` → line 10 on the quoted path.
        let tmp = tempfile::tempdir().expect("tempdir");
        ...
        assert!(
            ...
            "Python traceback frame must resolve the line number"
        );
    }

    #[test]
    fn code_path_scanner_still_matches_update_paren_wrap() {
        // Regression: Claude-Code `Update(src/cool.rs)` must still match
        ...
        assert!(
            ...
            "Update(path) must still match the inner path with no location"
        );
    }
