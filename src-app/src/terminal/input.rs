...
/// for pasting into a PTY. `None` when every path is filtered out
...
/// PowerShell-compatible single-quote form because Splitlane's default Windows
...
impl TerminalView {
    ...
    /// If `x` falls on the (widened) scrollbar strip and there is
    ...
    /// Scroll the grid so `target_offset` scrollback lines sit above the
    ...
    pub(super) fn handle_mouse_down(
        ...
        // A Left press on the scrollbar strip starts a jump/drag and
        ...
        // Cmd/Ctrl+Left-click on a link: DEFER the open to mouse-up.
    ...
    pub(super) fn handle_mouse_move(
        ...
        // While dragging the scrollbar, map the pixel delta since the
        ...
        // Track hovered cell for URL regex detection.
        ...
        // OSC 8 takes priority over regex URL detection,
        ...
        if open_link_modifier_held(&event.modifiers) {
            ...
            // NOT rescan on sub-cell jitter (no per-event scanning).
    ...
    /// Resolve the hyperlink under `hover_point` through the
    ...
    pub(super) fn handle_modifiers_changed(
        ...
        // GPUI fires no MouseMove when only a modifier changes, so link
    ...
    /// Open a resolved hyperlink. `.md` routes to the in-pane markdown
    ...
    pub(super) fn handle_mouse_up(
        ...
        // End a scrollbar drag on the LEFT release without running the
        ...
        // Shift overrides mouse mode for text selection.
        if mode.intersects(Modes::MOUSE_MODE) && !event.modifiers.shift {
            // A Ctrl-press may have stashed a pending link on mouse-down
        ...
        // A Ctrl/Cmd-click stashed the link under the press. It opens
        ...
        // Open on a genuine click (empty selection = no drag).
        if selection_empty
    ...
    pub(super) fn handle_paste(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        // File(s) copied in the OS file manager (Nautilus/Finder/
    ...
    /// Deliver an automation / agent payload while NEVER synthesizing a submit
    /// out of the body. When the target has bracketed paste active, the bytes
    /// are wrapped (embedded newlines stay literal inside the agent's editor);
    /// when it does NOT, they are written VERBATIM - crucially not through the
    /// interactive `\n` -> `\r` rewrite that `write_paste_text` applies, which
    /// would turn a multi-line prompt into N carriage returns and defeat the
    /// single, SEPARATE deferred `\r` that is the only sanctioned submission
    /// (the human-in-the-loop invariant: a payload never submits itself). This
    /// is the divergence from `paste_text`: a human pressing Ctrl+V into a bare shell still wants newline -> run, but a `send_text`
    ...
    pub(super) fn handle_scroll_wheel(
        ...
        // Reset the sub-line accumulator on gesture start so an
        ...
        // Scroll-sensitivity multiplier, cached on the view at
    ...
    pub(super) fn handle_scroll_page_up(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        // In an alt-screen TUI (lazygit, less, vim, k9s, full-screen
    ...
    pub(super) fn handle_scroll_page_down(&mut self, _window: &mut Window, cx: &mut Context<Self>) {
        // See handle_scroll_page_up. `\x1b[6~` is plain PageDown.
        let alt_screen = self
            .terminal
            .session_backend()
            .modes()
...
mod tests {
    ...
    fn printable_altgr_commit_preserves_key_metadata_and_consumes_ctrl_alt() {
        ...
        let input = super::ghostty_text_key_input(
            &keystroke,
            splitlane_terminal_ghostty::KeyAction::Press,
            true,
            "@",
        );

        assert_eq!(input.key, splitlane_terminal_ghostty::Key::Character('0'));
        assert!(input.modifiers.contains(
            splitlane_terminal_ghostty::Modifiers::CONTROL
                | splitlane_terminal_ghostty::Modifiers::ALT
        ));
    ...
    // Shell-quoting of file-manager paths for paste.
    #[test]
