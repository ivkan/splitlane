...
use splitlane_config::schema::{TerminalBackendConfig, TerminalConfig, TerminalSurfaceProfile};
...
use super::types::{
    Column, CopyModeCursorState, CursorShape, HyperlinkZone, Line, Modes, Point, SearchHighlight,
    ShellQuoting, TerminalWindowSize, terminal_metric_to_u16,
};
...
/// The deferred half of a surface's construction - shell resolution, the
/// PTY fork and (on the relevant platforms) the libghostty start, packed into a
/// thunk that runs when the surface is first shown. Boxing keeps the captured
/// state (including the cfg-gated ghostty handles) out of the view's type.
type DeferredBackendStart = Box<dyn FnOnce(&mut Context<TerminalView>)>;
...
fn warn_ghostty_unavailable_once() {
    ...
    WARNED.call_once(|| {
        log::warn!(
            target: "splitlane::terminal::backend",
...
/// Check once whether SPLITLANE_LATENCY_PROBE=1 is set.
/// Cached in a OnceLock so the env var is read only on first call.
#[cfg(debug_assertions)]
pub(crate) fn probe_enabled() -> bool {
    static ENABLED: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ENABLED.get_or_init(|| std::env::var("SPLITLANE_LATENCY_PROBE").as_deref() == Ok("1"))
}

/// Human-readable in-pane message for a failed PTY spawn - written into the
/// display-only placeholder kept by the background-spawn path (and the
...
fn renderer_cursor_shape_from_config(
    shape: splitlane_config::schema::CursorShapeConfig,
) -> CursorShape {
    use splitlane_config::schema::CursorShapeConfig as C;
    match shape {
...
pub(crate) fn hsla_from_hex_color(raw: &str) -> Option<Hsla> {
    let normalized = splitlane_config::schema::normalize_hex_color(raw)?;
...
// Cursor blink interval moved to `terminal::blink::CURSOR_BLINK_INTERVAL`.
...
/// Stable authority for an in-progress scrollbar drag. Geometry is
...
pub struct TerminalView {
    ...
    /// Painted scrollbar geometry - set by TerminalElement::paint(),
    ...
    /// Active scrollbar drag, or `None`. Holds the cursor Y and the
    ...
    /// The find bar's `Aa` toggle. Off by default, which is what a terminal
    /// search should be: the user is looking for a word in output they did not
    /// write, and its case is the program's choice, not theirs.
    ...
    /// Cursor blink override (On / Off / TerminalControlled). Read
    /// from config at construction.
    pub(super) cursor_blink_mode: splitlane_config::schema::CursorBlinkConfig,
    /// Default cursor shape used before applications override it through
    /// DECSCUSR. Custom shapes are painted by Splitlane.
    ...
    /// Resolved scroll-wheel multiplier for scrollback (1.0 = default).
    ...
    /// Renderer switch: block elements use Splitlane's built-in quad renderer
    ...
    pub(super) ghostty_pressed_keys:
        std::collections::HashMap<String, splitlane_terminal_ghostty::KeyInput>,
    ...
    pub(super) ghostty_pending_text_key:
        Option<(gpui::Keystroke, splitlane_terminal_ghostty::KeyAction, bool)>,
    /// Last hovered cell position for URL regex detection.
    ...
    /// The link under the cursor at modifier+mouse-down. The open is
    ...
    /// Backend start held until this surface is first shown. `None` once
    /// started - and for a display-only test view, which never starts one -
    /// which is what makes [`Self::ensure_backend_started`] idempotent.
    deferred_backend_start: Option<DeferredBackendStart>,
    /// The agent surface this PTY belongs to, when it is one: the
    /// [`crate::project::Thread::id`] whose `SessionBinding` produced the
    /// process running here. `None` for a plain shell.
    ///
    /// It lives on the view rather than on the pane holding it because an
    /// agent surface can move: into a slot, between slots, between panes by
    /// drag. Anything keyed on the pane would have to be carried along by
    /// every one of those moves, and forgetting one fails silently: the
    /// surface keeps running and simply stops being an agent surface, so its
    ...
    ///
    /// Every terminal in a pane now has a record, so `agent_thread_id` no
    /// longer separates the two - and `Pane::terminal_tab_title` used to ask
    /// exactly that question to decide whether to name a surface by its rail
    /// row or by its shell prompt. Asking it of a shell renamed the shell.
    pub surface_is_agent: bool,
    /// The name this surface's record carries - what its rail row shows.
    ///
    /// The floor under the slot header's name, used only while the process has
    /// reported no title of its own. A freshly mounted agent has not: its PTY
    /// still says the default "Terminal", so the header read "Terminal" while
    /// the rail read the session's name, in the same frame - and the OSC sync
    /// that would have reconciled them skips exactly the word "Terminal",
    /// because that is also what alacritty stamps on a `ResetTitle`. Once the
    /// CLI names itself its own title wins: the header follows the process.
    ///
    /// On the view rather than pushed with the other slot-header facts because
    /// it is the surface's identity and has to travel with the process when
    /// the surface moves between panes.
    pub record_title: Option<String>,
}

/// How many grid rows the last-command harvest will read. A command that wraps
/// past this is a runaway, and a 34px header is not where it should be shown in
/// full.
const LAST_COMMAND_MAX_ROWS: i64 = 4;

/// Byte offset of the `nth` char in `text`, or its end. The grid harvest works
/// in columns and the text it produced is a `String`, so the two have to be
/// bridged somewhere; doing it here keeps the slicing panic-free on a prompt
/// with multi-byte characters in it.
fn char_byte_offset(text: &str, nth: usize) -> usize {
    text.char_indices()
        .nth(nth)
...
impl TerminalView {
    /// Bind this view to a container surface record.
    ///
    /// One call, both fields. `agent_thread_id` says *which* record, and
    /// `surface_is_agent` says whether the record is an agent's - and
    /// `Pane::terminal_tab_title` reads the second to decide how to name the
    /// surface. Setting one without the other renames it: an agent that got
    /// only the id would be named by its shell prompt, and a shell that got
    /// only the id used to be named by its rail row. There is no way to set
    /// them apart now.
    pub fn bind_surface(&mut self, thread_id: u64, is_agent: bool) {
        self.agent_thread_id = Some(thread_id);
        self.surface_is_agent = is_agent;
    }
    ...
    /// Spawn a terminal with an explicit per-surface env map. The
    ...
    pub fn with_cwd_env_and_profile(
        ...
        // Paint immediately. Phase 1 - build a display-only placeholder
        // on the render thread. Phase 2 - resolve the spawn params and open the
        // PTY on the background executor, then `promote()` the placeholder in
        // place, so an N-pane restore never serializes N blocking spawns on the
        // main thread.
        //
        // Phase 2 no longer starts here. It is packed into
        // `deferred_backend_start` and runs when the view is first shown (see
        // [`Self::ensure_backend_started`]). A restored surface is a
        // *description* until it reaches the screen: no child process, no shell
        // resolution and no shell-integration file write for surfaces the user
        // never opens. This is what makes lazy mount the general rule instead of
        // an Agents-only exception.
        let (cols, rows) = initial_size.unwrap_or(TerminalState::DEFAULT_GRID_SIZE);
        // The real quoting is that of the shell resolved at mount; until then a
        // platform default keeps drag-and-drop path quoting sane. The deferred
        // start overwrites it the moment the params are known.
        ...
        // A surface that has not mounted yet has no live shell to report a
        // cwd, and both session save (`layout::serde`) and `surface.list` read
        // it from here. Seed it with the directory the shell *will* start in, so
        // a surface the user never opens keeps its own cwd across a restart
        // instead of silently collapsing onto the workspace root. `promote`
        // overwrites it with the spawned child's real cwd at mount.
        if let Some(cwd) = cwd.as_ref() {
            terminal.current_cwd = Some(cwd.to_string_lossy().into_owned());
        }
        let requested_backend = splitlane_config::loader::load_config()
            .terminal
            .unwrap_or_default()
            .backend;
        ...
        let ghostty_spawn = if should_start_ghostty(requested_backend) {
            let initial_window_size = TerminalWindowSize::new(cols, rows, 0, 0);
        ...
        let mut view = Self::from_terminal_state(workspace_id, terminal, cx);

        view.deferred_backend_start = Some(Box::new(move |cx: &mut Context<Self>| {
            // Capture the foreground signal mask on the MAIN thread so the
            // background-spawned child still gets correct Ctrl-C / Ctrl-Z
            // `ensure_backend_started` only ever runs on the main
            // thread (render, or an IPC call already hopped onto it).
            ...
            cx.spawn(
                async move |this: gpui::WeakEntity<Self>, cx: &mut gpui::AsyncApp| {
                    ...
                    // and so does param resolution (shell lookup + shell-integration
                    // rc write).
                    let (shell_quoting, outcome) = executor
                        .spawn(async move {
                            ...
                            let shell_quoting = params.shell_quoting;
                            #[cfg(any(
                                all(target_os = "linux", feature = "libghostty-linux"),
                                all(
                                    target_os = "windows",
                            ...
                            let outcome = if let Some((ghostty, runtime_pending)) = ghostty_spawn {
                                let max_scrollback = splitlane_config::loader::load_config()
                                    .terminal
                                    .unwrap_or_default()
                            ...
                            let outcome = BackgroundSpawnOutcome::Alacritty(
                                TerminalState::open_pty_and_eventloop(params, pending, signal_mask),
                            );
                            (shell_quoting, outcome)
                        })
                        .await;
                    let _ = this.update(cx, |view, cx| {
                        // The placeholder was built before the shell was known
                        // (the backend start is deferred): adopt the resolved shell's quoting now.
                        view.terminal.shell_quoting = shell_quoting;
                        match outcome {
    ...
    /// Open this surface's backend, once, on first show.
    ///
    /// A session restores every surface as a description; the child process is
    /// forked here - from [`Render::render`], i.e. the moment the surface
    /// actually reaches the screen, or from an explicit caller (an IPC write)
    /// that needs it live before then. Idempotent: the thunk is taken on the
    /// first call, so re-showing a surface never re-forks a shell and never
    /// re-runs an agent's launch command.
    pub(crate) fn ensure_backend_started(&mut self, cx: &mut Context<Self>) {
        let Some(start) = self.deferred_backend_start.take() else {
            return;
        };
        start(cx);
    }
    ...
    fn from_terminal_state(
                ...
                loop {
                    ...
                    let result = cx.update(|cx| {
                        this.update(cx, |view: &mut Self, cx: &mut Context<Self>| {
                            ...
                            // Close only on a user-initiated or clean
        ...
        // Subscribe to the app-scoped `BlinkPhase` so this terminal's
        ...
        // constructs a TerminalView outside `SplitlaneApp::new` (test
...
impl TerminalView {
    ...
    pub fn commit_text(&mut self, text: &str, _cx: &mut Context<Self>) {
        ...
        ))]
        {
            ...
            let input = pending
                ...
                .unwrap_or_else(|| splitlane_terminal_ghostty::KeyInput {
                    key: splitlane_terminal_ghostty::Key::Unidentified,
                    action: splitlane_terminal_ghostty::KeyAction::Press,
                    modifiers: splitlane_terminal_ghostty::Modifiers::empty(),
                    consumed_modifiers: splitlane_terminal_ghostty::Modifiers::empty(),
                    text: text.to_string(),
                    unshifted_codepoint: None,
                    composing: false,
                });
            let mut release = input.clone();
            release.action = splitlane_terminal_ghostty::KeyAction::Release;
            release.text.clear();
    ...
    /// The last command this shell ran, for the slot header's "↻ rerun".
    ///
    /// Read out of the grid between the OSC 133 `CommandStart` and
    /// `OutputStart` marks: the shell integration prints `CommandStart` at the
    /// end of its prompt, so that mark's line AND column are exactly where the
    /// command text begins, and `OutputStart` is printed the moment the user
    /// submits. Harvesting between them gives the command the shell actually
    /// ran, prompt excluded - which is the difference between an affordance
    /// that can name what it will do and one that asks the user to trust it.
    ///
    /// `None` without shell integration, which is the honest answer: with no
    /// marks there is no way to know what the last command was, and a "rerun"
    /// button that sends the shell a blind history recall would be offering to
    /// run something it cannot name.
    pub fn last_command(&self) -> Option<gpui::SharedString> {
        ...
        let (at_prompt, span) = {
            ...
            (marks.at_prompt(), marks.last_command_span())
        };
        // Sending text into a running program is not a rerun, so the
        // affordance is withheld until the shell is ready for one.
        if !at_prompt {
            return None;
        }
        // What the shell itself reported (`133;E`), which is what Splitlane's
        // own integration sends. It beats the grid harvest below in every way
        // that matters: no prompt to strip, no wrapping to reassemble, and the
        // text is what the shell was handed rather than what the screen shows.
        if let Some(reported) = self
            ...
            .clone()
        {
            return Some(gpui::SharedString::from(reported));
        }
        // Fallback for a shell integration that is not ours: one that marks
        // the end of its prompt with `133;B` puts that mark's line AND column
        // exactly where the command text begins, so the span up to
        // `OutputStart` is the command, prompt excluded.
        let (start, end) = span?;
        let metrics = backend.grid_metrics();
        let history_size = i64::from(metrics.topmost_line.0.saturating_neg());
        let mut text = String::new();
        // A command can wrap, but only a runaway one wraps far. The cap keeps a
        // pathological line out of a 34px header without truncating anything a
        // person actually typed.
        for abs_line in start.abs_line..end.abs_line.min(start.abs_line + LAST_COMMAND_MAX_ROWS) {
            let line = Line((abs_line - history_size) as i32);
            let Some(row) = backend.line_text_at(Point {
                line,
                column: Column(0),
            }) else {
                break;
            };
            // The first row starts where the prompt ended; the rest are wraps
            // and start at column zero.
            let from = if abs_line == start.abs_line {
                start.column
            } else {
                0
            };
            let Some(slice) = row.text.get(char_byte_offset(&row.text, from)..) else {
                break;
            };
            text.push_str(slice.trim_end());
        }
        crate::terminal::marks::sanitize_command_line(text.trim().as_bytes())
            .map(gpui::SharedString::from)
    }

    /// Put a command on the shell's input line WITHOUT submitting it, the way
    /// a paste would.
    ///
    /// The half of "↻ rerun" for a command the header could not show in full.
    /// A middle-elided label - `cargo test …pane_header` - is not something a
    /// user can consent to running: the elision is exactly where a second
    /// command would hide. So the click types it instead, the shell's own line
    /// shows it whole, and the Enter is the user's.
    ///
    /// Goes through the paste path rather than a raw write so bracketed paste
    /// is honoured on both VT backends: inside brackets the shell treats the
    /// text as text, which is the difference between offering a command and
    /// running one.
    pub fn type_command(&self, command: &str) {
        let mode = self.terminal.session_backend().modes();
        self.write_paste_text(command, mode);
    }
            ...
            if sequence_would_submit(&seq) {
                return Err(format!(
                    "keystroke '{keystroke_str}' would submit (CR/LF); use \
                     surface.send_text with submit=true (`splitlane send --submit`) instead"
                ));
            }
...
/// PTY. Single choke point for the `send_keystroke` refusal above - pure so
...
impl TerminalView {
    ...
    /// point. Mirrors `detect_url_at_hover`: extracts line text with
...
/// Pane subscribes for ChildExited/TitleChanged; SplitlaneApp subscribes
...
pub enum TerminalEvent {
    ...
    /// Consumed by `SplitlaneApp` to surface a "Copied" toast.
    SelectionCopied,
    /// Cmd/Ctrl-click on a `.md`/`.markdown` path detected by the
    /// file-path scanner. The receiver (SplitlaneApp) splits the
    ...
    /// (SplitlaneApp) resolves the user's preferred editor via the
    ...
    /// The per-pane font override changed. The receiver
    /// (SplitlaneApp) persists the session so the zoom survives a crash,
    /// not just a clean quit (same rationale as `SurfaceRenamed`).
    FontZoomChanged,
    /// The user widened this find bar to every session. The
    /// receiver (`SplitlaneApp`) opens the palette's "Output" tab with the
    /// query, which is where the fan-out lives since the design dissolved
    /// fleet search into the palette.
    ///
    /// No regex flag. The palette has one query box and it also filters names
    /// and projects, so the scan behind that tab is a literal substring match
    /// and a stray `(` must not turn it into an error.
    FleetSearchRequested { query: String },
}
...
impl TerminalView {
    ...
    /// The find bar: a strip across the top of the pane's body, per the
    /// design's "Find bar (per pane, ⌘F)".
    ///
    /// Drawn as an overlay rather than as a row above the terminal. A row would
    /// take 30px of height from the grid, and taking height from a grid means
    /// reflowing the PTY - every fullscreen TUI in the app would repaint on
    /// ⌘F, and repaint again on Esc. The bar covers the top line of scrollback
    /// instead, which is the line the user is least likely to be reading while
    /// typing a query.
    fn render_search_overlay(&self, cx: &mut Context<Self>) -> gpui::AnyElement {
        use gpui::{FontWeight, Hsla, MouseButton, px, svg};

        // Themed chrome, not hardcoded hexes - the bar has to hold in all six
        // themes and under the hot-reload watcher.
        ...
        // The design's counter reads "3 of 12" - a sentence, not a fraction.
        let (status_text, status_color) = if has_regex_error {
            ...
            (format!("{current_match} of {match_count}"), ui.muted)
        };

        // Real input entity (cursor, selection, IME, clipboard) - the same
        // widget the rail uses, focused on open. The caret and "Search"
        // placeholder are painted by the widget itself; we only own the
        // wrapper box (width + inherited face, size and colour).
        let field = div()
            .id("search-field")
            .flex()
            .flex_1()
        ...
        // `Aa` and `.*`: one chip shape, two states. Inactive is a hairline
        // over nothing; active is the accent's own surface and border, which is
        // the one place in the bar that carries colour.
        let toggle = |id: &'static str, label: &'static str, active: bool| {
            div()
                .id(id)
                .flex()
                .flex_none()
                ...
                .text_color(if active { ui.accent } else { ui.muted })
                .child(label)
        };

        let case_toggle = toggle("search-case-toggle", "Aa", case_active)
            .tooltip(crate::ui_primitives::text_tooltip("Match case"))
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|this, _, _window, cx| this.toggle_search_case(cx)),
            );
        let regex_toggle = toggle("search-regex-toggle", ".*", regex_active)
            .tooltip(crate::ui_primitives::text_tooltip("Regular expression"))
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|this, _, _window, cx| this.toggle_search_regex(cx)),
            );

        // Widen the query to every open session. The clickable
        // counterpart of the remappable `toggle_fleet_search` action; both open
        // the palette's "Output" tab, which is where the fan-out lives.
        ...
        let prev_btn = icon_btn("search-prev", "icons/chevron_up.svg", nav_color)
            .tooltip(crate::ui_primitives::text_tooltip("Previous match (⇧⏎)"))
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|this, _, _window, cx| this.search_prev(cx)),
            );
        let next_btn = icon_btn("search-next", "icons/chevron_down.svg", nav_color)
            .tooltip(crate::ui_primitives::text_tooltip("Next match (⏎)"))
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|this, _, _window, cx| this.search_next(cx)),
            );
        // Esc closes it, and the design draws no close box. This one is here
        // for the hand already on the mouse; it says the same thing Esc does.
        let close_btn = icon_btn("search-close", "icons/close.svg", ui.muted)
            .tooltip(crate::ui_primitives::text_tooltip("Close (Esc)"))
            .on_mouse_down(
                MouseButton::Left,
        ...
        div()
            ...
            .child(
                svg()
                    ...
                    // The one accent glyph the design puts in this bar.
        ...
        if reports_focus {
            ...
            let ghostty_encoded = self
                ...
                .write_ghostty_focus(if focused {
                    splitlane_terminal_ghostty::FocusEvent::Gained
                } else {
                    splitlane_terminal_ghostty::FocusEvent::Lost
                })
                .is_handled();
            #[cfg(not(any(
                all(target_os = "linux", feature = "libghostty-linux"),
...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        // First show is the mount. Reaching `render` is the one signal that
        // means "this surface is on screen" for every container - a CLI pane in
        // the active workspace, a selected tab, an Agents thread, a bottom-dock
        // terminal - so a restored session pays for the surfaces the user looks
        // at and nothing else.
        self.ensure_backend_started(cx);
        ...
        let mut el = div()
            ...
            // Hand cursor over a hovered link, text IBeam otherwise -
            ...
            // Reveal/clear a link the instant Ctrl/Cmd is pressed or
            ...
            .on_action(
                cx.listener(|this, _: &crate::ToggleSearchCase, _window, cx| {
            ...
            // Per-pane font zoom (±1 pt, clamp [8, 32]).
            .on_action(
            ...
            // Fan the current query out to the whole fleet.
            .on_action(
...
/// Decide cursor visibility for one blink tick. `On` always blinks
...
fn resolve_cursor_visible(
    mode: splitlane_config::schema::CursorBlinkConfig,
    decscusr_blinking: bool,
    phase_visible: bool,
) -> bool {
    use splitlane_config::schema::CursorBlinkConfig as M;
    match mode {
        M::On => phase_visible,
        M::Off => true,
...
mod tests {
    ...
    /// Constructing a surface must not start anything. Session restore
    /// builds one `TerminalView` per persisted surface, and the child process
    /// is forked only when the surface is first shown, so a restored session
    /// costs zero shells for panes nobody opened. The assertion lives inside
    /// the constructor closure on purpose: reading the flag back would say
    /// nothing about whether a spawn was already in flight.
    #[gpui::test]
    fn constructing_a_surface_defers_its_backend_start(cx: &mut TestAppContext) {
        let cx = cx.add_empty_window();
        let _view = cx.new(|cx| {
            let view = TerminalView::with_cwd(1, None, None, cx);
            assert!(
                view.deferred_backend_start.is_some(),
                "a freshly built surface must hold its backend start until first show"
            );
            view
        });
    }
    ...
    // --- send_keystroke submission guard ---
    ...
    // --- extract_scrollback / restore_scrollback tests ---

    #[test]
    fn scrollback_round_trip() {
        // The mockable PtyBackend is gone; a display-only
    ...
    fn cursor_blink_override_resolves_correctly() {
        use splitlane_config::schema::CursorBlinkConfig as M;
        // On always blinks (follows phase), ignoring DECSCUSR.
