//! Config file I/O: read-modify-write helpers for `splitlane.json`.
//!
...
/// Serialize every read-modify-write of `splitlane.json`.
///
...
/// cannot overwrite the user's recoverable `splitlane.json` with `{}`.
fn load_raw_config(path: &Path) -> Result<serde_json::Value, ()> {
    match splitlane_config::loader::read_config_string(path) {
        splitlane_config::loader::ConfigRead::Absent => Ok(serde_json::json!({})),
        splitlane_config::loader::ConfigRead::Rejected => Err(()),
        splitlane_config::loader::ConfigRead::Contents(contents) => {
...
fn write_config_checked(path: &PathBuf, value: &serde_json::Value) -> bool {
    ...
    // Write atomically (tmp + rename) so a crash mid-write can't
    // truncate `splitlane.json`. A truncated file parses as invalid JSON, which
...
/// telemetry consent modal, which must honor the choice
...
pub fn save_config_values_checked<const N: usize>(values: [(&str, serde_json::Value); N]) -> bool {
    let Some(path) = splitlane_config::loader::config_path() else {
...
/// (`new_key` now belongs to `action_name`, so its previous owner
...
/// Save a single shortcut override to `splitlane.json`.
///
...
/// Remove all user shortcut overrides from `splitlane.json`, restoring defaults.
...
/// Return a copy of `config` with a single field updated *in memory*,
...
) -> splitlane_config::schema::SplitlaneConfig {
...
/// Save a single field inside the `"terminal": { ... }` block in `splitlane.json`
/// (the Terminal settings tab). A `Null` value removes the key (restoring
...
/// `splitlane.json`, preserving sibling agent-panel settings and unknown fields.
...
// `with_commands` and `save_commands_checked` stood here. `commands[]` was the
// presets' store, and presets are files now: nothing in this build writes the
// key. It is still *read*, once, by the migration that copies its entries into
// the presets folder - which is why it is read-only rather than gone.
...
mod tests {
    ...
    fn write_config_is_atomic_and_leaves_no_temp() {
        // The write goes through tmp+rename, the target ends up with
        // the full content, and no temp file is left behind in the directory.
        let dir = tempfile::TempDir::new().unwrap();
        let p = dir.path().join("splitlane.json");
    ...
    fn load_raw_config_rejects_non_object_roots() {
        ...
        assert!(
            load_raw_config(&p).is_err(),
            "a valid JSON non-object is not a writable splitlane config root"
        );
    }
    ...
    fn merge_shortcut_collision_evicts_other_action() {
        // Binding a key already owned by another action must evict the
