//! A focused multi-line text area for
//! the Agents Composer.
//!
...
// not exercised by the Composer but are intentionally part of
// the widget API so other callers (settings flows, an attachment
// composer, an edit-message fork) can reuse the
...
actions!(
    ...
    [
        ...
        // is a no-op (no default behaviour to swallow). The trigger
        // character (`@` / `/`) must stay in the buffer after dismissal,
        // which falls out naturally
...
/// `SplitlaneTextArea` key context so it does not conflict with
/// `TextInput` bindings on the single-line widget.
pub fn register_keybindings(cx: &mut App) {
    cx.bind_keys([
        KeyBinding::new("backspace", TaBackspace, Some("SplitlaneTextArea")),
        KeyBinding::new("delete", TaDelete, Some("SplitlaneTextArea")),
        KeyBinding::new("left", TaLeft, Some("SplitlaneTextArea")),
        KeyBinding::new("right", TaRight, Some("SplitlaneTextArea")),
        KeyBinding::new("up", TaUp, Some("SplitlaneTextArea")),
        KeyBinding::new("down", TaDown, Some("SplitlaneTextArea")),
        KeyBinding::new("shift-left", TaSelectLeft, Some("SplitlaneTextArea")),
        KeyBinding::new("shift-right", TaSelectRight, Some("SplitlaneTextArea")),
        KeyBinding::new("shift-up", TaSelectUp, Some("SplitlaneTextArea")),
        KeyBinding::new("shift-down", TaSelectDown, Some("SplitlaneTextArea")),
        KeyBinding::new("home", TaHome, Some("SplitlaneTextArea")),
        KeyBinding::new("end", TaEnd, Some("SplitlaneTextArea")),
        // Shift+Enter for a literal newline; plain Enter
        // fires `TaSubmit` which the Composer interprets as "send".
        KeyBinding::new("enter", TaSubmit, Some("SplitlaneTextArea")),
        KeyBinding::new("shift-enter", TaInsertNewline, Some("SplitlaneTextArea")),
        // Escape dismisses the Composer's popups via the
        // registered `on_escape` callback (no-op when none is set).
        KeyBinding::new("escape", TaEscape, Some("SplitlaneTextArea")),
        // The Composer's explicit
        ...
        KeyBinding::new(
            "secondary-enter",
            TaSubmitImmediate,
            Some("SplitlaneTextArea"),
        ),
    ]);
    #[cfg(target_os = "macos")]
    cx.bind_keys([
        KeyBinding::new("cmd-a", TaSelectAll, Some("SplitlaneTextArea")),
        KeyBinding::new("cmd-c", TaCopy, Some("SplitlaneTextArea")),
        KeyBinding::new("cmd-v", TaPaste, Some("SplitlaneTextArea")),
        KeyBinding::new("cmd-x", TaCut, Some("SplitlaneTextArea")),
        // Bypass the queue and send immediately.
        KeyBinding::new(
            "cmd-shift-enter",
            TaSubmitImmediate,
            Some("SplitlaneTextArea"),
    ...
    cx.bind_keys([
        KeyBinding::new("ctrl-a", TaSelectAll, Some("SplitlaneTextArea")),
        KeyBinding::new("ctrl-c", TaCopy, Some("SplitlaneTextArea")),
        KeyBinding::new("ctrl-v", TaPaste, Some("SplitlaneTextArea")),
        ...
        KeyBinding::new("ctrl-shift-v", TaPaste, Some("SplitlaneTextArea")),
        KeyBinding::new("ctrl-x", TaCut, Some("SplitlaneTextArea")),
        // Bypass the queue and send immediately.
        KeyBinding::new(
            "ctrl-shift-enter",
            TaSubmitImmediate,
...
/// Callback fired after any mutation that changes the content
...
/// Callback fired on Escape. Boxed for the same reason as
...
/// Callback fired on Ctrl+Shift+Enter (Cmd+Shift+Enter on
...
///
/// The decoration shape is intentionally minimal in this first cut: a
...
/// serialization can recover the literal `@path` token
...
/// Atomic cursor + selection over decorations was a follow-up cut.
#[derive(Debug, Clone)]
...
pub struct TextArea {
    ...
    /// Launch Pad: when `true`, Enter fires `on_submit` even on an
    ...
    /// Inline chip decorations. Rendered as paint-pass
...
impl TextArea {
    ...
    /// Install a callback that fires after every content
    ...
    /// Install an Escape callback. Bound via `KeyBinding::new
    /// ("escape", TaEscape, "SplitlaneTextArea")` so it fires only
    ...
    /// Install a callback that fires on Ctrl+Shift+Enter
    ...
    /// Current cursor byte offset. The Composer reads this to
    /// scan backwards for an active `@<query>` / `/<query>` token.
    pub fn cursor_offset(&self) -> usize {
        self.cursor()
    }

    /// Replace the bytes in `range` with `replacement` and
    ...
    pub fn replace_range(
        ...
        self.selection_reversed = false;
        self.marked_range = None;
        // `replace_selection` already ends with `fire_change` (the two
    ...
    /// Rebuild the decoration list after an edit that
    ...
    /// Dispatch to the registered `on_change` callback, if
    ...
    fn backspace(&mut self, _: &TaBackspace, _w: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            // When the cursor sits flush against the
    ...
    fn delete(&mut self, _: &TaDelete, _w: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            // Symmetrical to backspace: when the cursor is at
    ...
    fn left(&mut self, _: &TaLeft, _w: &mut Window, cx: &mut Context<Self>) {
        if self.selected_range.is_empty() {
            let prev = prev_grapheme(&self.content, self.cursor());
            // Chips are not character-traversable. When
    ...
    /// Thin instance wrapper around the pure
    ...
    /// If `offset` would land STRICTLY inside a chip decoration, snap
    /// to the chip boundary in the direction of travel. Chips are not
    /// character-traversable; `Home`/`End`/`Up`/`Down` went straight
    ...
    fn submit(&mut self, _: &TaSubmit, w: &mut Window, cx: &mut Context<Self>) {
        // Enter sends. Unhappy path: empty submit is a
        // no-op - unless the consumer opted into empty submits (the
    ...
    fn submit_immediate(&mut self, _: &TaSubmitImmediate, w: &mut Window, cx: &mut Context<Self>) {
        // Empty submit is still a no-op -- there is
        // nothing to send.
...
impl Render for TextArea {
    fn render(&mut self, w: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content_view = TextAreaContent {
            ...
            // Subtle accent tint for the "just inserted"
        ...
        div()
            .id("splitlane-text-area")
            .key_context("SplitlaneTextArea")
...
struct TextAreaContent {
    ...
    cursor_color: Hsla,
    /// Decorations to paint as chips over the shaped text.
    decorations: Vec<Decoration>,
    /// Surface color for the chip fill. Read from the
    /// active theme at render time so the chip blends with the
    /// composer's card surface.
    chip_bg: Hsla,
    /// Border / outline color for the chip.
    chip_border: Hsla,
    /// Tinted fill used when the chip is
    ...
    chip_accent_bg: Hsla,
    /// Accent border used in the selected
    /// state.
...
impl Element for TextAreaContent {
    ...
    fn paint(
        ...
        // Decoration chip overlays. Each decoration paints
        ...
        if !self.content.is_empty() {
            for deco in &self.decorations {
                ...
                // Chip height = line_height - 1px (Zed
...
/// selection.
/// Whether two byte ranges share any overlap.
fn ranges_overlap(a: &Range<usize>, b: &Range<usize>) -> bool {
    a.start < b.end && b.start < a.end
}

/// Decoration whose byte range contains `offset`. Inclusive
...
/// Decoration whose right edge sits at `offset` (cursor is
...
/// Decoration whose left edge sits at `offset` (cursor is
...
mod tests {
    ...
    ///
    /// The hand-rolled buffer this replaced passed whatever it was tested with
    /// and failed all four of these: it dropped anything with `cmd`/`ctrl`, had
    /// no selection, no cursor to move, and no `delete`. A test that calls the
    /// handler cannot see any of that.
    #[gpui::test]
    fn a_field_answers_the_keys_a_person_presses(cx: &mut gpui::TestAppContext) {
        ...
        cx.update(|window, cx| {
            cx.write_to_clipboard(ClipboardItem::new_string("atlas".to_string()));
            window.focus(&handle, cx);
            window.draw(cx).clear();
        });

        // Type, then select all and paste over it: the commonest rename there
        // is, and the one that was refused.
        cx.simulate_input("old");
        assert_eq!(view.read_with(cx, |area, _| area.value()), "old");
        cx.simulate_keystrokes(&format!("{cmd}-a"));
        cx.simulate_keystrokes(&format!("{cmd}-v"));
        assert_eq!(view.read_with(cx, |area, _| area.value()), "atlas");

        // The cursor moves, and `backspace` takes the character it is behind.
        cx.simulate_keystrokes("left left backspace");
        assert_eq!(view.read_with(cx, |area, _| area.value()), "atas");

        // `delete` takes the one in front of it, which is the other half of
        // having a cursor at all.
        cx.simulate_keystrokes("delete");
        assert_eq!(view.read_with(cx, |area, _| area.value()), "ats");

        // And copy works, so the name can be taken somewhere else.
        cx.simulate_keystrokes(&format!("{cmd}-a {cmd}-c"));
        let copied = cx.update(|_, cx| {
            cx.read_from_clipboard()
                .and_then(|item| item.text())
                .unwrap_or_default()
        });
        assert_eq!(copied, "ats");
    }

    /// The platform's own paste chord has to reach the field, and nothing else
    /// in the suite observes that: every other test drives the widget through
    /// its methods, so a binding that never dispatches still passes them all.
    ///
    /// Reported from live use as "renaming a session does not support Cmd+V" -
    /// typing worked, because text arrives through the input handler and needs
    /// no action dispatch at all.
    #[gpui::test]
    fn the_paste_chord_reaches_the_field(cx: &mut gpui::TestAppContext) {
        ...
        assert_eq!(
            view.read_with(cx, |area, _| area.content.clone()),
            "pasted",
            "{chord} did not reach the field"
        );
    }
    ...
    // ----------------------------------------------------------------
    // Atomic-cursor helpers. These tests cover the pure
