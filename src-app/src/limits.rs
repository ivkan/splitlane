//! Centralized ingress/egress size caps.
//!
...
//!
//! - **`MAX_PANES`** (4) - [`crate::layout::MAX_PANES`]. Live UI create cap
//!   (split / drop-to-split / IPC `surface.split` / `workspace.create`) ↔ read
//!   cap in [`splitlane_config::loader::validate_layout`] and at session
//!   restore.
//! - **`MAX_WORKSPACES`** (128) - [`crate::workspace::MAX_WORKSPACES`]. Live
//!   container-create cap ↔ `restore_workspaces` cap. One number for
//!   one list: the two rails used to cap their two views separately (20 and
//!   128), and merging at the lower bound would have dropped containers.
//! - **`MAX_CONFIG_SIZE_BYTES`** (1 MiB) - `splitlane_config::loader`. Read cap
//!   on `splitlane.json`; the app's own config writer never approaches it.
...
/// Deduplicated from two identical `const`s.
...
/// when persisting a pane. Its READ-side counterpart is
...
/// keeping one source is what guarantees Store and Load stay symmetric.
...
/// (`splitlane_mcp::ipc_client::MAX_RESPONSE_LEN`, a cross-crate mirror of this
...
/// Read cap on `session.json`. Sized from the write side:
...
/// `splitlane.json`, which previously had no session-side counterpart.
...
/// Splitlane but lives in a user-writable cache directory, so a corrupt or
