...
//!
//! The AST is fully owned (`String`, not `CowStr`) so `FileView` can keep
...
/// Hard upper bound on input size accepted by `parse_with_limit`. Past
/// 10 MB → return a `TooLarge` error so the view shows a warning
/// instead of allocating a many-million-element AST that would thrash GPUI
/// layout. 100 KB (the "fast path" budget) parses in well under 5 ms.
...
impl Walker {
    ...
    fn on_event(&mut self, event: Event<'_>) {
        ...
        match event {
            ...
            Event::Code(text) => {
                ...
                style.code = true;
                // Inline code reaches the renderer via `push_span`, NOT
    ...
    fn on_start(&mut self, tag: Tag<'_>) {
        match tag {
            ...
            Tag::Image {
                ...
                // benign path) and cap the length.
    ...
    fn push_text(&mut self, text: String) {
        // Strip bidi-control / zero-width code points from ALL ingress
...
/// Remove bidi-control / zero-width code points from `text`. Shared by
...
mod tests {
    ...
    fn body_text_strips_bidi_and_zero_width() {
        // A U+202E (RLO) override or zero-width char embedded directly in
    ...
    fn inline_code_strips_bidi_and_zero_width() {
        // Inline code reaches the renderer via push_span (not push_text),
    ...
    fn parses_100kb_under_budget() {
        ...
        // **Best of five, not one.** A wall-clock budget asserted on a single
        // run measures the machine as much as the code, and this one was
        // measured doing exactly that: under a twelve-way CPU load the parse
        // took 74.6 ms against a 60 ms budget and the suite went red - once in
        // roughly forty runs, with nothing wrong in the parser. That is the
        // worst shape a test can have, because the release pipeline runs this
        // on four shared CI legs where load is not ours to control, and a red
        // leg there costs a whole tagged build that cannot be salvaged.
        //
        // The minimum is the honest statistic for this question. A real
        // regression slows the *best* run too, so nothing is given up; only
        // scheduler noise, which inflates the worst run and never the best, is
        // taken out. Five is enough to make being descheduled through all of
        // them unlikely, and costs a few tens of milliseconds.
        ...
        assert!(
            ...
            "100 KB parse took {:?} at best of five, exceeds {} ms budget",
            elapsed,
            budget_ms
        );
    }
