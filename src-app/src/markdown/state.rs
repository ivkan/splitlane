//! Per-file markdown scroll-position persistence.
//!
//! On `FileView::open(path)`, the view consults `MarkdownState::load_offset`
...
//!
//! Storage: `dirs::cache_dir()/splitlane/markdown_state.json`, written
...
/// Process-wide shared state. All `FileView` panes serialize their
...
/// Public entry point used by `FileView::open` to look up a previously
/// saved offset for `path`.
pub fn lookup_offset_for(path: &Path) -> Option<f32> {
    shared().lock().ok()?.lookup_offset(path)
}

/// Public entry point used by `FileView`'s persistence task. Records
...
/// installed Splitlane's cache namespace -- belt + suspenders, the file
