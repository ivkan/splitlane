//! Markdown viewer security boundary.
//!
...
//! files inherit the privileges of `splitlane` itself - they could load
...
//!   check when the file exists. Missing files are accepted (a
...
//! - splitlane does not *load* images - the parser emits a `[image: <url>]`
//!   placeholder text span (`parser.rs`), so `validate_image_ref` has no load
//!   path to gate yet.
//! - splitlane does not *open* markdown links either - the StyledText render
...
///
/// `#[allow(dead_code)]` because splitlane does not yet *load* images -
...
/// Schemes splitlane's link handler is willing to delegate to the OS.
...
pub fn validate_image_ref(doc_root: &Path, image_ref: &str) -> Result<PathBuf, ImageRefError> {
    ...
    // lexical form if the file does not exist.
...
// ---------------------------------------------------------------------------
// Tests - image refs (4 cases) + link URLs (3 cases) + broader scheme
// coverage. Plus missing-file passthrough.
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn link_url_file_is_rejected() {
        // `[click](file:///bin/sh)` - must reject so a
    ...
    fn link_url_scheme_match_is_case_insensitive() {
        // `HTTPS://...` is legal per RFC 3986 section 3.1. Browsers normalise
    ...
    fn allowlist_is_http_https_only() {
        // Regression guard: when markdown link click hit-testing is
    ...
    fn image_ref_traversal_is_rejected() {
        // `![](../../etc/passwd)` - lexical escape catches
    ...
    fn image_ref_https_scheme_is_rejected() {
        // Reject `http`/`https` for IMAGES specifically - the
    ...
    /// A missing image file is NOT a security issue - a 404 is
    ...
    /// RFC 3986 section 3.1 composite schemes use `+`, `-`, `.` after the
    ...
    /// The image-ref scheme allowlist is empty (no
