...
//! both validated by an early spike against the real CLI.
//!
...
fn run_opencode_list(program: &str) -> Option<Vec<u8>> {
    ...
    let output =
        match splitlane_process::run_with_timeout(cmd, OPENCODE_DEADLINE, OPENCODE_STDOUT_CAP) {
            Ok(out) => out,
            Err(splitlane_process::ProcError::Spawn(err))
...
///
/// Field map (validated against opencode 1.14.41 in an early spike):
...
fn record_to_session(record: &Value, cwd: &str) -> Option<SessionMeta> {
    ...
    Some(SessionMeta {
        ...
        session_id,
        // OpenCode has no transcript file to stat, but `session list` already
        // reports `updated` - the same "last touched" notion the mtime stands
        // in for elsewhere - and `timestamp` above is derived from it.
        last_activity_secs: timestamp_ms / 1_000,
        timestamp,
        cwd: record_cwd,
        ...
        summary,
        // OpenCode's `session list` CLI contract carries no
...
/// anyway).
///
/// Crate-visible because the About dialog stamps the running binary's mtime
/// with it. A second copy of the civil-from-days arithmetic for one label
/// would be a worse trade than the odd import.
pub(crate) fn unix_ms_to_iso8601(ms: i64) -> String {
...
mod tests {
    ...
    /// initial investigation. Single record, `directory: "/home/alice"`,
    ...
    fn parse_sessions_happy_path_extracts_real_cli_record() {
        let (sessions, omitted) = parse_sessions(FIXTURE.as_bytes(), "/home/alice");
        assert_eq!(omitted, 0);
        assert_eq!(sessions.len(), 1, "fixture has one record at /home/alice");
        let meta = &sessions[0];
        assert_eq!(meta.agent, SessionAgent::OpenCode);
        assert_eq!(meta.session_id, "ses_1f80d49aeffeaKV4Lq4mc0c3cu");
        assert_eq!(meta.cwd, "/home/alice");
    ...
    fn read_sessions_returns_empty_when_binary_missing() {
        ...
        // covers the missing-binary case without depending on the test runner's PATH.
        let (sessions, omitted) =
            read_sessions_with_program("opencode-does-not-exist-zzz-9d2c1a", "/home/alice");
        assert_eq!(omitted, 0);
        assert!(sessions.is_empty());
    }
