//! Per-pane TCP listening-port + agent-process detection.
//!
...
//!
//! Cost contract: the process table is traversed ONCE per tick -
...
//!
//! BFS (not DFS) ordering is load-bearing: detection picks the agent binary
...
pub struct PaneScan {
    ...
    /// identity-pill agent; the union across panes feeds the
...
/// derived by the caller from `TerminalAgent::ALL` (one unified
/// vocabulary; matching is exact against `/proc/<pid>/comm`, which the
...
fn macos_children_map() -> std::collections::HashMap<u32, Vec<u32>> {
    ...
    let pids = match pids_by_type(ProcFilter::All) {
        ...
        Err(e) => {
            ...
            // diagnosable in splitlane-debug.log. Latched to log ONCE: this runs
...
mod tests {
    ...
    // Several agent binaries in the same subtree → the one nearest the
    // root wins: first-seen BFS order wins, duplicates
