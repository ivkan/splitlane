...
/// Separator between a base name and its cwd qualifier (`cargo-run@splitlane`).
/// ASCII and shell-typeable so a copied reference round-trips cleanly through
/// an agent prompt; `@` reads as "command @ directory". (An ASCII
/// separator was chosen over an earlier draft's `·` for typeability and
/// matchability.)
const CWD_SEP: char = '@';
...
/// Input order is preserved 1:1 in the output.
...
/// spawn-time label de-dup (`workspace.up`) shares the exact
...
mod tests {
    ...
    fn collision_qualified_by_cwd_basename() {
        let names = resolve_surface_names(&[
            auto("cargo-run", Some("/home/a/splitlane")),
            auto("cargo-run", Some("/home/a/web")),
        ]);
        assert_eq!(names, vec!["cargo-run@splitlane", "cargo-run@web"]);
    }
