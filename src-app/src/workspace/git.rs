...
//!
//! Extracted from `workspace.rs` during the src-app refactor.
...
/// What a file tree row says about a path's git status. The design
/// draws exactly two marks - `M` for changed and `A` for new - so this is
/// deliberately not a mirror of git's two-letter status code: `R`, `C`, `T` and
/// `U` all read as "this file is not what HEAD has", which is what `M` says.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GitMark {
    /// New to the repository: untracked, or staged as an addition.
    Added,
...
/// Wall-clock deadline for `git status --porcelain`. Same shape as the diff-stat
/// probe: bounds a dead network mount or a hanging credential helper.
const GIT_STATUS_DEADLINE: std::time::Duration = std::time::Duration::from_secs(10);

/// stdout cap for `git status --porcelain`. A repository with a very large
/// untracked tree can emit megabytes; 4 MiB covers any real one and bounds a
/// hijacked git.
const GIT_STATUS_STDOUT_CAP: u64 = 4 * 1024 * 1024;

/// Cap on the marks retained for one tree. A file tree shows tens of rows at a
/// time; a repository mid-rebase can report six figures of paths, and holding
/// every one of them would cost more than the marks are worth. Past the cap the
/// remaining paths simply carry no mark - a missing mark reads as "unchanged",
/// which is the safe direction to be wrong in for a decoration.
const GIT_STATUS_PATH_CAP: usize = 20_000;

/// Read `git status --porcelain` for the repository containing `cwd` and return
/// one mark per changed path, keyed by **absolute** path.
///
/// Blocking: spawns git and parses its whole output. Call it from
/// `smol::unblock`, never on the render thread.
///
/// Porcelain paths are relative to the repository top level (not to `cwd`), so
/// the top level is asked for first and the paths are joined onto it - a
/// container opened on a subdirectory of a repository still gets marks that
/// point at the right files.
pub fn status_marks(cwd: &str) -> std::collections::HashMap<std::path::PathBuf, GitMark> {
    let mut marks = std::collections::HashMap::new();
    let Some(top) = git_status_stdout(cwd, &["rev-parse", "--show-toplevel"])
        .map(|out| String::from_utf8_lossy(&out).trim().to_string())
    ...
    // `-z` because a path may contain anything a filesystem allows, including
    // a newline; without it git quotes such a path and the parse becomes a
    // second escaping problem. `-uall` so a new file inside a new directory is
    // marked itself rather than collapsing into a mark on its parent.
    let Some(out) = git_status_stdout(cwd, &["status", "--porcelain", "-z", "-uall"]) else {
        return marks;
    };
    parse_status_porcelain_z(&out, &top, &mut marks);
    marks
}

/// Parse `git status --porcelain -z` into marks. Split out from
/// [`status_marks`] so the format - including the extra NUL-separated field a
/// rename or copy emits - is testable without a repository.
fn parse_status_porcelain_z(
    out: &[u8],
    top: &std::path::Path,
    marks: &mut std::collections::HashMap<std::path::PathBuf, GitMark>,
) {
    let mut fields = out.split(|b| *b == 0).filter(|f| !f.is_empty());
    while let Some(entry) = fields.next() {
        // `XY <path>`: two status bytes, a space, then the path.
        if entry.len() < 4 {
            continue;
        }
        let x = entry[0];
        let y = entry[1];
        // A rename or copy writes the source path as its own following field.
        // It has no status line of its own, so it must be consumed here or the
        // next iteration would read it as a malformed entry.
        if x == b'R' || x == b'C' || y == b'R' || y == b'C' {
            let _ = fields.next();
        }
        if marks.len() >= GIT_STATUS_PATH_CAP {
            log::debug!(
                "git status: more than {GIT_STATUS_PATH_CAP} changed paths; \
                 the rest carry no tree mark"
            );
            return;
        }
        ...
        // `??` is untracked and `A` is a staged addition; both are new to the
        // repository, which is what the design's `A` says. Everything else that
        // git reports at all differs from HEAD, which is what `M` says.
        let mark = if x == b'?' || x == b'A' || y == b'A' {
            GitMark::Added
        } else {
            GitMark::Modified
        };
        marks.insert(top.join(path), mark);
    }
}
...
/// Canonicalize, falling back to the path itself, in the spelling git uses.
///
/// `repo_root` and `worktree_root` are handed to git as arguments and compared
/// against the paths git prints, and on Windows the verbatim `\\?\` spelling
/// `canonicalize` returns fails at both: `git worktree add` cannot create
/// leading directories under it, and it never compares equal to git's output,
/// so "is this checkout the repository's own?" answered no for every branch.
...
///
/// A relative `commondir` (`../..`) joined onto a worktree git dir
...
mod tests {
    ...
    fn resolve_repo_root_normal_repo() {
        ...
        assert_eq!(
            repo_root,
            Some(crate::runtime_paths::strip_verbatim_prefix(
    ...
    fn resolve_repo_root_worktree_relative_commondir() {
        ...
        assert_eq!(
            repo_root,
            Some(crate::runtime_paths::strip_verbatim_prefix(
                std::fs::canonicalize(dir.path().join("main")).unwrap()
            ))
        );
    }
    ...
    fn resolve_worktree_root_uses_stored_gitdir_pointer() {
        ...
        assert_eq!(
            root,
            crate::runtime_paths::strip_verbatim_prefix(
                std::fs::canonicalize(worktree_root).unwrap()
            )
        );
    }
    ...
    fn normalize_lexically_collapses_dotdot() {
        // A relative commondir on a non-existent worktree must still
    ...
    /// The `-z` format's rename entry carries its source path as an extra
    /// field. Missing that is how the parse would read `old.rs` as a status
    /// line and everything after it would shift by one.
    #[test]
    fn porcelain_z_parses_marks_and_consumes_a_rename_source() {
        use std::path::Path;
        let out: Vec<u8> = [
            b"?? new.rs\0".as_slice(),
            b"A  staged.rs\0".as_slice(),
            b" M edited.rs\0".as_slice(),
            b"R  moved.rs\0old.rs\0".as_slice(),
            b" D gone.rs\0".as_slice(),
        ]
        .concat();
        let top = Path::new("/repo");
        let mut marks = std::collections::HashMap::new();
        parse_status_porcelain_z(&out, top, &mut marks);
        assert_eq!(marks.get(Path::new("/repo/new.rs")), Some(&GitMark::Added));
        assert_eq!(
            marks.get(Path::new("/repo/staged.rs")),
            Some(&GitMark::Added)
        );
        assert_eq!(
            marks.get(Path::new("/repo/edited.rs")),
            Some(&GitMark::Modified)
        );
        assert_eq!(
            marks.get(Path::new("/repo/moved.rs")),
            Some(&GitMark::Modified)
        );
        assert_eq!(
            marks.get(Path::new("/repo/gone.rs")),
            Some(&GitMark::Modified)
        );
        // The rename source is a field of the rename entry, never an entry.
        assert!(!marks.contains_key(Path::new("/repo/old.rs")));
        assert_eq!(marks.len(), 5);
    }

    /// A path relative to the repository top level, joined onto it - so a
    /// container opened on a subdirectory still marks the right file.
    #[test]
    fn porcelain_z_paths_are_relative_to_the_repository_top_level() {
        use std::path::Path;
        let mut marks = std::collections::HashMap::new();
        parse_status_porcelain_z(b" M src/app/mod.rs\0", Path::new("/repo"), &mut marks);
        assert_eq!(
            marks.get(Path::new("/repo/src/app/mod.rs")),
            Some(&GitMark::Modified)
        );
    }
}
