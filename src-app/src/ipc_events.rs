...
//! OUT to a client, so the flow engine and any lead agent had to poll. This is
...
//!
//! Backpressure: each subscriber has a bounded queue. A slow client
...
// `events.subscribe` now streams on Windows too - the named-pipe
...
// the full splitlane-app cannot be cross-checked for Windows on the Linux build
...
/// small JSON lines stays well under an 8 MiB ceiling in practice.
const SUBSCRIBER_QUEUE_CAP: usize = 1024;

/// The event types a client may subscribe to. An `events.subscribe` that names
/// a type outside this set is rejected rather than producing a
