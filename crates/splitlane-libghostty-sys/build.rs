...
fn main() -> BuildResult<()> {
    println!("cargo:rerun-if-env-changed=SPLITLANE_LIBGHOSTTY_DIR");
    ...
    println!(
        "cargo:rustc-env=SPLITLANE_GHOSTTY_API_VERSION={}",
        manifest_value(&manifest, "api_version")?
    );
    println!(
        "cargo:rustc-env=SPLITLANE_GHOSTTY_APP_VERSION={}",
    ...
    let (prepared, uses_bundled_archive) = match std::env::var_os("SPLITLANE_LIBGHOSTTY_DIR") {
    ...
    // These two carry the OLD product name on purpose, and must survive any
    // future rename pass. They are not a naming choice: they reconstruct the
    // cache and prefix paths that the committed Windows prebuilt was ACTUALLY
    // built with, and are compared field-by-field against `build-info.txt`
    // below. The prebuilt was produced before the product was renamed, so the
    // paths recorded in it carry the old name; renaming these makes the build
    // script expect a build that never happened, and the Windows leg fails
    // with `build metadata canonical_cache_path is ..., expected ...`.
    //
    // They change only when the prebuilt is rebuilt - and then they change
    // because `build-info.txt` changed, not the other way round.
...
fn corrective_action(platform: NativePlatform, target: &str) -> String {
    match platform {
        NativePlatform::Linux => format!(
            "restore native/libghostty/prebuilt/{target}, or run scripts/build-libghostty-linux.sh --target {target} and set SPLITLANE_LIBGHOSTTY_DIR to its output; Cargo performs no downloads"
        ),
