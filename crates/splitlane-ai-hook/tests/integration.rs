...
//! path is computed). The tests must compile and pass on all three OSes.
//!
...
//! pipe handles from accumulating on Windows CI runners.
...
/// Path to the `splitlane-ai-hook` binary as produced by the surrounding
...
const HOOK_BIN: &str = env!("CARGO_BIN_EXE_splitlane-ai-hook");

/// Wall-clock timeout for a single frame to arrive on the mock listener.
/// The hook binary is ~360 KB stripped (as measured); local
...
impl MockServer {
    ...
    fn start() -> Self {
        ...
        let name = socket_path
            ...
            .expect("to_fs_name must succeed on a fresh endpoint path");
        let listener = ListenerOptions::new()
            .name(name)
            .create_sync()
            .expect("ListenerOptions::create_sync must succeed on a fresh endpoint");
...
fn unique_ipc_path() -> (PathBuf, PathKeepalive) {
    ...
    let dir = tempfile::TempDir::new().expect("TempDir allocation failed");
    let n = UNIQUE.fetch_add(1, Ordering::Relaxed);
    let path = dir.path().join(format!("splitlane-test-{n}.sock"));
...
fn unique_ipc_path() -> (PathBuf, PathKeepalive) {
    ...
    let path = PathBuf::from(format!(
        r"\\.\pipe\splitlane-test-{pid}-{n}-{nanos}-{nonce:016x}"
    ));
...
/// Spawn `splitlane-ai-hook <event>` with the given env and stdin; wait
/// up to `EXIT_TIMEOUT` for it to exit; panic if the process hangs.
/// Returns the `ExitStatus` so callers can assert exit 0: the hook must never
/// fail the agent CLI that invoked it.
fn run_hook(event: &str, env: &HookEnv<'_>, stdin_bytes: &[u8]) -> std::process::ExitStatus {
    let mut cmd = Command::new(HOOK_BIN);
    cmd.arg(event)
        // Strip inherited env so the host's `SPLITLANE_*` vars cannot
        // leak into the hook - crucial on developer machines that run
        // Splitlane locally while tests execute.
        .env_clear()
        // Preserve OS basics that the hook needs to start at all.
        // Different OSes require different minima.
        .envs(minimal_os_env())
        .env("SPLITLANE_WORKSPACE_ID", env.workspace_id.to_string())
        .env("SPLITLANE_AI_TOOL", env.tool)
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        ...
        // its normal diagnostics to `$SPLITLANE_HOOK_LOG`, not stderr,
        // so inheriting does not pollute test output on happy paths.
        .stderr(Stdio::inherit());

    if let Some(path) = env.socket_path {
        cmd.env("SPLITLANE_SOCKET_PATH", path);
    }
    if let Some(pid) = env.pid {
        cmd.env("SPLITLANE_AI_PID", pid.to_string());
    }
    if let Some(log) = env.hook_log {
        cmd.env("SPLITLANE_HOOK_LOG", log);
    }

    let mut child = cmd.spawn().expect("hook subprocess spawn failed");
    child
        .stdin
        .as_mut()
        .expect("hook stdin must be piped")
        .write_all(stdin_bytes)
        .expect("stdin write_all failed");
        ...
        match child.try_wait() {
            ...
            Ok(None) => {
                if start.elapsed() > EXIT_TIMEOUT {
                    let _ = child.kill();
                    panic!("hook subprocess did not exit within {EXIT_TIMEOUT:?}");
                }
...
fn assert_envelope<'a>(
    ...
    assert!(
        ...
        "[{scenario}]: hook frame must be a JSON-RPC notification"
    );
    let params = &frame["params"];
    assert_eq!(
        params["workspace_id"].as_u64(),
        Some(expected.workspace_id),
        "[{scenario}]: params.workspace_id mismatch"
    );
    assert_eq!(
        params["tool"], expected.tool,
        "[{scenario}]: params.tool mismatch"
    );
    assert!(
        params.get("hook_payload").is_some(),
        "[{scenario}]: params.hook_payload must be present"
    );
    for key in expected.absent_keys {
        assert!(
            params.get(*key).is_none(),
            "[{scenario}]: params.{key} must NOT be present on this event"
        );
    }
    params
}
...
fn claude_user_prompt_submit_dispatches_ai_prompt_submit() {
    ...
    assert!(status.success(), "hook must exit 0 on happy path");
...
fn claude_subagent_stop_also_dispatches_ai_stop() {
    // SubagentStop is mapped identically to Stop.
    let server = MockServer::start();
...
fn claude_pre_tool_use_dispatches_ai_tool_use_with_tool_name() {
    ...
    assert_eq!(
        ...
        "tool_name must be mirrored at top level of params"
    );
}

#[test]
fn claude_post_tool_use_also_dispatches_ai_tool_use() {
    // PostToolUse is mapped identically to PreToolUse.
    let server = MockServer::start();
    let stdin = json!({
        "session_id": "abc",
...
fn codex_session_start_dispatches_ai_session_start_with_pid() {
    ...
    assert_eq!(
        params["pid"].as_u64(),
        Some(4242),
        "SessionStart must carry pid at top level of params"
    );
}
...
fn socket_missing_hook_exits_0_and_no_frame_arrives() {
    ...
    // must exit 0 (it never fails the agent) and no frame must arrive on any of our
    ...
    assert!(
        status.success(),
        "hook must exit 0 even when socket is unreachable"
    );
    ...
    assert!(
        ...
        "no frame must land on an unrelated listener when socket is missing"
    );
}

#[test]
fn malformed_stdin_logs_diagnostic_and_exits_0() {
    // When stdin is invalid JSON, the hook exits 0 but
    // writes one diagnostic line to $SPLITLANE_HOOK_LOG if set.
    let server = MockServer::start();
    ...
    assert!(status.success(), "hook must exit 0 on malformed stdin");

    assert!(
        server.try_recv(Duration::from_millis(250)).is_none(),
        "no frame must be sent when stdin is not parseable JSON"
    );

    let log = std::fs::read_to_string(&log_path)
        .unwrap_or_else(|e| panic!("expected $SPLITLANE_HOOK_LOG at {log_path:?}: {e}"));
    assert!(
        log.contains("splitlane-ai-hook:") && log.contains("invalid stdin JSON"),
        "diagnostic log must mention the parse failure; got: {log:?}"
    );
}

#[test]
fn oversized_stdin_is_rejected_and_drops_frame() {
    // The ai-hook's only untrusted input is stdin (it is the hook
    // *producer*, not a JSONL file parser - that surface lives in the
    // sibling session loaders, which are bounded separately). The stdin read is
    // capped at MAX_STDIN_BYTES (16 MiB; `crates/splitlane-ai-hook/src/main.rs`
    ...
    assert!(
        status.success(),
        "hook must still exit 0 on oversized stdin"
    );

    assert!(
        server.try_recv(Duration::from_millis(250)).is_none(),
        "no frame must be sent when stdin exceeds the byte cap"
    );

    let log = std::fs::read_to_string(&log_path)
        .unwrap_or_else(|e| panic!("expected $SPLITLANE_HOOK_LOG at {log_path:?}: {e}"));
    assert!(
        log.contains("splitlane-ai-hook:") && log.contains("stdin exceeds"),
        "diagnostic log must mention the oversize rejection; got: {log:?}"
    );
}
