...
//! Splitlane AI-hook callback binary.
//!
//! Invoked by Claude Code / Codex CLI hooks from inside a Splitlane PTY. Reads
//! the hook JSON from stdin, builds a JSON-RPC 2.0 frame, and writes it to
//! Splitlane's IPC socket/pipe. Exits 0 on every path (silent fail) so a
//! Splitlane outage never breaks the user's AI CLI session.
//!
//! Contents: the blocking JSON-RPC client `send_frame`; the Claude Code hook
//! event → `ai.*` mapping with its env/stdin plumbing; the Codex hook event
//! mapping (`SessionStart`, `PermissionRequest`), tool-identity lookup via
//! `$SPLITLANE_AI_TOOL`, and session-start PID capture via `$SPLITLANE_AI_PID`
//! (set by the shim) with `hook_payload.pid` fallback.
...
/// Write deadline for the one-shot hook frame. The hook is invoked
...
/// Shim-synthesized frame carrying the wrapped agent
/// binary's REAL exit status (the shell's ChildExit never sees it).
const METHOD_EXIT: &str = "ai.exit";
const LIFECYCLE_EVENT_SOURCE_ENV: &str = "SPLITLANE_AI_EVENT_SOURCE";
...
/// attempt was enough to drop a lifecycle frame whenever the Splitlane main
...
/// a hook (and still never breaking the session: the final failure stays silent).
...
pub fn send_frame(socket_path: &Path, frame: &serde_json::Value) -> std::io::Result<()> {
    ...
    if payload.len() > MAX_HOOK_FRAME_BYTES {
        return Err(std::io::Error::new(
            ...
            "splitlane hook IPC frame exceeds the server size cap",
        ));
    }

    let mut stream = connect_frame_stream(socket_path)?;
    // Bound the write. The shim invokes this hook synchronously on
    ...
    // is the hook's "fail silent, never break the session" contract (a bounded
...
fn wait_for_io(deadline: Instant) -> std::io::Result<()> {
    ...
    if now >= deadline {
        return Err(std::io::Error::new(
            ...
            "splitlane hook IPC write timed out",
        ));
    }
        ...
        match stream.write(payload) {
            Ok(0) => {
                return Err(std::io::Error::new(
                    ...
                    "splitlane hook IPC write made no progress",
                ));
            }
...
/// - Codex (shared arms above plus):
///   - `SessionStart` → `ai.session_start` with required top-level `pid`
///     (nonzero u32; server validates at `ipc_handler.rs:351-358`). PID
///     comes from the `pid` parameter (usually `$SPLITLANE_AI_PID`) or
///     falls back to `hook_payload.pid` before giving up.
///   - `PermissionRequest` has no arm and is no longer registered by the shim
///     for any agent. It used to become an `ai.notification` the server
///     ignored, and then a call that waited for a person to answer a bar inside
///     Splitlane; an approval is answered in the agent's own terminal now.
#[cfg(test)]
fn build_frame(
    event: &str,
    workspace_id: u64,
    ...
    let method = match event {
        ...
        "Notification" => {
            ...
            match notif_type {
                ...
                // Surface unknown types to `$SPLITLANE_HOOK_LOG` so the whitelist
        ...
        // Exit is shim-synthesized like SessionEnd: `splitlane-shim` fires it
        ...
        // `$SPLITLANE_AI_EXIT_CODE`. Hard-require the code: a frame without
        ...
        // SessionEnd is invoked by `splitlane-shim` after the real AI binary
        // exits, NOT by claude/codex themselves - neither tool fires a
        // session-end hook event. The shim runs `splitlane-ai-hook SessionEnd`
        ...
        // No `PermissionRequest` arm: `dispatch` answers that event through
        // the permission exchange and returns before a frame is ever built.
        // It used to map to `ai.notification` with
        // `notification_type: "permission_prompt"`, which the server never
        // read - a frame built, sent and dropped. Removed rather than left
        // unreachable, so the next reader is not told about a mapping that
        // stopped happening.
        _ => return None,
    };
...
// ---------------------------------------------------------------------------
// Env-driven tool + PID detection
// ---------------------------------------------------------------------------

/// Resolve the AI tool identity from `$SPLITLANE_AI_TOOL`, which the shim
...
/// `"claude"` (the Claude Code behavior when the shim is not deployed).
fn detect_tool() -> String {
    detect_tool_from(env::var("SPLITLANE_AI_TOOL").ok().as_deref())
}
...
/// Read the AI binary PID from `$SPLITLANE_AI_PID` (set by the shim
...
fn read_ai_pid() -> Option<u32> {
    read_ai_pid_from(env::var("SPLITLANE_AI_PID").ok().as_deref())
}
...
/// Read the wrapped agent binary's raw exit code from
/// `$SPLITLANE_AI_EXIT_CODE` (set by the shim's `notify_exit`). `None` if
...
fn read_exit_code() -> Option<i32> {
    read_exit_code_from(env::var("SPLITLANE_AI_EXIT_CODE").ok().as_deref())
}
...
/// Append a single diagnostic line to `$SPLITLANE_HOOK_LOG` if set. Silent
...
/// symlinks on Unix. A malicious `$SPLITLANE_HOOK_LOG` symlink can cause
...
/// is a single `splitlane-ai-hook: <msg>` line containing no payload data.
fn diagnose(msg: &str) {
    diagnose_to(
        msg,
        env::var_os("SPLITLANE_HOOK_LOG").as_deref().map(Path::new),
    );
}
...
fn diagnose_to(msg: &str, log_path: Option<&Path>) {
    ...
    let line = format!("splitlane-ai-hook: {msg}\n");
...
/// the user's Claude Code session must never break because of a Splitlane
/// outage.
fn dispatch() {
    ...
    // `$SPLITLANE_AI_EXIT_CODE`. Every other event requires
    ...
    let hook_payload = if event == "SessionEnd" {
        ...
        let Some(code) = read_exit_code() else {
            diagnose("Exit: missing or invalid SPLITLANE_AI_EXIT_CODE");
            return;
        };
    ...
    // Best-effort: a missing or malformed surface_id leaves the
    // server falling back to workspace-only routing.
    let surface_id = read_surface_id();

    // Every event this binary handles reports and leaves. `PermissionRequest`
    // used to be the exception: it opened a connection that waited for a person
    // to answer a bar inside Splitlane. Nothing waits any more, and the event is
    // no longer registered by the shim for any agent - an agent asks in its own
    // terminal, so a hook that answered for it would be standing in the way.
    let Some(frame) = build_frame_with_event_source(
        ...
        // so a developer reading `$SPLITLANE_HOOK_LOG` knows whether to fix
        // their event name or check their env / stdin.
        let reason = if event == "SessionStart" {
            "missing pid (set $SPLITLANE_AI_PID or include pid in hook JSON)"
        } else if event == "Exit" {
...
/// invoked outside a Splitlane PTY - either way, do nothing.
fn read_socket_path() -> Option<PathBuf> {
    let raw = env::var_os("SPLITLANE_SOCKET_PATH")?;
    let path = PathBuf::from(raw);
    if !path.is_absolute() {
        diagnose("SPLITLANE_SOCKET_PATH is not absolute");
        return None;
    }
    Some(path)
}
...
fn read_workspace_id() -> Option<u64> {
    let raw = env::var("SPLITLANE_WORKSPACE_ID").ok()?;
    match raw.parse::<u64>() {
        Ok(id) => Some(id),
        Err(_) => {
            diagnose("SPLITLANE_WORKSPACE_ID is not u64");
            None
        }
    }
}
...
/// `$SPLITLANE_SURFACE_ID`. Optional: a missing or unparseable value
/// degrades cleanly.
fn read_surface_id() -> Option<u64> {
    env::var("SPLITLANE_SURFACE_ID").ok()?.parse::<u64>().ok()
}
...
fn read_stdin_json(event: &str) -> Option<serde_json::Value> {
    let mut buf = Vec::new();
    // Read ONE byte past the cap so a payload of *exactly*
...
// (`\\.\pipe\...`). Windows coverage lives in the integration tests. The pure-function
...
mod unix_tests {
    ...
    /// Spin up a `Listener` on a `tempfile::TempDir` path, call
...
mod tests {
    ...
    /// This event was once mapped onto `ai.notification` so the server could
    /// "later distinguish it from generic notifications", and the server never
    /// read it. A later version replaced that with a call that waited for a person;
    /// that is gone too. There is no mapping, and this test says so - otherwise
    /// the next person to add an arm here would silently start sending a frame
    /// nobody reads, again.
    #[test]
    fn permission_request_builds_no_notification_frame() {
        let payload = json!({ "message": "Approve shell command?" });
        assert!(
            build_frame("PermissionRequest", 2, "codex", payload, None, None).is_none(),
            "an approval is answered in the agent's own terminal - nothing here \
             reports it"
        );
    }
    ...
    fn send_frame_rejects_oversized_frame_before_connecting() {
        ...
        #[cfg(not(windows))]
        let path = Path::new("/tmp/splitlane-oversized-hook-test.sock");
        #[cfg(windows)]
        let path = Path::new(r"\\.\pipe\splitlane-oversized-hook-test");
    ...
    fn diagnose_to_appends_lines_when_log_path_set() {
        ...
        assert!(contents.contains("splitlane-ai-hook: first line"));
        assert!(contents.contains("splitlane-ai-hook: second line"));
        assert_eq!(contents.matches('\n').count(), 2);
    }

    /// Locks in that the Windows named-pipe path produced by
    /// `src-app/src/runtime_paths.rs:82` (`\\.\pipe\splitlane`) is recognised
    ...
    fn windows_named_pipe_path_is_absolute() {
        ...
        let p = PathBuf::from(r"\\.\pipe\splitlane");
        assert!(
            p.is_absolute(),
