...
//!
//! `splitlane hooks setup` writes Splitlane's `splitlane-ai-hook` callback into an
//! agent's **user-scope** config so the agent reports its turn state to the
//! running Splitlane instance. This is the durable counterpart to the shim's
//! ephemeral, project-local injection (`splitlane-shim::hooks`): the shim writes
...
//! notification-hook mechanism the callback plugs into. The other agents the
//! shim reports for (Codex, Gemini, opencode, ...) get their hooks or plugin
//! injected per launch by the shim, so `setup` reports them as such rather
//! than inventing a user-scope shape.
//!
//! The matcher-group shape and the `_splitlane_managed` marker are duplicated
//! from `splitlane-shim::hooks` (NOT shared: the shim is size-budgeted and does
//! not depend on this crate). They MUST stay byte-identical so the shim's
//! detection (`is_splitlane_matcher_group`) recognizes entries written here and
//! a future shim-skip can suppress the redundant ephemeral injection.
//!
...
/// Claude Code 2.x hook events Splitlane registers. Mirror of
/// `splitlane-shim::hooks::CLAUDE_HOOK_EVENTS` (kept in sync deliberately).
...
/// Marker on the outer matcher-group wrapper identifying a Splitlane-managed
/// hook. Mirror of the shim's marker so both writers recognize each other.
const MANAGED_MARKER: &str = "_splitlane_managed";

/// Claude Code's config directory: `CLAUDE_CONFIG_DIR` when set, because the
/// CLI honours it, else `~/.claude`. A user who sets it may have no `~/.claude`
/// at all, and a second account keeps its hooks in its own directory - so
/// `CLAUDE_CONFIG_DIR=... splitlane mcp install` hooks that account, and the
/// shim's persistent-hook check reads the same directory.
...
/// `<config dir>/settings.json` - where Claude Code reads user-scope hooks.
/// NOT `~/.claude.json` (that is the MCP-server file `mcp install` targets).
...
fn claude_detected() -> bool {
    which::which("claude").is_ok() || claude_config_dir().is_some_and(|dir| dir.exists())
}
...
/// `splitlane-shim::hooks::is_splitlane_hook_command`.
fn is_splitlane_hook_command(command: &str) -> bool {
...
fn is_splitlane_hook_program(program: &str) -> bool {
    let base = Path::new(&program)
        ...
        .and_then(|s| s.to_str())
        .unwrap_or(program);
    base == "splitlane-ai-hook" || base == "splitlane-ai-hook.exe"
}
...
/// True iff `group` is a Splitlane-managed matcher-group: the `_splitlane_managed`
...
/// command path current across Splitlane updates.
...
/// Install (or refresh) Claude Code's persistent Splitlane hooks. Idempotent and
...
fn status(expected_hook_path: &Path) -> Result<StatusOutcome> {
    ...
    let found_path = splitlane_hook_program_token(first).unwrap_or_default();
    Ok(classify(&found_path, expected_hook_path))
}

const HOOKS_USAGE: &str = "\
splitlane hooks - register the Splitlane agent-notification hooks with your agents

Usage:
  splitlane hooks setup       Install persistent hooks for every supported agent
  splitlane hooks uninstall   Remove the Splitlane hooks
  splitlane hooks status      Report the hook installation state per agent";
...
/// Entry point for `splitlane hooks <subcommand>`. `args` is everything after
/// `splitlane hooks`. `hook_path` is the stable ai-hook location resolved by the
    ...
    match command {
        HooksCommand::Setup => {
            let Some(hook_path) = hook_path else {
                let _ = writeln!(
                    err,
                    "hooks: the splitlane-ai-hook binary is unavailable (data dir unresolvable); cannot install"
                );
                return 1;
            };
        ...
        HooksCommand::Uninstall => match uninstall() {
            ...
            Ok(UninstallOutcome::NothingToRemove) => {
                let _ = writeln!(out, "claude-code: no Splitlane hooks present");
                0
            }
            Err(e) => {
                let _ = writeln!(err, "claude-code: error: {e:#}");
...
/// user-scope install applies. Gemini (hooks) and opencode (a status plugin)
/// are handled the same way, per launch.
fn report_other_agents(out: &mut dyn Write) {
    ...
    if which::which("opencode").is_ok() {
        let _ = writeln!(
            out,
            "opencode: status plugin installed per-launch by the shim (no user-scope install)"
        );
    }
}
...
mod tests {
    ...
    fn managed_group_matches_shim_shape() {
        let g = managed_group(Path::new("/bin/splitlane-ai-hook"), "Stop");
        assert_eq!(g[MANAGED_MARKER], json!(true));
        assert_eq!(g["hooks"][0]["type"], json!("command"));
        #[cfg(not(windows))]
        assert_eq!(
            g["hooks"][0]["command"],
            json!("/bin/splitlane-ai-hook Stop")
        );
        #[cfg(windows)]
        assert!(
            g["hooks"][0]["command"]
    ...
    fn managed_group_quotes_paths_that_shell_would_split() {
        ...
        assert_eq!(
            splitlane_hook_program_token(command).as_deref(),
            Some(expected.as_str())
        );
        assert!(is_managed_group(&g));
        assert!(matches!(
            classify(&splitlane_hook_program_token(command).unwrap(), path),
            StatusOutcome::Installed { .. }
        ));
    }
    ...
    fn command_program_token_handles_shell_escaped_quotes() {
        ...
        assert!(is_splitlane_hook_command(
            "'/tmp/with space/splitlane-ai-hook' Stop"
        ));
        assert!(is_splitlane_hook_command(
            "powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \"& 'C:/Program Files/Splitlane/bin/splitlane-ai-hook.exe' Stop\""
        ));
    }
    ...
    fn windows_hook_paths_use_forward_slashes() {
        let path = Path::new(r"C:\Users\Arthur\AppData\Local\splitlane\bin\splitlane-ai-hook.exe");
        let g = managed_group(path, "Stop");
        let command = g["hooks"][0]["command"].as_str().unwrap();
        ...
        assert_eq!(
            splitlane_hook_program_token(command).as_deref(),
            Some("C:/Users/Arthur/AppData/Local/splitlane/bin/splitlane-ai-hook.exe")
        );
        assert!(is_managed_group(&g));
    }
    ...
    fn set_then_remove_round_trips_to_empty() {
        let mut root = json!({});
        set_managed_hooks(&mut root, Path::new("/bin/splitlane-ai-hook"));
    ...
    fn install_is_idempotent_and_updates_on_path_change() {
        ...
        let (wrote, had) = writeit("/v1/splitlane-ai-hook");
        assert!(wrote && !had, "first install writes, no prior");
        // Re-run identical → no write.
        let (wrote, had) = writeit("/v1/splitlane-ai-hook");
        assert!(!wrote && had, "idempotent re-run");
        // New path (Splitlane update) → rewrite, prior present.
        let (wrote, had) = writeit("/v2/splitlane-ai-hook");
        assert!(wrote && had, "path change rewrites");

        let root = read(&path);
        assert_eq!(
            root["hooks"]["Stop"][0]["hooks"][0]["command"],
            json!(hook_command(Path::new("/v2/splitlane-ai-hook"), "Stop"))
        );
