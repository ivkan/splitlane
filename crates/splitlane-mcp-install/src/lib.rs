//! `splitlane-mcp-install` - the GPU-free engine behind
//! `splitlane mcp install | uninstall | status`.
//!
//! Why a separate crate:
//! - **Testable without GPU.** All config-merge / detection logic lives
//!   here as pure-ish functions driven by `serde_json::Value` and
//!   `toml_edit::DocumentMut`, so `cargo test -p splitlane-mcp-install` runs
//!   with zero GPUI / Vulkan dependency.
//! - **Keeps `toml_edit` out of the embedded bridge.** The size-budgeted
//!   `splitlane-mcp` binary depends only on
//!   `serde`/`serde_json`/`interprocess`. This crate links *only* into
//!   `splitlane-app`, never into `splitlane-mcp` - verifiable with
//!   `cargo tree -p splitlane-mcp` (it must list neither `toml_edit` nor
