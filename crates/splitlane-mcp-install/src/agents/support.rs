...
//!   non-zero exit (preferred path for Claude Code / Codex).
...
pub(crate) const ENTRY: &str = "splitlane";
...
/// Wall-clock deadline for an agent CLI shell-out. `mcp add` is a quick
...
pub(crate) fn shell_out(program: &str, args: &[&str]) -> Result<()> {
    // Windows: a bare `Command::new("claude")` goes through
    ...
    // Bound the CLI with a wall-clock deadline so a hung `claude`/`codex
    // mcp add` (network stall, auth prompt) can't block the installer.
    // run_with_timeout nulls stdin and caps stdout/stderr for us.
    let output = splitlane_process::run_with_timeout(command, CLI_DEADLINE, CLI_STDOUT_CAP)
...
/// Upsert `root[container][splitlane] = entry` at `path`, idempotently and
...
/// Remove `root[container][splitlane]` at `path`. No-op when the file or
/// entry is absent.
...
/// Read-only state of the `splitlane` JSON entry at `path`. `extract`
...
pub(crate) fn toml_status(path: &Path, expected: Option<&Path>) -> Result<StatusOutcome> {
    ...
    let found = entry
        ...
        .and_then(|c| c.as_str())
        .map(str::to_string);
    // `args` is OPTIONAL, and that is the second writer talking. `install`
    // prefers `codex mcp add` over editing `~/.codex/config.toml` ourselves,
    // and the CLI writes the table with `command` alone - no `args` key -
    // where our own direct-merge writer writes `args = []`. Absent and empty
    // are the same claim ("no arguments"), so requiring the key answered
    // `needs repair` immediately after a successful CLI install. Measured
    // 6 September 2026. A `args` that is present and non-empty is still a
    // repair, and so is one that is present and not an array.
    let args_ok = match entry.get("args") {
        None => true,
        Some(args) => args.as_array().is_some_and(|args| args.is_empty()),
    };
...
mod tests {
    ...
    fn json_install_preserves_siblings() {
        ...
        assert_eq!(after["mcpServers"]["splitlane"]["command"], json!("/p"));
    }
    ...
    fn json_uninstall_removes_only_target() {
        ...
        std::fs::write(
            &p,
            serde_json::to_vec(&json!({
                "mcpServers": { "splitlane": { "command": "/p" }, "other": { "command": "x" } }
            }))
            .unwrap(),
        )
        .unwrap();
        ...
        let after: serde_json::Value = serde_json::from_slice(&std::fs::read(&p).unwrap()).unwrap();
        assert!(after["mcpServers"].get("splitlane").is_none());
        assert_eq!(after["mcpServers"]["other"]["command"], json!("x"));
    ...
    fn json_status_reports_installed_and_stale() {
        ...
        std::fs::write(
            &p,
            serde_json::to_vec(&json!({ "mcpServers": { "splitlane": { "command": "/cur" } } }))
                .unwrap(),
        )
        .unwrap();
    ...
    fn json_status_without_expected_path_requires_command() {
        ...
        std::fs::write(
            &p,
            serde_json::to_vec(&json!({ "mcpServers": { "splitlane": { "args": [] } } })).unwrap(),
        )
        .unwrap();
    ...
    fn array_command_extracts_first_element() {
        let entry = json!({ "type": "local", "command": ["/bin/splitlane-mcp"], "enabled": true });
        assert_eq!(
            array_command(&entry),
            Some("/bin/splitlane-mcp".to_string())
        );
    }
}
