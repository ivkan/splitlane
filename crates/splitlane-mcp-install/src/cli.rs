//! `splitlane mcp <subcommand>` CLI front-end.
//!
...
//! bypasses this formatter and consumes [`crate::api`] directly.
...
const USAGE: &str = "\
splitlane mcp - register the Splitlane MCP bridge with your CLI agents

Usage:
  splitlane mcp install      Register the bridge with every detected agent
  splitlane mcp uninstall    Remove the Splitlane entry from every agent
  splitlane mcp status       Report the bridge registration state per agent";
...
/// Entry point. `args` is everything *after* `splitlane mcp` (i.e. the
    ...
    for r in &results {
        match &r.kind {
            ...
            UninstallKind::NothingToRemove => {
                let _ = writeln!(out, "{}: no Splitlane entry (nothing to remove)", r.id);
            }
    ...
    for r in &results {
        match &r.kind {
            ...
            StatusKind::Stale { found, expected } => {
                let _ = writeln!(
                    out,
                    "{}: stale path (config points at {found}, expected {expected}) - re-run `splitlane mcp install`",
                    r.id
                );
            }
            StatusKind::NeedsRepair { path, reason } => {
                ...
                let _ = writeln!(
                    out,
                    "{}: needs repair{suffix} ({reason}) - re-run `splitlane mcp install`",
                    r.id
                );
            }
...
mod tests {
    ...
    fn install_refuses_when_bridge_missing() {
        ...
        let missing = Path::new("/definitely/not/here/splitlane-mcp");
    ...
    fn install_writes_present_skips_absent() {
        ...
        let bridge = dir.path().join("splitlane-mcp");
