//! Structured, programmatic API over the agent writers.
//!
//! [`crate::cli`] formats human output for `splitlane mcp …`; this module
...
pub enum OverallState {
    ...
    NoAgents,
    /// At least one detected agent has no `splitlane` entry yet.
    NeedsInstall,
...
pub(crate) fn install_with(
    ...
    // Resolve each writer's presence EXACTLY ONCE. `presence()` does a
    ...
    let bridge = if any_present {
        match bridge {
            ...
            Some(p) => {
                return Err(format!(
                    "MCP bridge binary is missing at {} - launch Splitlane once to extract it, then retry. Nothing was written.",
                    p.display()
                ));
            }
            None => {
                return Err(
                    "could not resolve the Splitlane data directory, so the bridge path is unknown. Nothing was written."
                        .to_string(),
                );
            }
        }
...
/// Remove the `splitlane` entry from every detected agent.
