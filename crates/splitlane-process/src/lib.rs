//! Bounded external-process execution shared across Splitlane crates.
//!
//! `std`-only, zero external dependencies, so it can be a dependency of the
//! embedded `splitlane-shim` without inflating the binary that ships inside the
//! main executable.
//!
//! Every external subprocess in Splitlane must run under a wall-clock deadline
...
/// Splitlane runs as a GUI-subsystem process with no console of its own, so every
