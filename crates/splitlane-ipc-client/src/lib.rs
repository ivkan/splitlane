...
//! splitlane-ipc-client - blocking JSON-RPC client for Splitlane's local IPC socket.
//!
...
//! Windows named pipe). Unlike `splitlane-ai-hook` (fire-and-forget), this
...
//! runs as the same user that launched Splitlane.
//!
//! Shared crate (no GPUI / `src-app` dependency): consumed both by the MCP
//! bridge (`splitlane-mcp`) and the `splitlane` CLI subcommands.
...
/// Per-reply read cap on the untrusted IPC socket. Mirrors the server's
...
/// Abstraction over "send a JSON-RPC request to Splitlane, get the `result`".
/// Lets callers (MCP layer, CLI) be unit-tested against a fake transport with
/// no live socket.
pub trait IpcTransport {
    /// Call a Splitlane IPC method. Returns the `result` value on success, or
...
pub(crate) fn parse_response(line: &str) -> Result<Value, String> {
    let value: Value = serde_json::from_str(line.trim())
        .map_err(|e| format!("invalid JSON-RPC response from splitlane: {e}"))?;
    ...
    value
        .get("result")
        .cloned()
        .ok_or_else(|| "splitlane response missing both `result` and `error`".to_string())
}
...
fn jsonrpc_error_message_from_value(value: &Value) -> Option<String> {
    ...
    Some(format!("splitlane error {code}: {message}"))
}

/// Open a connection, write the newline-terminated request, and read back one
/// newline-delimited response line.
///
/// The read deadline is enforced at the OS level on Unix and through
...
/// agent retrying `read_pane` against a wedged Splitlane exhausted the
...
fn send_and_receive(socket: &Path, request: &Value) -> io::Result<String> {
    ...
    #[cfg(not(windows))]
    {
        ...
        // Cap the reply read at MAX_RESPONSE_LEN (Take rebuilt per call, so
        ...
        match reader.by_ref().take(MAX_RESPONSE_LEN).read_line(&mut line) {
            Ok(n) if n as u64 >= MAX_RESPONSE_LEN && !line.ends_with('\n') => Err(io::Error::new(
                ...
                "splitlane response exceeded the size cap",
            )),
            Ok(_) => Ok(line),
                ...
                ) =>
            {
                Err(io::Error::new(
                    ...
                    "splitlane did not respond within 10s",
                ))
            }
            Err(e) => Err(e),
        }
    ...
    fn wait_for_completion(
        ...
        match unsafe { WaitForSingleObject(overlapped.hEvent, timeout_ms) } {
            ...
            WAIT_TIMEOUT => {
                ...
                Err(io::Error::new(
                    io::ErrorKind::TimedOut,
                    "splitlane named-pipe I/O timed out",
                ))
            }
            WAIT_FAILED => {
        ...
        while !payload.is_empty() {
            ...
            if transferred == 0 {
                return Err(io::Error::new(
                    ...
                    "splitlane IPC write made no progress",
                ));
            }
        ...
        match windows_pipe::read_some(stream, &mut chunk[..read_buffer_len], remaining) {
            Ok(0) if out.is_empty() => {
                return Err(io::Error::new(
                    ...
                    "splitlane closed the IPC connection without a response",
                ));
            }
            Ok(0) => break,
            Ok(n) => {
...
/// Open a persistent `events.subscribe` stream.
    ...
    R: BufRead,
{
    ...
    loop {
        ...
        if remaining == 0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
                "splitlane event line exceeded the size cap",
            ));
        }
        ...
        if read == 0 {
            ...
            return Err(io::Error::new(
                io::ErrorKind::UnexpectedEof,
                "splitlane event stream ended mid-line",
            ));
        }
...
/// Resolve the Splitlane IPC socket path. `SPLITLANE_SOCKET_PATH` (inherited
/// from the Splitlane PTY through the agent that launched this process) is
/// authoritative - it carries the exact path the running instance bound.
/// Falls back to the current build profile's default (`splitlane-dev` in debug,
/// `splitlane` in release), mirroring `src-app/src/runtime_paths.rs`.
pub fn resolve_socket_path() -> Option<PathBuf> {
    if let Some(p) = socket_path_from_env(std::env::var("SPLITLANE_SOCKET_PATH").ok().as_deref()) {
        return Some(p);
    }
    default_socket_path()
}

/// Validate a `SPLITLANE_SOCKET_PATH` value: present and absolute. A relative
/// path means the env was clobbered or we're outside a Splitlane PTY.
...
mod tests {
    ...
    fn tolerate_unsupported_swallows_only_unsupported() {
        // Regression (Windows port): Windows named pipes reject I/O
    ...
    fn socket_path_from_env_requires_absolute() {
        ...
        #[cfg(not(windows))]
        let absolute = "/run/user/1000/splitlane/splitlane.sock";
        #[cfg(windows)]
        let absolute = r"\\.\pipe\splitlane";
        assert_eq!(
    ...
    fn windows_ipc_client_round_trips_when_response_is_delayed() {
        ...
        let path = PathBuf::from(format!(
            r"\\.\pipe\splitlane-ipc-client-test-{}-{}",
            std::process::id(),
    ...
    /// A full request/response round-trip over a real local socket
    /// (not just the pure helpers). Spins up an `interprocess` listener that
    /// speaks the Splitlane framing - read one newline-delimited request, echo
    ...
    fn ipc_client_round_trips_against_a_live_socket() {
        ...
        let path = dir.path().join("splitlane-test.sock");
        let name = path.as_path().to_fs_name::<GenericFilePath>().unwrap();
        let listener: Listener = ListenerOptions::new().name(name).create_sync().unwrap();
