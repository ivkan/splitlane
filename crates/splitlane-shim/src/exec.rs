//! Run the real AI binary + Unix signal handling.
...
use crate::{
    locate_sibling_hook_binary, SPLITLANE_AI_EVENT_SOURCE_ENV, SPLITLANE_AI_EVENT_SOURCE_INTERRUPT,
};
#[cfg(any(unix, windows))]
use std::io::Write;
...
// The shim originally used `CommandExt::exec()` on Unix for zero-fork process
// replacement. The `HookConfigGuard` drop-cleanup contract came later, and it is
// incompatible with `exec()` - process replacement skips every Rust
...
// is redundant - but it is spelled out explicitly to make the
...
pub(crate) fn run_real(tool: &str, path: &Path, args: &[OsString]) -> (ExitCode, Option<i32>) {
    ...
    cmd.args(args)
        ...
        // SPLITLANE_AI_PID - stable session identity propagated to every
        // `splitlane-ai-hook` invocation fired by claude/codex during this
    ...
    unsafe {
        ...
        // Capture the shim's own PID in the parent, before fork. Inside
        ...
        cmd.pre_exec(move || {
            ...
            // On Linux, install PR_SET_PDEATHSIG so the spawned agent CLI
            // (claude/codex/opencode) is killed by the kernel as
            // soon as the shim's parent process (Splitlane) dies --
            // even on a hard `kill -9` of Splitlane that bypasses
            ...
            // covers the realistic Linux path. macOS takes a separate
            // path: `spawn_parent_death_guard` below.
            //
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // Close the fork↔prctl race. PDEATHSIG only fires on
                ...
                // Splitlane runs under `systemd --user` (a PR_SET_CHILD_SUBREAPER
    ...
    // Windows Ctrl+C -> `ai.stop` (claude/codex interrupt their
    ...
    // Capture Splitlane's PID before spawn for the macOS orphan
    // guard's reparent detection. SAFETY: `getppid` is a trivial syscall.
    #[cfg(target_os = "macos")]
    let parent_pid = unsafe { libc::getppid() } as u32;
    // Flipped the instant `child.wait()` reaps the agent, so the
    ...
    let mut child = match cmd.spawn() {
        Ok(c) => c,
        Err(e) => {
            eprintln!("splitlane-shim: spawn '{}' failed: {e}", path.display());
            return (ExitCode::from(127), None);
        }
    };

    // macOS parity for the Linux `PR_SET_PDEATHSIG` guard - kill
    // the agent if Splitlane dies, so a `kill -9` of Splitlane leaves no orphan.
    // Spawned after we hold the child PID.
    #[cfg(target_os = "macos")]
    spawn_parent_death_guard(child.id(), parent_pid, std::sync::Arc::clone(&child_reaped));

    let wait_result = child.wait();
    // Tell the guard the agent is reaped BEFORE its PID can be
    ...
    match wait_result {
        ...
        Err(e) => {
            eprintln!("splitlane-shim: wait on '{}' failed: {e}", path.display());
            (ExitCode::from(1), None)
        }
    }
}

/// Map a child `ExitStatus` to this process's `ExitCode`.
///
...
/// The same mapping, kept as a full-width `i32` for the
...
/// `sigwait`s on it. On every Ctrl+C, send `ai.stop` to Splitlane so the
...
/// (see Stevens, APUE, section 12.8 "pthread_sigmask").
#[cfg(unix)]
pub(crate) fn install_sigint_watcher(tool: &str) {
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            // session - graceful degradation: the shim never lets its own
            // telemetry break the agent it wraps).
            let sig = unsafe {
                let mut set: libc::sigset_t = std::mem::zeroed();
                libc::sigemptyset(&mut set);
                libc::sigaddset(&mut set, libc::SIGINT);
...
/// Ceiling on concurrent detached reaper threads. A user mashing
...
/// Spawn `splitlane-ai-hook Stop` with `{}` piped to stdin. Best-effort;
...
///
/// The reaper count is bounded by [`MAX_INFLIGHT_REAPERS`] - once that
...
/// Windows parity for the Unix `sigwait`
...
/// macOS parity for the Linux
/// `PR_SET_PDEATHSIG` guard. When Splitlane is hard-killed
...
/// the user's API tokens. kqueue `NOTE_EXIT` is the textbook idiom, but any
/// equivalent will do: a tiny thread that polls `getppid()` is the same
/// reparent-detection the Linux fork↔prctl race-close already uses (`getppid() !=
...
/// Splitlane, confirm no orphaned agent survives) is still required before it is
/// trusted.
#[cfg(target_os = "macos")]
pub(crate) fn spawn_parent_death_guard(
    ...
    std::thread::spawn(move || {
        loop {
            ...
            //
            // A changed parent means Splitlane exited and the kernel reparented
