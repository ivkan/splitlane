//! Tool detection + real-binary location.
...
/// if the shim was invoked under an unexpected name (direct `splitlane-shim`
...
/// Every binary name the app's shim extractor may materialize this shim under -
...
/// lowercase matches against [`WRAPPED_TOOLS`] are accepted - the app's
/// extractor controls the extracted filenames, so anything else here means the
...
/// that is the shim binary itself by inode (hardlink defense).
    ...
    I: IntoIterator<Item = PathBuf>,
{
    ...
    // Capture the shim's own file identity. A candidate matching
    ...
    for dir in path_entries {
        ...
        for name in &candidates {
            ...
            // On Unix, require the executable bit too. A non-executable
...
/// Capture a file's cross-platform identity. `same_file::Handle` uses the native
