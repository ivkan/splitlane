...
//! Splitlane AI-binary shim.
//!
//! Copied (by the app's extractor) under every `TerminalAgent` binary name
//! (`claude`, `codex`, `gemini`, `cursor-agent`, …) into the Splitlane bin
//! cache dir, which the app prepends to the PTY's `$PATH`. When the user
...
//!    platforms spawn + wait (`Command::status()`): the hook-config drop-cleanup
//!    guards and the `ai.exit` exit-status report are incompatible
//!    with `exec()`-style process replacement. The exit code is
//!    propagated verbatim (shell `128+signum` convention for signals).
//!
//! The core is detect / find / exec. Hook config injection
//! (`.claude/settings.local.json`, `.codex/hooks.json`) and env-var injection
//! (`$SPLITLANE_AI_TOOL` / `$SPLITLANE_AI_PID`, consumed by
//! `splitlane-ai-hook`) wrap around that skeleton.
...
const SPLITLANE_AI_EVENT_SOURCE_ENV: &str = "SPLITLANE_AI_EVENT_SOURCE";
const SPLITLANE_AI_EVENT_SOURCE_INTERRUPT: &str = "interrupt";
...
/// `splitlane-ai-hook`'s `diagnose()`: appends one line to `$SPLITLANE_HOOK_LOG`
...
pub(crate) fn diagnose(msg: &str) {
    let Some(path) = env::var_os("SPLITLANE_HOOK_LOG") else {
        return;
    };
    if path.is_empty() {
        return;
    ...
    let line = format!("splitlane-shim[{}]: {msg}\n", std::process::id());
...
fn main() -> ExitCode {
    let Some(tool) = detect_tool() else {
        // Direct invocation (`./splitlane-shim`) or unexpected rename. Exit 2
        // matches `getopts` convention for "usage error" - the one case
        // where stderr output is acceptable because the user's command
        // cannot proceed regardless of Splitlane state.
        eprintln!(
            "splitlane-shim: invoked under an unexpected name; copy or \
             hardlink this binary under one of the Splitlane-wrapped agent \
    ...
    let Some(real) = find_real_binary(tool) else {
        ...
        eprintln!("splitlane-shim: could not find real '{tool}' on PATH after self-exclusion");
    ...
    // read-only FS / missing permissions (hook reporting must never break
    // the agent it wraps) - and for every wrapped
    ...
    // will fire `splitlane-ai-hook` and the sidebar should update.
    diagnose(&format!(
    ...
    // Report the agent binary's REAL exit status. The shell's
    ...
    // `splitlane-ai-hook SessionEnd` to send a single `ai.session_end`
...
fn install_hook_guard(tool: &str) -> Option<ToolHookGuard> {
    match tool {
        ...
        // Dedicated merged hook file - wholly Splitlane-owned, zero RMW.
...
/// Best-effort notify of `ai.exit { exit_code }` after the
...
/// rides in `SPLITLANE_AI_EXIT_CODE` since the hook's stdin is null on
...
fn notify_exit(tool: &str, exit_code: i32, interrupted: bool) {
    ...
    cmd.arg("Exit")
        .env("SPLITLANE_AI_TOOL", tool)
        .env("SPLITLANE_AI_PID", std::process::id().to_string())
        .env("SPLITLANE_AI_EXIT_CODE", exit_code.to_string())
        .stdin(std::process::Stdio::null())
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    if interrupted {
        cmd.env(
            SPLITLANE_AI_EVENT_SOURCE_ENV,
            SPLITLANE_AI_EVENT_SOURCE_INTERRUPT,
        );
    }
    let _ = cmd.status();
}
...
///
/// Locates `splitlane-ai-hook` next to this shim binary (the app extracts
...
/// with `SessionEnd` and the `SPLITLANE_AI_TOOL` env so the hook tags the
/// frame with the right tool identity. Inherits `SPLITLANE_SOCKET_PATH`
/// and `SPLITLANE_WORKSPACE_ID` from the shim's own env (they were set
...
/// 15 ms latency budget for shim overhead is preserved
...
/// Resolve `splitlane-ai-hook` (or `.exe` on Windows) sitting in the same
...
pub(crate) fn locate_sibling_hook_binary() -> Option<PathBuf> {
    ...
    let dir = exe.parent()?;
    #[cfg(unix)]
    let name = "splitlane-ai-hook";
    #[cfg(windows)]
    let name = "splitlane-ai-hook.exe";
...
mod tests {
    ...
    fn command_preserves_event_arg(command: &str, event: &str) -> bool {
        ...
        // quote (escaped or not) once it has been through JSON. Matching on
        // suffixes missed the escaped form; matching on the token does not.
        command
            .split_whitespace()
    ...
    fn detect_tool_from_stem_rejects_everything_else() {
        assert_eq!(detect_tool_from_stem("splitlane-shim"), None);
    ...
    /// A real binary on `$PATH` carries the executable bit; the walk
    ...
    fn find_real_binary_in_skips_non_executable_homonym() {
        // Negative test: a non-executable file named like the tool
    ...
    /// A hardlink of the shim binary planted in a DIFFERENT `$PATH`
    /// directory must be detected by file identity and skipped. The previous
    /// dir-only check let this through, recursively re-invoking the shim
    /// every time the user typed `claude` -- a single-user fork-bomb.
    #[test]
    fn shim_refuses_hardlink_loop() {
        ...
        let real_shim = shim_dir.path().join("splitlane-shim");
    ...
    /// Linux-gated timing guard. Replaces a criterion benchmark
    /// with a lightweight check that stays within
    ...
    fn find_real_binary_in_completes_under_15ms_budget() {
        let dirs: Vec<PathBuf> = (0..20)
            .map(|i| PathBuf::from(format!("/tmp/splitlane-nonexistent-{i}")))
            .collect();
    ...
    // same env-free discipline the `splitlane-ai-hook` tests use.
    ...
    fn count_splitlane_entries(root: &serde_json::Value, event: &str) -> usize {
        root["hooks"][event]
            .as_array()
            .map(|a| a.iter().filter(|v| is_splitlane_matcher_group(v)).count())
            .unwrap_or(0)
    }
    ...
    fn install_at_creates_file_with_all_five_events() {
        ...
        for event in CLAUDE_HOOK_EVENTS {
            ...
            // whether `current_exe()` finds a sibling `splitlane-ai-hook` -
            ...
            // by `is_splitlane_hook_command`, and it must preserve the event
            ...
            assert!(
                ...
                "{event}: command {cmd:?} must be recognized as splitlane-managed"
            );
            assert!(
            ...
            // Five seconds for every event, because every event reports and
            // leaves. `PreToolUse` carried no key at all while it held a tool
            // call open for a person to answer a bar; nothing waits any more,
            // and a hook with no deadline is a hook nobody can cut off.
            assert_eq!(
                handlers[0].pointer("/hooks/0/timeout"),
                Some(&json!(5)),
                "{event} reports and leaves - timeout is in seconds per Claude \
                 Code docs"
            );
            ...
            // ignore anyway).
            assert_eq!(
                handlers[0].get("_splitlane_managed"),
                Some(&json!(true)),
                "outer matcher-group must carry the managed marker"
            );
            assert!(
                handlers[0].pointer("/hooks/0/_splitlane_managed").is_none(),
    ...
    fn install_at_preserves_existing_user_hooks_and_permissions() {
        ...
        // After install: user entry + Splitlane entry side-by-side.
        let root = read_settings(&claude_dir);
        let arr = root["hooks"]["UserPromptSubmit"].as_array().unwrap();
        assert_eq!(arr.len(), 2, "user + splitlane entries coexist");
        assert_eq!(
            arr.iter().filter(|v| is_splitlane_matcher_group(v)).count(),
            1
        );
    ...
    fn install_at_is_idempotent_on_reinstall() {
        ...
        for event in CLAUDE_HOOK_EVENTS {
            assert_eq!(
                count_splitlane_entries(&root, event),
                1,
                "{event} must carry exactly one Splitlane entry after re-install"
            );
        }
    ...
    fn merge_replaces_non_object_hooks_and_populates_events() {
        ...
        merge_splitlane_hooks(&mut root);
        for event in CLAUDE_HOOK_EVENTS {
            assert_eq!(
                count_splitlane_entries(&root, event),
                1,
    ...
    fn cleanup_removes_managed_entries_even_when_marker_was_stripped() {
        ...
        // `_splitlane_managed` marker from the inner hook object. The
        ...
        let stripped = json!({
            "hooks": {
                "Stop": [
                    {
                        // `_splitlane_managed` on the outer wrapper is gone.
                        "hooks": [
                            {
                                "type": "command",
                                "command": "splitlane-ai-hook Stop",
                                "timeout": 5
                            }
                        ]
                    }
    ...
    fn install_at_tolerates_corrupt_existing_json() {
        ...
        let root = read_settings(&claude_dir);
        assert_eq!(count_splitlane_entries(&root, "UserPromptSubmit"), 1);
    ...
    fn merge_does_not_clobber_user_hooks_in_other_events() {
        ...
        assert_eq!(arr.len(), 2, "user's Bash matcher + Splitlane entry");
    ...
    fn qoder_merge_skips_notification_event() {
        ...
        assert!(
            group.get("_splitlane_managed").is_none(),
            "Qoder public schema does not document Splitlane-only markers"
        );
        assert!(
    ...
    fn gemini_nested_merge_writes_official_shape_and_roundtrips() {
        ...
        assert_eq!(inner[0]["name"], json!("splitlane-status"));
        ...
        // No Splitlane-only marker field (stricter parsers).
        assert!(group.get("_splitlane_managed").is_none());
    ...
    fn cursor_flat_merge_stamps_version_and_preserves_user_entries() {
        ...
        let arr = root["hooks"]["preToolUse"].as_array().unwrap();
        assert_eq!(arr.len(), 2, "user entry + splitlane entry");
    ...
    fn managed_guard_install_and_drop_roundtrip_in_clone_dir() {
        ...
        assert!(
            group.get("_splitlane_managed").is_none(),
            "CodeBuddy public schema does not document Splitlane-only markers"
        );
    ...
    fn pi_extension_guard_roundtrip() {
        ...
        let ext = ext_dir.join(SPLITLANE_TS_BASENAME);
        let content = std::fs::read_to_string(&ext).unwrap();
        assert!(
            content.contains("SPLITLANE_SOCKET_PATH"),
            "extension must be env-gated to stay inert outside Splitlane"
        );
        drop(guard);
    ...
    fn opencode_guard_declares_plugin_and_cleans_up() {
        ...
        let plugin = dir.join("plugins").join(SPLITLANE_TS_BASENAME);
        ...
        assert!(entries[0]
            .as_str()
            .unwrap()
            .ends_with(SPLITLANE_TS_BASENAME));
    ...
    fn grok_guard_writes_dedicated_file_and_removes_on_drop() {
        ...
        let path = hooks_dir.join("splitlane.json");
        let root: serde_json::Value =
        ...
        assert!(
            ...
            "Grok's stdin payload has no notification_type, so the hook's own \
             whitelist would drop every frame"
        );
        assert!(
            root["hooks"].get("PermissionRequest").is_none(),
            "an approval is answered in Grok's own terminal - the hook binary \
             has no arm for that event at all"
        );
        assert!(
            root["hooks"]["PreToolUse"][0]
                .get("_splitlane_managed")
                .is_none(),
            "Grok public docs do not document Splitlane-only markers"
        );
    ...
    fn hermes_guard_appends_block_and_strips_on_drop() {
        ...
        assert!(
            !content.contains("pre_approval_request"),
            "an approval is answered in Hermes' own terminal"
        );
    ...
    // ---------- TOML feature-flag mutation (Unix) ----------
    ...
    fn enable_codex_feature_flag_concurrent_no_duplicate_features() {
        // Two concurrent shims racing to enable the flag must not
    ...
    // ---------- Windows JSONL parser + argv rewrite ----------
    //
    ...
    #[cfg(not(unix))]
    #[test]
    fn parse_codex_event_maps_known_types_to_splitlane_events() {
        assert_eq!(
    ...
    /// Schema-pin test: assert that every entry in
    ...
    /// version (which used `format!("splitlane-ai-hook {event}")` directly).
    #[test]
    fn is_splitlane_hook_command_accepts_legacy_bare_name() {
        for event in CLAUDE_HOOK_EVENTS {
            let cmd = format!("splitlane-ai-hook {event}");
            assert!(
                is_splitlane_hook_command(&cmd),
    ...
    #[test]
    fn is_splitlane_hook_command_accepts_unix_absolute_path() {
        let cmd = "/home/user/.cache/splitlane/bin/0.1.0/splitlane-ai-hook Stop";
        assert!(is_splitlane_hook_command(cmd));

        let cmd = "/usr/local/bin/splitlane-ai-hook PreToolUse";
        assert!(is_splitlane_hook_command(cmd));
    }

    /// Windows variant: the binary basename is `splitlane-ai-hook.exe` and
    ...
    #[test]
    fn is_splitlane_hook_command_accepts_exe_basename() {
        let cmd = "/some/path/splitlane-ai-hook.exe Stop";
        assert!(is_splitlane_hook_command(cmd));
    }
    ...
    /// `remove_splitlane_hooks` can purge stale entries written by an
    /// earlier splitlane install that has since been removed.
    #[test]
    fn is_splitlane_hook_command_recognizes_orphans_without_filesystem_check() {
        // Path that almost certainly does not exist - the function must NOT
        // touch the filesystem.
        let cmd = "/nonexistent/old/cache/splitlane-ai-hook UserPromptSubmit";
        assert!(
            is_splitlane_hook_command(cmd),
    ...
    /// User hooks must NOT be misclassified as splitlane-managed. The
    ...
    #[test]
    fn is_splitlane_hook_command_rejects_user_hooks() {
        let user_hooks = [
            "echo hello",
            "/usr/bin/git status",
            "node my-hook.js",
            "splitlane-shim Stop",       // sibling binary, different name
            "my-splitlane-ai-hook Stop", // similar but distinct basename
            "/path/to/splitlane-ai-hook-2 Stop", // suffixed name
            "",                          // empty
            "   ",                       // whitespace only
        ...
        for cmd in user_hooks {
            assert!(
                !is_splitlane_hook_command(cmd),
                "user hook {cmd:?} must NOT be classified as splitlane-managed"
            );
        }
    }

    /// Round-trip property: `resolve_hook_command` must produce a string
    /// that `is_splitlane_hook_command` recognizes, regardless of which
