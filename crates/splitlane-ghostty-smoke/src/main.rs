...
mod linux {
    ...
    use splitlane_terminal_ghostty::{DisplayTerminal, WindowSize};

    const MARKER: &str = "SPLITLANE_GHOSTTY_PACKAGE_OK";
    ...
    pub(super) fn run() -> Result<()> {
        ...
        std::thread::Builder::new()
            .name("splitlane-ghostty-package-smoke".into())
            .spawn(move || {
...
mod windows {
    ...
    use splitlane_terminal_ghostty::{
        Color, DisplayTerminal, Key, KeyAction, KeyInput, Modifiers, Rgb, WindowSize,
    };

    const MARKER: &str = "SPLITLANE_GHOSTTY_WINDOWS_HEADLESS_OK";
