...
use crate::schema::SplitlaneConfig;
...
/// Watches the Splitlane config file for changes and triggers hot-reload.
///
...
pub struct ConfigWatcher {
    callback: Arc<dyn Fn(SplitlaneConfig) + Send + Sync>,
    config_path: PathBuf,
}
...
impl ConfigWatcher {
    ...
    pub fn new(callback: Arc<dyn Fn(SplitlaneConfig) + Send + Sync>) -> Option<Self> {
    ...
    #[cfg(test)]
    fn new_with_path(path: PathBuf, callback: Arc<dyn Fn(SplitlaneConfig) + Send + Sync>) -> Self {
        Self {
            callback,
            config_path: path,
        }
    ...
    pub fn start(&self) -> Result<(), notify::Error> {
        ...
        // `config_path()` (e.g., `/home/u/.config/splitlane/splitlane.json`),
...
fn attempt_reload(
    config_path: &Path,
    current_config: &mut SplitlaneConfig,
    callback: &Arc<dyn Fn(SplitlaneConfig) + Send + Sync>,
) {
    // Read through the shared helper so the oversize guard (cheap stat
    ...
    // Parse exactly once. A syntax error keeps the previous config
    ...
    // A save that didn't actually change the parsed config (whitespace,
...
mod tests {
    ...
    fn test_config_watcher_new_with_path() {
        ...
        let cb = Arc::new(|_: SplitlaneConfig| {});
    ...
    fn test_event_targets_config() {
        let config_path = PathBuf::from("/tmp/splitlane/splitlane.json");

        let matching_event = Event {
            kind: EventKind::Modify(notify::event::ModifyKind::Data(
                notify::event::DataChange::Content,
            )),
            paths: vec![PathBuf::from("/tmp/splitlane/splitlane.json")],
        ...
        let non_matching_event = Event {
            ...
            paths: vec![PathBuf::from("/tmp/splitlane/other.json")],
            attrs: Default::default(),
        };
    ...
    fn test_attempt_reload_missing_file_keeps_old_config() {
        ...
        let mut current = SplitlaneConfig {
            default_shell: Some("/bin/bash".to_string()),
            ..Default::default()
        };
        let called = Arc::new(Mutex::new(false));
        let called_clone = Arc::clone(&called);
        let cb: Arc<dyn Fn(SplitlaneConfig) + Send + Sync> =
    ...
    fn test_attempt_reload_valid_config_calls_callback() {
        ...
        let mut current = SplitlaneConfig::default();
        let received = Arc::new(Mutex::new(None::<SplitlaneConfig>));
    ...
    fn test_attempt_reload_unchanged_config_skips_callback() {
        // A reload whose parsed result equals the current config must
    ...
    fn test_attempt_reload_oversize_file_rejected() {
        // Negative test: the hot reload path now applies the same
    ...
    fn test_watcher_detects_file_change() {
        ...
        let received = Arc::new(Mutex::new(Vec::<SplitlaneConfig>::new()));
        let received_clone = Arc::clone(&received);
        let cb: Arc<dyn Fn(SplitlaneConfig) + Send + Sync> =
